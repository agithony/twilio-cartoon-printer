...
test("static template inventory and samples are valid", () => {
    const definitions = buildDefinitions("https://booth.example.com");
    assert.deepEqual(Object.keys(definitions).sort(), ["broadcast", "delivery", "nudgeDropoff", "promo", "rating"]);
    assert.equal(definitions.delivery.variables[2], "assets/twilio-bug-red.png");
    assert.equal(definitions.delivery.types["twilio/card"].actions.length, 1);
    assert.equal(definitions.rating.types["twilio/quick-reply"].actions.length, 3);
