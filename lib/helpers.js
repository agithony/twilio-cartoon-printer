...
async function sendSms(to, from, body, mediaUrl) {
    ...
    // right sender from the pool based on destination country, and retries
    // across senders in the pool if one fails. Falls back to the raw `from`
    // number when the service SID isn't set (local dev, simple setups).
    const mssid = settings.get("twilioMessagingServiceSid");
    const opts = mssid ? { to, messagingServiceSid: mssid, body } : { to, from, body };
