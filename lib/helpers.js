...
async function sendSms(to, from, body, mediaUrl, options = {}) {
    const channel = require("./channel");
    // Reply on whichever channel the recipient reached us on (SMS or WhatsApp).
    // Callers may force a channel via options.channel; otherwise use the
    // last-known channel for this phone (defaults to SMS for unknown numbers).
    const ch = options.channel || channel.get(to);
    const toAddr = channel.toAddress(to, ch);

    // Prefer the Messaging Service when configured — Twilio auto-picks the
    // right sender from the pool based on destination country/channel, and
    // retries across senders in the pool if one fails. Falls back to the raw
    // `from` number when the service SID isn't set (local dev, simple setups).
    const mssid = settings.get("twilioMessagingServiceSid");
    const opts = mssid
        ? { to: toAddr, messagingServiceSid: mssid }
        : { to: toAddr, from: channel.toAddress(from, ch) };

    // WhatsApp requires an approved Content Template for business-initiated
    // messages sent outside the 24h customer-service window. Callers flag such
    // messages with options.templateKey; we resolve the approved HX SID and
    // send it as a template. When no template is registered we fall back to
    // free-form text (valid only while still inside the session window —
    // Twilio rejects it otherwise, which the catch below logs).
    if (ch === "whatsapp" && options.templateKey) {
        const contentSid = settings.getWhatsAppTemplate(options.templateKey);
        if (contentSid) {
            opts.contentSid = contentSid;
            if (options.contentVariables) {
                opts.contentVariables = typeof options.contentVariables === "string"
                    ? options.contentVariables
                    : JSON.stringify(options.contentVariables);
            }
        } else {
            console.warn(`📱 WhatsApp "${options.templateKey}": no Content Template SID configured — sending free-form (only valid in-session).`);
            opts.body = body;
        }
    } else {
        opts.body = body;
    ...
    try {
        ...
        console.error(`📱 Failed to send message to ${maskPhone(to)}: ${err.message} — retrying in 10s`);
        setTimeout(async () => {
            ...
            try {
                ...
                console.error(`📱 Retry failed for ${maskPhone(to)}: ${retryErr.message}`);
            }
        }, 10000);
    }
}
