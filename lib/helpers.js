...
const { openai, twilioClient, MODELS } = require("./config");
...
async function moderateImage(base64Image) {
    try {
        const response = await withRetry(() => openai.moderations.create({
