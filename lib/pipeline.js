...
async function generateImage(job) {
    ...
    // For multi-subject photos, simplify background and prioritize facial likeness
    if (scene.subjects > 1 && backgroundLine) {
        const before = fullPrompt;
        fullPrompt = fullPrompt.replace(/\n\s*Background:.*$/m, "\nBackground: Simple, clean, softly blurred setting.");
        if (before === fullPrompt) {
            console.log("⚠️  Multi-subject: regex did NOT match any Background: line");
        } else {
            console.log("✅ Multi-subject: replaced detailed background with simple setting");
        }
        fullPrompt += "\n\nMULTIPLE SUBJECTS: Facial likeness for EVERY person is the absolute #1 priority. Each face must be unmistakably the same person from the original photo. Simplify background details and reduce artistic embellishments as needed to preserve perfect facial recognition.";
    } else if (scene.subjects > 1) {
        console.log("⚠️  Multi-subject detected but no backgroundLine to simplify");
    }
