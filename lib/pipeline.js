...
async function generateImage(job) {
    ...
    for (const filename of styleRefFiles) {
        const filePath = path.join(settings.STYLE_REFS_DIR, filename);
        if (fs.existsSync(filePath)) {
    ...
    if (hasStyleRefs) {
        ...
        parts.push(`Art style: Render this portrait in the art style shown in the style reference ${styleRefWord} — match their color palette, line work, shapes, and visual aesthetic. ${stylePrompt}`);
    } else {
