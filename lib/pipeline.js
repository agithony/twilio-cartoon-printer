...
async function generateImage(job) {
    ...
    // Built-in detector: look the key up in the canonical STYLES registry.
    // Previously used `!!styleObj.core`, but `core` is now user-editable on
    // custom styles too — so that shortcut would misclassify customs as built-in
    // and skip the preserve/composition append below.
    ...
    // Emit whenever a brand is present. The brand/wardrobe block is the main
    // source of photoreal drift (words like "realistic", "authentic", "stadium-ready"
    // in the wardrobe fragment), so this reminder is needed for built-in styles too
    // — not only when a style reference image is attached.
    if (hasBrands) {
    ...
    // Final style lock — re-assert the chosen art style at the very end.
    // The brand/wardrobe and background blocks describe subject matter with
    // cue words that can bias the renderer (e.g. "realistic fabric", "packed
    // stadium", "studio lighting"). A last-line restatement of the chosen
    // style keeps the render on-model. Note: this is style-AGNOSTIC on
    // purpose — some styles in the matrix (Magazine Cover, Action Figure,
    // Sports Trading Card) are intentionally photographic, so we don't say
    // "don't be photo". We only insist on "render in THIS style".
    // Skip on reviewer override.
    if (!hasReviewFeedback && hasBrands) {
        const styleLock = styleObj.core || `${styleObj.name || "the selected"} art style`;
