...
async function generateImage(job) {
    ...
    if (bgMode === "ai" && bgRefBuffers.length > 0 && bgAnalysis) {
        ...
        // Skip default background prompt if the style prompt already contains background instructions
        // (custom styles may embed "BACKGROUND: ..." in their prompt text — appending a conflicting
        // default like "Keep the background similar to the original photo" would override the style)
        const styleHasBgInstruction = stylePrompt && /background/i.test(stylePrompt);
