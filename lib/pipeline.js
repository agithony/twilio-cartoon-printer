...
function buildAiReviewPrompt(checks, hasBrandRefs) {
    const criteria = [];
    let n = 1;
    if (checks.likeness !== false) criteria.push(`${n++}. Subject likeness — facial features, hair color/style, skin tone, and overall appearance must match between the original and the portrait.`);
    ...
    if (checks.branding !== false && hasBrandRefs) criteria.push(`${n++}. Branding & logos — images 3+ are brand reference images. Clothing, logos, crests, and text on clothing in the portrait must closely match those references.`);
    if (checks.accessories !== false) criteria.push(`${n++}. Accessories — glasses, hats, jewelry, facial hair, and other visible accessories from the original must be preserved in the portrait.`);
    if (checks.anatomy !== false) criteria.push(`${n++}. Anatomical quality — no extra fingers, merged limbs, distorted faces, or other anatomical anomalies.`);
...
Be thorough but reasonable — minor artistic stylization is expected and acceptable. Focus on clear errors that would make the portrait unacceptable to the subject.

Respond with EXACTLY one of:
- PASS (if all criteria are met)
- FAIL: followed by which criteria failed and a brief explanation of what went wrong`;
}
...
async function aiReviewImage(job) {
    ...
    const prompt = buildAiReviewPrompt(reviewChecks, hasBrandRefs);
