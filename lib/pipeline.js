...
async function generateImage(job) {
    ...
    const brandPrompt = settings.getForEvent("brandPrompt", ev);
    ...
    const brandRefFiles = settings.getForEvent("brandReferenceFiles", ev) || [];
    ...
    // For multi-subject photos, aggressively simplify to prioritize facial likeness
    if (scene.subjects > 1) {
        // 1. Drop brandPrompt (trophy etc) — too token-expensive, steals from face budget
        //    Format: "\n\nApply the following to ALL subjects in the image: ..."
        //    Ends right before "\nClothing:" or "\nBackground:" or end of string
        fullPrompt = fullPrompt.replace(/\n\nApply the following to ALL subjects in the image:[\s\S]*?(?=\nClothing:|\n\s*Background:|$)/, "");

        // 2. Simplify background to solid gradient
        //    Note: stored era prompts have leading space before "Background:" (e.g. " Background:")
        if (backgroundLine) {
            fullPrompt = fullPrompt.replace(/\n\s*Background:.*$/m, "\nBackground: Solid soft gradient.");
        }

        // 3. Simplify clothing — keep era name, drop detailed garment list after " -- "
        //    e.g. "Clothing: 1960s-1970s golf fashion -- fitted polo shirt..." → "Clothing: 1960s-1970s golf fashion."
        fullPrompt = fullPrompt.replace(/(\n\s*Clothing:\s*[^\n]*?)\s+(?:--|—)\s+[^\n]*/m, "$1.");

        // 4. Per-subject positional face preservation
        const posLabels = [];
        if (scene.positions === "left-right pair") {
            posLabels.push("Person on the left", "Person on the right");
        } else {
            for (let i = 0; i < scene.subjects; i++) {
                posLabels.push(`Person ${i + 1}`);
            }
        }
        const faceLines = posLabels.map(p => `${p}: preserve their exact facial likeness — face shape, features, skin tone, expression.`).join("\n");
        const petLine = scene.pets !== "none" ? `\nInclude the ${scene.pets} naturally.` : "";

        fullPrompt += `\n\n${faceLines}${petLine}`;
        fullPrompt += "\n\nFacial likeness for EVERY person is the absolute #1 priority. Each face must be unmistakably the same person from the original photo.";

        console.log("✅ Multi-subject: stripped trophy, simplified clothing/background, added per-face lines");
    }

    // Append admin review feedback (from Reject + Re-analyze with instructions)
    if (job.reviewFeedback) {
        fullPrompt += "\n\nAdditional instruction from reviewer: " + job.reviewFeedback;
...
module.exports = { generateImage, printJob, jobPaths, moveStagedToFinal, cleanupStaged };
