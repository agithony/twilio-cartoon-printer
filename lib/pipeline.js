...
const { downloadImage, moderateImage, detectPerson, analyzeScene, parseScene, compositeWithTemplate, prepareForPrint, withRetry } = require("./helpers");
...
// ── Shared helpers for logging ──────────────────────────────────────────────
// maskPhone matches the formatter used in lib/dashboard.js and lib/outreach.js
// — keeps country code + last 4 digits, shows "Kiosk" for api: prefixes. Used
// anywhere pipeline.js wants to include a phone in a log line without leaking
// the middle digits. (Can't require the dashboard copy because that one pulls
// in the whole admin UI.)
...
// Prompt-logging dedupe. Multi-variant jobs generate the same prompt N times
// (once per sibling); logging the full 2000-char text N times made the logs
// a mess. We log the prompt once per parentJobId, then siblings get a 1-line
// reference to that parent. Bounded Map to avoid unbounded growth across
// many events.
const _loggedPrompts = new Map(); // parentJobId -> first-logged variantId/filePrefix
const LOGGED_PROMPTS_MAX = 200;
function rememberPromptLogged(key, ref) {
    if (_loggedPrompts.size >= LOGGED_PROMPTS_MAX) {
        // Evict oldest entry (Map iteration order is insertion order)
        const oldest = _loggedPrompts.keys().next().value;
        if (oldest !== undefined) _loggedPrompts.delete(oldest);
    }
    _loggedPrompts.set(key, ref);
}

// Heartbeat helpers. Long-running awaits (image gen, 2-4 min) log a short
// "still working" line every 30s so operators tailing the log can tell a
// slow-but-healthy call apart from a wedged one. Not intended for every
// long operation — just the ones that would otherwise be silent for minutes.
const HEARTBEAT_INTERVAL_MS = 30_000;
function startHeartbeat(label, startTime) {
    const start = startTime || Date.now();
    const timer = setInterval(() => {
        const elapsedSec = Math.round((Date.now() - start) / 1000);
        console.log(`⏳ Still generating ${label}... (${elapsedSec}s)`);
    }, HEARTBEAT_INTERVAL_MS);
    // unref() so the heartbeat never keeps the event loop alive on shutdown
...
function jobPaths(job, { staged = false } = {}) {
    ...
    const styleKey = style && activeStyles[style] ? style : (settings.getForEvent("defaultStyle", eventName) || DEFAULT_STYLE);
...
// Azure Files SMB quirk: transient EACCES/EBUSY when another process has
// the file open. Retry briefly. Mirrors lib/queue.js renameWithRetry().
async function renameWithRetry(src, dst, maxAttempts = 4) {
    let lastErr;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try { return await fsp.rename(src, dst); }
        catch (err) {
            lastErr = err;
            ...
            await new Promise((r) => setTimeout(r, base + (process.pid % 37)));
            console.warn(`⚠️  Image rename retry ${attempt + 1}/${maxAttempts} for ${path.basename(src)} after ${err.code}`);
        }
    }
    throw lastErr;
}
...
async function moveStagedToFinal(job) {
    ...
    try {
        for (const [src, dst] of [[staged.inputPath, final.inputPath], [staged.outputPath, final.outputPath], [staged.mmsPath, final.mmsPath]]) {
            if (fs.existsSync(src)) await renameWithRetry(src, dst);
        }
    } finally {
        try {
            const gallery = require("./photogallery");
            gallery.rememberPhotoGalleryJob(job);
            gallery.invalidatePhotoGalleryCache(job.eventName);
        } catch {}
    }
}
...
async function generateImage(job) {
    ...
    const outputProfile = job.outputProfile || settings.getOutputProfile(ev);
    job.outputProfile = outputProfile;
    // API / kiosk jobs skip all outbound SMS — surface failures via job state
    // instead (the caller polls /api/generate/:prefix/status).
    const adapter = job.channel ? channels.ADAPTERS[job.channel] : null;
    const notify = job.noDelivery
        ? async () => {}
        : (toPhone, _fromPhone, body, mUrl) => messaging.send(toPhone, "_raw", {}, {
            _body: body,
            mediaUrl: mUrl || null,
            ...(adapter ? { adapter } : {}),
        });
    const { styleKey, inputPath, outputPath, mmsPath } = jobPaths(job, { staged: true });
    const activeStyles = settings.getActiveStyles(ev);
    ...
    if (moderation.flagged) {
        ...
        await notify(
            userPhone,
            appPhone,
            i18n.t(job.locale, "moderationFail", {}, ev),
        );
    ...
    if (job.cachedScene !== undefined) {
        ...
        if (!hasFace) {
            ...
            require("./still-working").cancel(userPhone);
            await notify(userPhone, appPhone, i18n.t(job.locale, "noFace", {}, ev));
            await fsp.unlink(inputPath);
    ...
    if (outputProfile.orientation === "landscape") {
        fullPrompt += "\n\nOUTPUT COMPOSITION: Create a horizontal 3:2 landscape image. Keep every subject fully inside the frame with comfortable space on both sides. Do not crop heads, faces, or key clothing and branding.";
    }
    ...
    if (scene.subjects > 1) {
        ...
        if (multiMode === "reject") {
            ...
            require("./still-working").cancel(userPhone);
            await notify(userPhone, appPhone, i18n.t(job.locale, "multiSubjectReject", {}, ev));
            await fsp.unlink(inputPath);
    ...
    // Log the full prompt once per multi-variant group. Siblings just get a
    // pointer to the first variant so you can grep for it without three full
    // 2000-char prompt dumps clogging the logs.
    const promptLogKey = job.parentJobId || job.filePrefix;
    const priorRef = job.parentJobId ? _loggedPrompts.get(job.parentJobId) : null;
    if (priorRef) {
        console.log(`📝 Prompt: same as variant ${priorRef} (parent ${job.parentJobId})`);
    } else {
        console.log(`📝 Prompt (direct to ${getModels().imageGen}):\n${fullPrompt}`);
        rememberPromptLogged(promptLogKey, job.filePrefix);
    }
    ...
    if (!fs.existsSync(outputPath)) {
        ...
        // Heartbeat: image generation can take 2-4 minutes silently. Without
        // this, the only log between "🎨 Generating..." and "💾 Saving..." is
        // nothing, which makes slow/hung calls indistinguishable from normal
        // ones. Log a 1-liner every 30s so operators watching the log have a
        // pulse. stopHeartbeat() is called in every exit path below.
        const heartbeatLabel = `${activeStyles[styleKey].name} (${job.filePrefix})`;
        const heartbeat = startHeartbeat(heartbeatLabel, genStart);
        let result;
        try {
            ...
            if (isBadImageBytes) {
                try {
                    ...
                    const rescueHeartbeat = startHeartbeat(heartbeatLabel + " rescue", retryStart);
                    try {
                        result = await withRetry(() => getOpenAI().images.edit(editParams));
                    } finally {
                        stopHeartbeat(rescueHeartbeat);
                    }
        ...
        await compositeWithTemplate(outputPath, outputProfile, ev);

        // 6. Resize to the configured output dimensions and encode as PNG.
        console.log("📐 Preparing image for print...");
        await prepareForPrint(outputPath, outputProfile, ev);
    }
...
async function printJob(job, printerName) {
    ...
    await printImage(outputPath, printerName, job.outputProfile);
    paper.decrement();
}
