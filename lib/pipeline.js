...
function resolveGenParams({ styleObj, bgMode, bgRefCount, configuredModel }) {
    ...
    const params = {
        ...
        // Stickers are square; everything else (incl. exact-bg) is portrait.
        // If a style were ever both sticker and exact-bg, sticker size wins.
...
async function generateImage(job) {
    ...
    const notify = job.noDelivery
        ? async () => {}
        : (toPhone, _fromPhone, body, mUrl) => messaging.send(toPhone, "_raw", {}, { _body: body, mediaUrl: mUrl || null });
    ...
    if (job.cachedScene !== undefined) {
        ...
        if (!hasFace) {
            ...
            require("./still-working").cancel(userPhone);
            await notify(userPhone, appPhone, settings.getMsg("noFace"));
            await fsp.unlink(inputPath);
    ...
    if (scene.subjects > 1) {
        ...
        if (multiMode === "reject") {
            ...
            require("./still-working").cancel(userPhone);
            await notify(userPhone, appPhone, settings.getMsg("multiSubjectReject"));
            await fsp.unlink(inputPath);
    ...
    if (!fs.existsSync(outputPath)) {
        ...
        // Reserved for the sticker post-processing branch (added in a later task).
        const isSticker = genParams.isSticker;
    ...
    if (!styleObj.sticker && !fs.existsSync(mmsPath)) {
