const DEFAULT_PRESERVE =
    "Preserve accurately: skin tone, eye color, hair color, hairstyle, facial hair, glasses, jewelry, clothing, and any visible accessories or distinguishing features. The portrait must be immediately recognizable as the same person.";

const DEFAULT_COMPOSITION =
    "Composition: Portrait framing from the chest up, positioned naturally as they appear in the original photo.";
...
const STYLES = {
    cartoon: {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a 3D animated character portrait in the style of Pixar and Disney Animation Studios (The Incredibles, Coco, Up, Brave). Do not add anyone not in the original photo.",
            preserve + " The cartoon should be immediately recognizable as the person in the original photo.",
            "Style: High-quality Pixar-style 3D render with warm, rich color grading. Big round expressive eyes with detailed iris reflections, soft rounded features with a touch of stylization. Smooth subsurface scattering on skin for that signature Pixar porcelain glow. Rich fabric and hair textures with individual strand detail. Every surface should have the polished, tactile quality of a Pixar film frame.",
            "Body proportions: Keep the body realistically proportioned — normal adult height, limbs, and torso like The Incredibles characters. Do NOT make the body short, stocky, or compressed. The face and head can be slightly stylized but the body should look natural and well-proportioned.",
            ...
            "Composition: Frame from the waist up. Give them a warm, friendly expression.",
    ...
    "pop-art": {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a bold pop art portrait in the style of Andy Warhol and Roy Lichtenstein. Do not add anyone not in the original photo.",
            preserve,
    ...
    watercolor: {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a beautiful watercolor painting portrait. Do not add anyone not in the original photo.",
            preserve,
    ...
    anime: {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a Japanese anime-style character portrait. Do not add anyone not in the original photo.",
            preserve,
    ...
    sketch: {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a detailed pencil sketch portrait. Do not add anyone not in the original photo.",
    ...
    "pixel-art": {
        ...
        buildPrompt: (preserve, composition) => [
            "Transform this photo into a pixel art portrait in a retro 16-bit video game style. Do not add anyone not in the original photo.",
            preserve,
...
function getActiveStyles(disabledStyles, customStyles, stylePromptOverrides, preserveLine, compositionLine) {
    ...
    if (customStyles) {
        for (const [key, style] of Object.entries(customStyles)) {
            if (!disabledStyles || !disabledStyles.includes(key)) {
                active[key] = { name: style.name, prompt: style.prompt, files: style.files || [] };
            }
        }
    }
    return active;
