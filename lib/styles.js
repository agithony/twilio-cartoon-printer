...
function parseStyle(body, activeStyles) {
    const styles = activeStyles || STYLES;
    const keys = Object.keys(styles);
    const defaultKey = keys.includes(DEFAULT_STYLE) ? DEFAULT_STYLE : keys[0] || DEFAULT_STYLE;
...
function getActiveStyles(disabledStyles, customStyles) {
    const active = {};
...
function getActiveStyleList(disabledStyles, customStyles) {
    return Object.keys(getActiveStyles(disabledStyles, customStyles));
}

module.exports = { STYLES, DEFAULT_STYLE, STYLE_LIST, parseStyle, getActiveStyles, getActiveStyleList };
