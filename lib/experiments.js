        ...
        const card = (m) => {
            ...
            return `<div class="card card-compact run-card" data-run-id="${escapeHtml(m.id)}" data-run-name="${escapeHtml(m.name)}" data-run-group="${escapeHtml(m.group || "")}">
              <a href="/eval/run/${encodeURIComponent(m.id)}" class="run-card-link">
                <div class="row">
                  <div>
                    <div style="font-weight:600;font-size:15px;margin-bottom:4px;">${escapeHtml(m.name)} ${pill} ${groupLabel}</div>
                    ...
                    const cells = entries.filter((e) => e.photo === p && e.variant === vn)
                        ...
                        .map((e) => {
                            if (e.status === "completed") {
                                ...
                                return `<button type="button" class="img-tile" data-img="${imgUrl}" data-prompt-url="${promptUrl}" data-caption="${escapeHtml(vn + " · " + p + " · rep " + e.rep + (durLabel ? " · " + durLabel : ""))}" style="border:none;padding:0;background:none;cursor:zoom-in;position:relative;">
