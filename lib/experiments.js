...
function buildRouter() {
    ...
    router.get("/", async (req, res) => {
        ...
        const running = summaries.filter((m) => m.status === "running");
        const recent = summaries.filter((m) => m.status !== "running").slice(0, 200);
        ...
        const card = (m) => {
            ...
            return `<div class="card card-compact run-card" data-run-id="${escapeHtml(m.id)}" data-run-name="${escapeHtml(m.name)}" data-run-group="${escapeHtml(m.group || "")}">
              <a href="/eval/run/${encodeURIComponent(m.id)}" class="run-card-link">
                <div class="row">
                  <div>
                    <div style="font-weight:600;font-size:15px;margin-bottom:4px;">${escapeHtml(m.name)} ${pill} ${groupLabel} ${starBadge}</div>
