    ...
    router.get("/", async (req, res) => {
        ...
        const card = (m) => {
            ...
            return `<div class="card card-compact run-card" data-run-id="${escapeHtml(m.id)}" data-run-name="${escapeHtml(m.name)}" data-run-group="${escapeHtml(m.group || "")}">
              ...
              <button type="button" class="run-move" data-move-run="${escapeHtml(m.id)}" aria-label="Move to group" title="Move to group">⇄</button>
        ...
        const body = `
          ...
          <style>
            .run-card { position: relative; padding-right: 80px; }
            .run-card-link { display: block; color: inherit; text-decoration: none; }
            .run-card-link:hover { text-decoration: none; }
            .run-card-link:hover * { text-decoration: none; }
            .run-delete { position: absolute; top: 10px; right: 10px; width: 28px; height: 28px; border: none; border-radius: 6px; background: transparent; color: var(--th-text-muted); font-size: 20px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 2; }
            .run-delete:hover { background: #fee2e2; color: #991b1b; }
            .run-move { position: absolute; top: 10px; right: 44px; width: 28px; height: 28px; border: none; border-radius: 6px; background: transparent; color: var(--th-text-muted); font-size: 15px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 2; }
            .run-move:hover { background: var(--th-border-subtle); color: var(--th-text); }
