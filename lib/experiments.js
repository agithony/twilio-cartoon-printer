...
// Canonical filename-segment sanitizer. The HTTP route that serves generated
// PNGs strips anything outside [A-Za-z0-9._-], so the same set must be used at
// write time or the reader will 404 on filenames containing spaces, parens,
// slashes, etc. (happens with user-typed custom variant names like "New Twilio
// Illustration"). Runs of disallowed chars collapse to a single '-', and
// leading/trailing dashes are trimmed per segment.
function sanitizeFilenameSegment(seg) {
    return String(seg ?? "")
...
async function saveManifest(manifest) {
    ...
    const next = prev.catch(() => {}).then(async () => {
        ...
        // Refresh the cached summary in-place (avoids re-reading from disk).
        _summaryCache.set(manifest.id, summarizeManifest(manifest));
    });
...
// Debounced save wrapper for the runner. Under load (dozens of parallel
// entries) we previously called saveManifest 4× per entry (running/complete
// plus internal state); every call rewrote the whole JSON. We now coalesce
// non-terminal writes to ~2s windows per manifest id. Callers that care about
// durability (run completion, API patches) call saveManifest directly, which
// always hits disk through the per-id queue above.
const MANIFEST_DEBOUNCE_MS = 2000;
const _debounceTimers = new Map();     // id → NodeJS.Timeout
const _debouncePending = new Map();    // id → manifest reference (latest)

function debouncedSaveManifest(manifest) {
    _debouncePending.set(manifest.id, manifest);
    // Always refresh the in-memory summary so /eval reflects the latest state
    // even before the debounced disk write fires. Disk is authoritative for
    // restarts; the cache is authoritative for live UI.
    _summaryCache.set(manifest.id, summarizeManifest(manifest));
    if (_debounceTimers.has(manifest.id)) return Promise.resolve();
    return new Promise((resolve) => {
        const timer = setTimeout(async () => {
            _debounceTimers.delete(manifest.id);
            const m = _debouncePending.get(manifest.id);
            _debouncePending.delete(manifest.id);
            if (m) {
                try { await saveManifest(m); }
                catch (err) { console.warn(`[experiments] debounced save failed ${m.id}: ${err.message}`); }
            }
            resolve();
        }, MANIFEST_DEBOUNCE_MS);
        _debounceTimers.set(manifest.id, timer);
    });
}

async function flushDebouncedSave(manifestId) {
    const timer = _debounceTimers.get(manifestId);
    if (timer) { clearTimeout(timer); _debounceTimers.delete(manifestId); }
    const pending = _debouncePending.get(manifestId);
    if (pending) {
        _debouncePending.delete(manifestId);
        await saveManifest(pending);
    }
}
...
// Per-run summary cache keyed by manifest id. The /eval index page and the
// /api/runs listing only need small header-level fields (status, totals,
// cost, group), but we previously re-parsed every full manifest on every
// request. Now we parse once and cache the derived summary; writes invalidate
// the cache entry via invalidateRunSummary().
...
function summarizeManifest(m) {
    ...
    return {
        ...
        group: m.group || "",
        eventName: (m.config && m.config.eventName) || "",
        totalCostUsd: m.totalCostUsd || 0,
        totals,
        wallMs: runWallTimeMs(m),
        starredCount: Array.isArray(m.starred) ? m.starred.length : 0,
    };
}

function invalidateRunSummary(id) {
    if (id) _summaryCache.delete(id);
...
async function listRunSummaries() {
    ...
    try {
        const entries = await fsp.readdir(EXPERIMENTS_DIR, { withFileTypes: true });
        dirs = entries.filter((e) => e.isDirectory() && e.name !== "test-photos");
    } catch (err) {
        if (err.code === "ENOENT") return [];
        throw err;
    }
    const summaries = [];
    for (const d of dirs) {
        if (_summaryCache.has(d.name)) { summaries.push(_summaryCache.get(d.name)); continue; }
        try {
            const m = await loadManifest(d.name);
            const s = summarizeManifest(m);
            _summaryCache.set(d.name, s);
            summaries.push(s);
        } catch (err) {
            console.warn(`[experiments] Skipping malformed manifest in ${d.name}: ${err.message}`);
        }
    }
                ...
                for (const background of config.backgrounds) {
                    for (const variant of config.variants) {
                        for (let rep = 1; rep <= reps; rep++) {
                            ...
                            const parts = [
                                path.parse(photo).name,
                                style, brandSeg, bgSeg, variant.name,
                            ].map(sanitizeFilenameSegment);
                            entries.push({
                                photo, style, brand, background,
                                variant: variant.name, rep,
                                status: "pending",
                                outputPath: `${parts.join("_")}_${rep}.png`,
                                promptText: null,
                                generationMs: null,
                                error: null,
                                startedAt: null,
...
async function runEntry(entry, manifest, deps) {
    ...
    try {
        ...
        const runDir = experimentDir(manifest.id);
        await fsp.writeFile(path.join(runDir, entry.outputPath), pngBuf);
        // Sidecar: prompt text next to the PNG. Served lazily on tile open so
        // the manifest + list HTML stay small. Non-fatal if the write fails —
        // manifest.promptText is still the authoritative fallback.
        const promptFile = entry.outputPath.replace(/\.png$/, ".prompt.txt");
        try { await fsp.writeFile(path.join(runDir, promptFile), promptText, "utf8"); }
        catch (err) { console.warn(`[experiments] prompt sidecar failed for ${promptFile}: ${err.message}`); }
...
async function runExperiment(manifest, deps) {
    ...
    // Terminal transitions use flushFinal (sync save); falls back to flushManifest for tests.
    const flushFinal = deps.flushFinal || deps.flushManifest;
    await flushFinal(manifest);
...
// One-time migration: before the sanitizer was added, outputPath could contain
// spaces/parens/etc. (e.g. "... _New Twilio Illustration_1.png"). The image
// route strips those chars at read time, so such files 404 in the UI. Walk
// every run, rename any file whose name changes under the sanitizer, and
// update the matching entry.outputPath in the manifest. Idempotent — skips
// files that already match, and uses a suffix if the target already exists.
async function migrateFilenamesToSanitized() {
    let manifests;
    try { manifests = await listManifests(); }
    catch (err) { console.warn(`[experiments] migrateFilenames: listManifests failed: ${err.message}`); return; }

    for (const m of manifests) {
        const dir = experimentDir(m.id);
        let changed = false;
        for (const entry of m.entries || []) {
            const oldName = entry.outputPath;
            if (!oldName || typeof oldName !== "string") continue;
            // Re-derive the canonical sanitized form from scratch, then append rep+ext.
            const parts = [
                path.parse(entry.photo || "").name,
                entry.style || "", entry.brand || "none", entry.background || "none",
                entry.variant || "",
            ].map(sanitizeFilenameSegment);
            const newName = `${parts.join("_")}_${entry.rep}.png`;
            ...
            try {
                ...
                // Old file isn't on disk (failed entry, already migrated elsewhere) —
                // still update the manifest so the UI builds the correct URL.
                entry.outputPath = target;
                changed = true;
                continue;
            }
            // If target already exists and isn't the same inode, disambiguate.
            let collisionCount = 0;
            while (true) {
                try { await fsp.access(targetPath, fs.constants.F_OK); }
                catch { break; }
                collisionCount += 1;
                target = newName.replace(/\.png$/, `-m${collisionCount}.png`);
                targetPath = path.join(dir, target);
                if (collisionCount > 20) { target = null; break; }
            }
            if (!target) {
                console.warn(`[experiments] migrateFilenames: giving up on ${oldPath} (too many collisions)`);
                continue;
            }
            try {
                ...
                entry.outputPath = target;
                changed = true;
                console.log(`[experiments] migrateFilenames: ${m.id}/${oldName} → ${target}`);
            } catch (err) {
                console.warn(`[experiments] migrateFilenames: rename failed ${oldPath}: ${err.message}`);
            }
        }
        if (changed) {
            try { await saveManifest(m); }
            catch (err) { console.warn(`[experiments] migrateFilenames: saveManifest(${m.id}) failed: ${err.message}`); }
        }
    }
}
...
async function defaultCallImageEdit({ entry, promptText }) {
    ...
    // gpt-image-1.5 supports. Force the model for this mode only; other
    // modes use the configured model.
    ...
    const portraitBuf = Buffer.from(data.b64_json, "base64");

    // Exact background mode: composite the transparent-alpha portrait onto
    // the literal uploaded background image. Mirrors pipeline.js so /eval
    // matches production.
    if (bgMode === "exact" && bgRefBuffers.length > 0) {
        try {
            const meta = await sharp(portraitBuf).metadata();
            return await compositeExactBackground({
                portraitBuf,
            ...
            console.error(`[experiments] Exact background compositing failed (using portrait as-is): ${bgErr.message}`);
        }
    }
...
function runWallTimeMs(manifest) {
    if (!Array.isArray(manifest.entries) || !manifest.entries.length) return null;
...
<script>
// Shared admin-page pending-review polling — toggles body.review-pending
// so twilio-brand.css can pulse the screen edges.
(function() {
  function poll() {
...
function buildRouter() {
    ...
    router.get("/", async (req, res) => {
        ...
        const showAllEvents = req.query.allEvents === "1";
        const visible = showAllEvents
            ? summaries
            : summaries.filter((m) => (m.eventName || "") === activeEvent);
        const running = visible.filter((m) => m.status === "running");
        const recent = visible.filter((m) => m.status !== "running").slice(0, 200);
        const hiddenCount = summaries.length - visible.length;

        const card = (m) => {
            const { done, failed, total } = m.totals;
            const pill = `<span class="pill pill-${m.status}">${m.status}</span>`;
            const cost = m.totalCostUsd ? `$${m.totalCostUsd.toFixed(2)}` : "";
            const wallMs = m.wallMs;
            const wallLabel = wallMs ? formatMs(wallMs) : "";
            const groupLabel = m.group ? `<span class="run-group-tag">${escapeHtml(m.group)}</span>` : "";
            const eventLabel = showAllEvents && m.eventName
                ? `<span class="run-event-tag" title="Event">${escapeHtml(m.eventName)}</span>`
                : "";
            const starBadge = m.starredCount > 0
                ? `<span class="run-star-badge" title="${m.starredCount} starred image${m.starredCount === 1 ? "" : "s"}"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>${m.starredCount}</span>`
                : "";
            return `<div class="card card-compact run-card" data-run-id="${escapeHtml(m.id)}" data-run-name="${escapeHtml(m.name)}" data-run-group="${escapeHtml(m.group || "")}">
              <a href="/eval/run/${encodeURIComponent(m.id)}" class="run-card-link">
                <div class="row">
                  <div>
                    <div style="font-weight:600;font-size:15px;margin-bottom:4px;">${escapeHtml(m.name)} ${pill} ${groupLabel} ${eventLabel} ${starBadge}</div>
                    <div class="muted">${escapeHtml(new Date(m.createdAt).toLocaleString())} · ${done}/${total} done${failed ? ` · <span style="color:#ef4444;">${failed} failed</span>` : ""}${wallLabel ? ` · ${wallLabel}` : ""}${cost ? ` · ${cost}` : ""}</div>
                  </div>
                </div>
              </a>
              <button type="button" class="run-move" data-move-run="${escapeHtml(m.id)}" aria-label="Move to group" title="Move to group"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg><span>Move</span></button>
              <button type="button" class="run-delete" data-delete-run="${escapeHtml(m.id)}" aria-label="Delete run" title="Delete run"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg><span>Delete</span></button>
            </div>`;
        };
        ...
        const eventToggle = activeEvent
            ? (showAllEvents
                ? `<a class="btn btn-secondary btn-sm" href="/eval" title="Show only runs for ${escapeHtml(activeEvent)}">Showing all events · filter to <strong>${escapeHtml(activeEvent)}</strong></a>`
                : `<a class="btn btn-secondary btn-sm" href="/eval?allEvents=1" title="Show runs from every event">Event: <strong>${escapeHtml(activeEvent)}</strong>${hiddenCount > 0 ? ` · ${hiddenCount} hidden` : ""} · show all</a>`)
            : "";
        ...
        const body = `
          ...
          <div class="group-tabs" id="groupTabs" role="tablist">
            <button type="button" class="group-tab" data-group-filter="" role="tab">All</button>
            ${allGroupNames.map((g) => `<button type="button" class="group-tab" data-group-filter="${escapeHtml(g)}" role="tab">${escapeHtml(g)}</button>`).join("")}
            <button type="button" class="group-tab" data-group-filter="__ungrouped__" role="tab">Ungrouped</button>
          </div>
          ...
          <div id="confirmModal" class="confirm-modal" style="display:none;" role="dialog" aria-modal="true" aria-labelledby="confirmTitle">
            <div class="confirm-backdrop"></div>
            <div class="confirm-card">
              <div class="confirm-title" id="confirmTitle">Delete run?</div>
              <div class="confirm-body" id="confirmBody"></div>
              <div class="confirm-actions">
                <button type="button" class="btn btn-secondary btn-sm" id="confirmCancel">Cancel</button>
                <button type="button" class="btn btn-sm confirm-danger" id="confirmOk">Delete</button>
              </div>
            </div>
          </div>

          <div id="moveModal" class="confirm-modal" style="display:none;" role="dialog" aria-modal="true" aria-labelledby="moveTitle">
            <div class="confirm-backdrop"></div>
            <div class="confirm-card">
              <div class="confirm-title" id="moveTitle">Move to group</div>
              <div class="confirm-body">
                <div id="moveChips" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;"></div>
                <label style="display:block;font-size:12px;color:var(--th-text-muted);margin-bottom:4px;">Or type a new group name</label>
                <input type="text" id="moveInput" maxlength="40" placeholder="e.g. PGA Golfer, Coachella" style="width:100%;padding:8px 10px;border:1px solid var(--th-card-border);border-radius:6px;background:var(--th-bg);color:var(--th-text);font-family:inherit;font-size:13px;">
                <div id="moveError" style="color:#ef4444;font-size:12px;margin-top:6px;min-height:16px;"></div>
              </div>
              <div class="confirm-actions">
                <button type="button" class="btn btn-secondary btn-sm" id="moveClear">Remove from group</button>
                <button type="button" class="btn btn-secondary btn-sm" id="moveCancel">Cancel</button>
                <button type="button" class="btn btn-sm" id="moveOk">Move</button>
              </div>
            </div>
          </div>

          <style>
            .run-card { position: relative; padding-right: 190px; }
            .run-card-link { display: block; color: inherit; text-decoration: none; }
            .run-card-link:hover { text-decoration: none; }
            .run-card-link:hover * { text-decoration: none; }
            .run-move, .run-delete {
                position: absolute;
                top: 50%;
            ...
            .run-move:hover { background: var(--th-border-subtle); border-color: var(--th-text-muted); }
            .run-delete {
                right: 10px;
            ...
            .run-delete:hover { background: #fee2e2; border-color: #fca5a5; color: #991b1b; }
            .run-move svg, .run-delete svg { flex-shrink: 0; }
            .run-group-tag { display: inline-block; padding: 2px 8px; border-radius: 999px; background: var(--th-border-subtle); color: var(--th-text); font-size: 11px; font-weight: 600; margin-left: 4px; letter-spacing: 0.02em; }
            .run-event-tag { display: inline-block; padding: 2px 8px; border-radius: 999px; background: #dbeafe; color: #1e40af; font-size: 11px; font-weight: 700; margin-left: 4px; letter-spacing: 0.02em; }
            .run-star-badge { display: inline-flex; align-items: center; gap: 3px; padding: 2px 8px; border-radius: 999px; background: #fef3c7; color: #92400e; font-size: 11px; font-weight: 700; margin-left: 4px; letter-spacing: 0.02em; vertical-align: middle; }
            .run-star-badge svg { flex-shrink: 0; }
            .group-tabs { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 16px; border-bottom: 1px solid var(--th-card-border); padding-bottom: 8px; }
            .group-tab { background: transparent; border: 1px solid var(--th-card-border); color: var(--th-text-muted); padding: 6px 14px; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; transition: all 0.12s; }
            .group-tab:hover { color: var(--th-text); border-color: var(--th-text-muted); }
            .group-tab.active { background: var(--brand-red); border-color: var(--brand-red); color: white; }
            .move-chip { padding: 4px 10px; border-radius: 999px; border: 1px solid var(--th-card-border); background: var(--th-border-subtle); color: var(--th-text); font-size: 12px; cursor: pointer; font-family: inherit; }
            .move-chip:hover { border-color: var(--brand-red); color: var(--brand-red); }
            .confirm-modal { position: fixed; inset: 0; z-index: 2000; }
            .confirm-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.55); backdrop-filter: blur(2px); }
            .confirm-card { position: relative; max-width: 440px; margin: 18vh auto 0; background: var(--th-card); border: 1px solid var(--th-card-border); border-radius: 12px; padding: 22px 22px 18px; box-shadow: 0 20px 60px rgba(0,0,0,0.5); }
            .confirm-title { font-weight: 700; font-size: 17px; margin-bottom: 8px; color: var(--th-text); }
            .confirm-body { color: var(--th-text); font-size: 14px; line-height: 1.5; margin-bottom: 18px; }
            .confirm-actions { display: flex; justify-content: flex-end; gap: 10px; }
            .confirm-danger { background: #991b1b; border-color: #991b1b; }
            .confirm-danger:hover { filter: brightness(0.92); }
          </style>
          <script>
            (function pollRuns() {
              ...
              const refresh = async () => {
                try {
                  const r = await fetch("/eval/api/runs");
                  if (!r.ok) return;
                  const payload = await r.json();
                  const runs = Array.isArray(payload) ? payload : (payload.runs || []);
                  let needsFullReload = false;
                  for (const m of runs) {
                    const card = document.querySelector('[data-run-id="' + CSS.escape(m.id) + '"]');
                    if (!card) { needsFullReload = true; continue; }
                    const totals = m.totals || { total: 0, done: 0, failed: 0 };
                    const pill = card.querySelector(".pill");
                    if (pill) { pill.className = "pill pill-" + m.status; pill.textContent = m.status; }
                    const meta = card.querySelector(".muted:not(.small)");
                    if (meta) {
                      const created = new Date(m.createdAt).toLocaleString();
                      const cost = m.totalCostUsd ? " · $" + m.totalCostUsd.toFixed(2) : "";
                      const failedPart = totals.failed ? ' · <span style="color:#ef4444;">' + totals.failed + " failed</span>" : "";
                      meta.innerHTML = created + " · " + totals.done + "/" + totals.total + " done" + failedPart + cost;
                    }
                  }
                  if (needsFullReload) location.reload();
                } catch {}
              };
            ...
            (function wireDelete() {
              ...
              function closeConfirm() { modal.style.display = "none"; document.body.style.overflow = ""; pending = null; okBtn.disabled = false; okBtn.textContent = "Delete"; }
              function openConfirm(runId, name, triggerBtn) {
                pending = { runId, triggerBtn };
                body.textContent = "Delete run “" + name + "”? This permanently removes all generated images.";
              ...
              document.addEventListener("keydown", (ev) => {
                ...
                if (ev.key === "Escape") { ev.preventDefault(); closeConfirm(); }
              ...
              okBtn.addEventListener("click", async () => {
                ...
                try {
                  const r = await fetch("/eval/api/runs/" + encodeURIComponent(pending.runId), { method: "DELETE" });
                  if (r.ok) { location.reload(); return; }
                  throw new Error("Delete failed (" + r.status + ")");
                } catch (err) {
                  okBtn.disabled = false;
                  okBtn.textContent = "Delete";
                  body.textContent = err.message || "Delete failed.";
                }
              });

              document.addEventListener("click", (ev) => {
                const btn = ev.target.closest("[data-delete-run]");
                if (!btn) return;
                ev.preventDefault();
                ev.stopPropagation();
                const card = btn.closest(".run-card");
                const name = card ? card.dataset.runName : btn.dataset.deleteRun;
                openConfirm(btn.dataset.deleteRun, name, btn);
              });
            })();
            (function wireGroupTabs() {
              const STORAGE_KEY = "eval-active-group";
              const tabs = document.querySelectorAll(".group-tab");
              if (!tabs.length) return;
              const saved = localStorage.getItem(STORAGE_KEY) || "";

              function applyFilter(filter) {
                tabs.forEach((t) => t.classList.toggle("active", (t.dataset.groupFilter || "") === filter));
                document.querySelectorAll(".run-card").forEach((card) => {
                  const g = card.dataset.runGroup || "";
                  let show;
                  if (filter === "") show = true;
                  else if (filter === "__ungrouped__") show = !g;
                  else show = (g === filter);
              ...
              const exists = [...tabs].some((t) => (t.dataset.groupFilter || "") === saved);
              applyFilter(exists ? saved : "");

              tabs.forEach((t) => t.addEventListener("click", () => applyFilter(t.dataset.groupFilter || "")));
            })();
            (function wireMove() {
              const modal = document.getElementById("moveModal");
              if (!modal) return;
              const chipsWrap = document.getElementById("moveChips");
              const input = document.getElementById("moveInput");
              const errEl = document.getElementById("moveError");
              const okBtn = document.getElementById("moveOk");
              const cancelBtn = document.getElementById("moveCancel");
              const clearBtn = document.getElementById("moveClear");
              const backdrop = modal.querySelector(".confirm-backdrop");
              let pending = null;

              function closeMove() { modal.style.display = "none"; document.body.style.overflow = ""; pending = null; errEl.textContent = ""; input.value = ""; okBtn.disabled = false; okBtn.textContent = "Move"; }
              function openMove(runId, currentGroup) {
                pending = { runId };
                input.value = currentGroup || "";
                errEl.textContent = "";
                const groups = [...new Set([...document.querySelectorAll(".run-card")].map((c) => c.dataset.runGroup).filter(Boolean))].sort();
                chipsWrap.innerHTML = groups.length
                  ? groups.map((g) => '<button type="button" class="move-chip" data-chip="' + g.replace(/"/g, "&quot;") + '">' + g + '</button>').join("")
                  : '<span style="font-size:12px;color:var(--th-text-muted);">No existing groups yet — type a name below.</span>';
                chipsWrap.querySelectorAll(".move-chip").forEach((c) => c.addEventListener("click", () => { input.value = c.dataset.chip; input.focus(); }));
                modal.style.display = "block";
                document.body.style.overflow = "hidden";
                setTimeout(() => input.focus(), 0);
              }
              async function submitGroup(value) {
                if (!pending) return;
                const g = (value || "").trim().slice(0, 40);
                okBtn.disabled = true;
                okBtn.textContent = "Saving…";
                try {
                  const r = await fetch("/eval/api/runs/" + encodeURIComponent(pending.runId), {
                    method: "PATCH",
                    headers: { "content-type": "application/json" },
              ...
              okBtn.addEventListener("click", () => submitGroup(input.value));
              clearBtn.addEventListener("click", () => submitGroup(""));
              cancelBtn.addEventListener("click", closeMove);
              backdrop.addEventListener("click", closeMove);
              document.addEventListener("keydown", (ev) => {
                if (modal.style.display === "none") return;
                if (ev.key === "Escape") { ev.preventDefault(); closeMove(); }
                else if (ev.key === "Enter" && document.activeElement === input) { ev.preventDefault(); okBtn.click(); }
              });

              document.addEventListener("click", (ev) => {
                const btn = ev.target.closest("[data-move-run]");
                if (!btn) return;
                ev.preventDefault();
                ev.stopPropagation();
                const card = btn.closest(".run-card");
                openMove(btn.dataset.moveRun, card ? (card.dataset.runGroup || "") : "");
              });
            })();
          </script>
        `;
    ...
    router.get("/new", async (req, res) => {
        ...
        const photoOptions = photos.length
            ? photos.map((p) =>
                `<label class="inline"><input type="checkbox" name="photos" value="${escapeHtml(p.filename)}" checked> ${escapeHtml(p.displayName || p.filename)}</label>`).join("")
        ...
        const groupDatalist = existingGroups.map((g) => `<option value="${escapeHtml(g)}"></option>`).join("");
        ...
        const body = `
          ...
          <form id="f" class="stack-lg">
            <div class="card">
              ...
              <label style="margin-top:12px;"><span class="label-text">Group (optional)</span>
                <span class="label-hint">Organize related runs under a label (e.g. "pop-art tuning", "launch prep"). Pick an existing group or type a new one. Leave blank for ungrouped.</span>
                <input name="group" list="groupOptions" placeholder="e.g. magazine-cover exploration" maxlength="40">
                <datalist id="groupOptions">${groupDatalist}</datalist>
              </label>
            ...
            <div class="card">
              <div class="row" style="gap:24px;flex-wrap:wrap;align-items:flex-start;">
                <label style="flex:1;min-width:220px;margin-bottom:0;"><span class="label-text">Reps per combination</span>
                  <span class="label-hint">gpt-image-2 is non-deterministic. 5 reps reveals how much variance a prompt has.</span>
                  <input type="number" name="reps" value="5" min="1" max="10" style="max-width:120px;">
                </label>
          ...
          function computeBgMenu(styleKey, brandKey) {
              ...
              if (!isContainer && !isWardrobePlusScene) options.push({ key: "plain-white", name: "Solid White" });
              return options;
          }
          ...
          function buildVariantBlock(preset) {
              ...
              // overrides arrives from the API in v2 shape { styles: {...}, brands: {...} };
              // legacy files in v1 flat shape { <styleKey>: {...} } are migrated on read,
              // but handle both defensively. This form only edits style overrides today.
              const styleOverrides = (preset && preset.overrides && preset.overrides.styles)
              ...
              const stylesForBlock = preset && preset.overrides
                  ? [...new Set([...Object.keys(styleOverrides), ...checkedStyles])]
                  : checkedStyles;
              ...
              stylesForBlock.forEach((sk) => {
                  const base = styleOverrides[sk] || LIVE_STYLES[sk] || { prompt: "", core: "", brandCore: "" };
          ...
          async function saveVariantToLibrary(block, btn) {
              ...
              if (r.ok) { btn.textContent = "Saved"; setTimeout(() => { btn.textContent = orig; btn.disabled = false; }, 1500); }
                  ...
                  const overwrite = r.status === 409
                      ? await twModal.confirm({ title: "Overwrite variant?", message: "Variant '" + name + "' already exists.", confirmText: "Overwrite", danger: true })
                      : false;
                  if (overwrite) {
          ...
          document.getElementById("f").addEventListener("submit", async (e) => {
              ...
              const data = {
                  name: f.name.value,
                  group: f.group ? f.group.value.trim() : "",
                  ...
                  reps: parseInt(f.reps.value, 10) || 5,
    ...
    router.get("/run/:id", async (req, res) => {
        ...
        const starredSet = new Set(Array.isArray(manifest.starred) ? manifest.starred : []);
                    ...
                    const cells = entries.filter((e) => e.photo === p && e.variant === vn)
                        ...
                        .map((e) => {
                            if (e.status === "completed") {
                                ...
                                const durLabel = e.generationMs ? formatMs(e.generationMs) : "";
                                const durBadge = durLabel ? `<span class="tile-duration">${escapeHtml(durLabel)}</span>` : "";
                                const promptUrl = `/eval/prompts/${encodeURIComponent(manifest.id)}/${encodeURIComponent(e.outputPath)}`;
                                const isStarred = starredSet.has(e.outputPath);
                                const starredAttr = isStarred ? " data-starred=\"1\"" : "";
                                const starredClass = isStarred ? " starred" : "";
                                return `<div class="img-tile-wrap${starredClass}" data-output="${escapeHtml(e.outputPath)}"${starredAttr}>
                                  <button type="button" class="img-tile" data-img="${imgUrl}" data-prompt-url="${promptUrl}" data-caption="${escapeHtml(vn + " · " + p + " · rep " + e.rep + (durLabel ? " · " + durLabel : ""))}" style="border:none;padding:0;background:none;cursor:zoom-in;position:relative;">
                                    <img src="${imgUrl}" width="180" loading="lazy" decoding="async" style="border-radius:8px;display:block;box-shadow:0 2px 8px rgba(0,0,0,0.15);transition:transform 0.1s;">
                                    ${durBadge}
                                  </button>
                                  <button type="button" class="tile-star" data-star-output="${escapeHtml(e.outputPath)}" aria-label="${isStarred ? "Unstar image" : "Star image"}" aria-pressed="${isStarred ? "true" : "false"}" title="${isStarred ? "Unstar" : "Star as favorite"}">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="${isStarred ? "currentColor" : "none"}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                                  </button>
                                </div>`;
                            }
        ...
        const body = `
          ...
          <div class="row row-top" style="margin-bottom:8px;">
            <div>
              <h1>${escapeHtml(manifest.name)} <span class="pill pill-${manifest.status}">${manifest.status}</span>${manifest.group ? ` <span class="run-group-tag">${escapeHtml(manifest.group)}</span>` : ""}</h1>
              <div class="subtitle">${escapeHtml(new Date(manifest.createdAt).toLocaleString())} · ${totalsGlobal.done}/${totalsGlobal.total} done${totalsGlobal.failed ? ` · <span style="color:#ef4444;">${totalsGlobal.failed} failed</span>` : ""}${runWallTimeMs(manifest) ? ` · ${formatMs(runWallTimeMs(manifest))} elapsed` : ""}${manifest.totalCostUsd ? ` · $${manifest.totalCostUsd.toFixed(2)}` : ""}</div>
            </div>
            <div class="cluster" style="gap:8px;">
              <button type="button" class="btn btn-secondary btn-sm" id="starredFilterBtn" aria-pressed="false" title="Show only starred images">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                <span id="starredFilterLabel">Starred only (<span id="starredCount">${starredSet.size}</span>)</span>
              </button>
          ...
          <style>
            ...
            .img-tile-wrap { position: relative; display: inline-block; border-radius: 10px; padding: 4px; transition: background 0.15s, box-shadow 0.15s; }
            .img-tile-wrap.starred { background: #fef3c7; box-shadow: 0 0 0 2px #f59e0b, 0 4px 16px rgba(245, 158, 11, 0.35); }
            .img-tile-wrap.starred img { box-shadow: 0 2px 8px rgba(0,0,0,0.2) !important; }
            .tile-star { position: absolute; bottom: 8px; left: 8px; width: 30px; height: 30px; display: inline-flex; align-items: center; justify-content: center; border: none; border-radius: 50%; background: rgba(0,0,0,0.55); color: #fff; cursor: pointer; z-index: 3; transition: background 0.15s, color 0.15s, transform 0.1s; opacity: 0; }
            .img-tile-wrap:hover .tile-star, .img-tile-wrap.starred .tile-star { opacity: 1; }
            .tile-star:hover { background: rgba(0,0,0,0.75); transform: scale(1.08); }
            .img-tile-wrap.starred .tile-star { background: #f59e0b; color: #fff; opacity: 1; }
            .img-tile-wrap.starred .tile-star:hover { background: #d97706; }
            body.starred-only .img-tile-wrap:not(.starred) { display: none; }
            body.starred-only .variant-row:not(:has(.img-tile-wrap.starred)) { display: none; }
            body.starred-only .photo-group:not(:has(.img-tile-wrap.starred)) { display: none; }
            body.starred-only .combo-card:not(:has(.img-tile-wrap.starred)) { display: none; }
            #starredFilterBtn[aria-pressed="true"] { background: #f59e0b; border-color: #f59e0b; color: #fff; }
            .tile-duration { position:absolute;top:6px;right:6px;background:rgba(0,0,0,0.72);color:#fff;font-size:11px;font-weight:600;padding:2px 7px;border-radius:10px;letter-spacing:0.02em;pointer-events:none; }
          ...
          (function pollRun() {
            if (!${running}) return;
            const refresh = async () => {
              const modalOpen = document.getElementById("imgModal")?.style.display === "block";
              const a = document.activeElement;
              const isTyping = a && ["INPUT","TEXTAREA","SELECT"].includes(a.tagName);
              try {
                const r = await fetch("/eval/api/runs/" + encodeURIComponent(runId));
                if (!r.ok) return;
                const m = await r.json();
                const header = document.querySelector("h1 .pill");
                if (header) { header.className = "pill pill-" + m.status; header.textContent = m.status; }
                const totals = m.entries.reduce((a, e) => {
                  a.total++;
                  if (e.status === "completed") a.done++;
                  else if (e.status === "failed") a.failed++;
                  return a;
                }, { total: 0, done: 0, failed: 0 });
                ...
                if (subtitle) {
                  ...
                  for (const e of m.entries) {
                    if (e.startedAt) { const t = new Date(e.startedAt).getTime(); if (t < minStart) minStart = t; }
                    if (e.completedAt) { const t = new Date(e.completedAt).getTime(); if (t > maxEnd) maxEnd = t; }
                  }
                  const liveEnd = m.status === "running" ? Date.now() : maxEnd;
                  const wallMs = Number.isFinite(minStart) && liveEnd > minStart ? (liveEnd - minStart) : null;
                  const fmt = (ms) => ms < 1000 ? ms + "ms" : ms < 60000 ? (ms/1000).toFixed(1) + "s" : Math.floor(ms/60000) + "m " + Math.round((ms%60000)/1000) + "s";
                  const elapsed = wallMs ? " · " + fmt(wallMs) + " elapsed" : "";
                  subtitle.innerHTML = created + " · " + totals.done + "/" + totals.total + " done" + failedPart + elapsed + cost;
                }
                // If generation finished, reload once so the final state (winners UI, etc.) is correct
                if (m.status !== "running") { clearInterval(timer); if (!modalOpen && !isTyping) location.reload(); return; }
                // Update only entries whose status changed (pending/running → completed/failed).
                // Never rewrite an already-rendered tile — that would destroy modal handlers.
                for (const e of m.entries) {
                  if (e.status !== "completed" && e.status !== "failed") continue;
                  const imgUrl = "/eval/images/" + encodeURIComponent(m.id) + "/" + encodeURIComponent(e.outputPath);
                  const existingTile = document.querySelector('[data-img="' + CSS.escape(imgUrl) + '"]');
                  const existingFailed = document.querySelector('[data-retry-output="' + CSS.escape(e.outputPath) + '"]');
                  if (existingTile || existingFailed) continue;
                  // Tile was pending and is now done — reload to pick up the new tile.
                  // Skip reload while user has a modal open or is typing.
                  if (!modalOpen && !isTyping) { location.reload(); return; }
                }
              } catch {}
            };
          ...
          async function fetchPrompt(url) {
              if (_promptCache.has(url)) return _promptCache.get(url);
              try {
                  const r = await fetch(url);
                  const text = r.ok ? await r.text() : "";
                  _promptCache.set(url, text);
                  return text;
              } catch { return ""; }
          }
          async function openModal(tile) {
              modalImg.src = tile.dataset.img;
              modalPrompt.value = "Loading prompt…";
              modalCaption.textContent = tile.dataset.caption;
              modalOpen.href = tile.dataset.img;
              modal.style.display = "block";
              document.body.style.overflow = "hidden";
              currentTile = tile;
              const url = tile.dataset.promptUrl;
              const openedFor = tile;
              const text = url ? await fetchPrompt(url) : "";
              // Avoid overwriting if the user navigated to another tile meanwhile.
              if (currentTile === openedFor) modalPrompt.value = text;
          }
          function closeModal() { modal.style.display = "none"; document.body.style.overflow = ""; modalImg.src = ""; currentTile = null; }
          function allTiles() { return Array.from(document.querySelectorAll(".img-tile")); }
          ...
          function navVertical(dir) {
              if (!currentTile) return;
              const row = currentTile.closest(".variant-row");
              if (!row) return;
              const cells = Array.from(row.querySelectorAll(".variant-cells > *"));
              const colIdx = cells.indexOf(currentTile);
              if (colIdx === -1) return;
              // Walk through every variant-row (across photo-groups and combo-cards) in DOM order.
              const allRows = Array.from(document.querySelectorAll(".variant-row"));
              const rowIdx = allRows.indexOf(row);
              for (let i = rowIdx + dir; i >= 0 && i < allRows.length; i += dir) {
                  const targetCells = Array.from(allRows[i].querySelectorAll(".variant-cells > *"));
                  const candidate = targetCells[colIdx] || targetCells[targetCells.length - 1];
                  if (candidate && candidate.classList.contains("img-tile")) {
                      openModal(candidate);
                      return;
                  }
              }
          ...
          document.addEventListener("keydown", (ev) => {
              if (modal.style.display === "none") return;
              // Don't hijack arrows when the user is focused inside the prompt textarea
              const inTextarea = ev.target && ev.target.tagName === "TEXTAREA";
              if (ev.key === "Escape") { closeModal(); return; }
              if (inTextarea) return;
              if (ev.key === "ArrowRight") { ev.preventDefault(); navHorizontal(1); }
              else if (ev.key === "ArrowLeft") { ev.preventDefault(); navHorizontal(-1); }
              else if (ev.key === "ArrowDown") { ev.preventDefault(); navVertical(1); }
              else if (ev.key === "ArrowUp") { ev.preventDefault(); navVertical(-1); }
          });
          document.querySelectorAll(".img-tile").forEach((btn) => btn.addEventListener("click", () => openModal(btn)));
          ...
          async function toggleStar(outputPath, btn) {
              const wrap = btn.closest(".img-tile-wrap");
              const wasStarred = wrap.classList.contains("starred");
              // Optimistic UI
              wrap.classList.toggle("starred", !wasStarred);
              btn.setAttribute("aria-pressed", !wasStarred ? "true" : "false");
              btn.setAttribute("aria-label", !wasStarred ? "Unstar image" : "Star image");
              btn.setAttribute("title", !wasStarred ? "Unstar" : "Star as favorite");
              const svg = btn.querySelector("svg");
              if (svg) svg.setAttribute("fill", !wasStarred ? "currentColor" : "none");
              if (!wasStarred) wrap.dataset.starred = "1"; else delete wrap.dataset.starred;
              const countEl = document.getElementById("starredCount");
              if (countEl) countEl.textContent = document.querySelectorAll(".img-tile-wrap.starred").length;
              try {
                  const r = await fetch("/eval/api/runs/" + encodeURIComponent(runId) + "/star", {
                      method: "POST",
                      headers: { "content-type": "application/json" },
                      body: JSON.stringify({ outputPath, starred: !wasStarred }),
                  });
                  ...
                  if (wasStarred) wrap.dataset.starred = "1"; else delete wrap.dataset.starred;
                  if (countEl) countEl.textContent = document.querySelectorAll(".img-tile-wrap.starred").length;
                  await twModal.alert({ title: "Couldn't save star", message: err.message });
              }
          }
          document.querySelectorAll(".tile-star").forEach((btn) => {
              btn.addEventListener("click", (ev) => {
                  ev.stopPropagation();
                  ev.preventDefault();
                  toggleStar(btn.dataset.starOutput, btn);
              });
          });

          const filterBtn = document.getElementById("starredFilterBtn");
          if (filterBtn) {
              filterBtn.addEventListener("click", () => {
                  const active = document.body.classList.toggle("starred-only");
          ...
          document.querySelectorAll(".tile-retry").forEach((btn) => btn.addEventListener("click", async (ev) => {
              ...
              try {
                  ...
                  await twModal.alert({ title: "Retry failed", message: err.message });
              }
          }));
          ...
          if (retryAllBtn) {
              retryAllBtn.addEventListener("click", async () => {
                  const ok = await twModal.confirm({
                      ...
                      message: "This will cost one image credit per retry.",
    ...
    router.get("/photos", async (req, res) => {
        ...
        const body = `
          ...
          document.querySelectorAll("[data-del]").forEach((b) => b.addEventListener("click", async () => {
              const ok = await twModal.confirm({ title: "Delete " + b.dataset.del + "?", confirmText: "Delete", danger: true });
              if (!ok) return;
              const r = await fetch("/eval/api/photos/" + encodeURIComponent(b.dataset.del), { method: "DELETE" });
              if (r.ok) location.reload();
              else { const j = await r.json().catch(() => ({})); await twModal.alert({ title: "Delete failed", message: j.error || "Check the server logs." }); }
          }));
          </script>
        `;
    ...
    router.get("/variants", async (req, res) => {
        ...
        const body = `
          ...
          <script>
            ...
            document.querySelectorAll("[data-del]").forEach((b) => b.addEventListener("click", async () => {
                const ok = await twModal.confirm({ title: "Delete variant?", message: "\"" + b.dataset.del + "\" will be removed.", confirmText: "Delete", danger: true });
                if (!ok) return;
    ...
    router.get("/api/runs", async (req, res) => {
        // Returns run summaries (status/totals/cost/group) — not full manifests.
        // Consumers that need entries should call /api/runs/:id.
        try { res.json({ runs: await listRunSummaries() }); }
    ...
    router.post("/api/runs", express.json({ limit: "1mb" }), async (req, res) => {
        try {
            ...
            const trimmedGroup = (group && typeof group === "string") ? group.trim().slice(0, 40) : "";
            const manifest = {
                id, name, createdAt: new Date().toISOString(), status: "pending",
                config, entries, winners: {}, starred: [], notes: "", totalCostUsd: 0,
            ...
            const deps = {
                // Hot path: debounced so per-entry transitions coalesce.
                flushManifest: debouncedSaveManifest,
                // Terminal transitions: durable sync write.
                flushFinal: async (m) => { await flushDebouncedSave(m.id); await saveManifest(m); },
            ...
            runExperiment(manifest, deps).catch(async (err) => {
                console.error(`[experiments] Experiment ${id} crashed: ${err.message}`);
                manifest.status = "failed";
                try { await flushDebouncedSave(id); } catch {}
    ...
    router.patch("/api/runs/:id", express.json(), async (req, res) => {
        try {
            ...
            if (typeof req.body.group === "string") {
                const g = req.body.group.trim().slice(0, 40);
                if (g) manifest.group = g;
    ...
    router.delete("/api/runs/:id", async (req, res) => {
        try {
            ...
            invalidateRunSummary(req.params.id);
            res.json({ ok: true });
        } catch (err) { res.status(500).json({ error: err.message }); }
    });

    router.post("/api/runs/:id/star", express.json(), async (req, res) => {
        try {
            const { outputPath, starred } = req.body || {};
            if (!outputPath || typeof outputPath !== "string") {
                return res.status(400).json({ error: "outputPath required" });
            }
            const manifest = await loadManifest(req.params.id);
            const hasEntry = (manifest.entries || []).some((e) => e.outputPath === outputPath);
            if (!hasEntry) return res.status(404).json({ error: "entry not found" });
            const current = new Set(Array.isArray(manifest.starred) ? manifest.starred : []);
            if (starred) current.add(outputPath);
            else current.delete(outputPath);
            manifest.starred = [...current];
            await saveManifest(manifest);
            invalidateRunSummary(req.params.id);
            res.json({ ok: true, starredCount: manifest.starred.length });
        } catch (err) { res.status(err.code === "ENOENT" ? 404 : 500).json({ error: err.message }); }
    });
    ...
    async function retryEntries(manifest, entries) {
        ...
        (async () => {
            ...
            if (!hasRunning) {
                ...
                manifest.totalCostUsd = manifest.entries.filter((e) => e.status === "completed").length * COST_PER_IMAGE_USD;
                await flushDebouncedSave(manifest.id);
                await saveManifest(manifest);
            }
    ...
    // Lazy prompt-text serving. Tiles load the prompt on open (not in the
    // list HTML) so pages with hundreds of tiles stay fast. Falls back to
    // manifest.promptText for older runs that predate the sidecar files.
    router.get("/prompts/:id/:file", async (req, res) => {
        const safeId = String(req.params.id).replace(/[^A-Za-z0-9._:-]/g, "");
        const safeFile = String(req.params.file).replace(/[^A-Za-z0-9._-]/g, "");
        if (!safeFile.endsWith(".png")) return res.status(400).end();
        const promptName = safeFile.replace(/\.png$/, ".prompt.txt");
        const fp = path.join(experimentDir(safeId), promptName);
        if (!fp.startsWith(EXPERIMENTS_DIR)) return res.status(400).end();
        try {
        ...
        // Fallback: pull from the manifest (older runs without sidecar).
        try {
            const m = await loadManifest(safeId);
            const entry = m.entries.find((e) => e.outputPath === safeFile);
            if (entry && entry.promptText) return res.type("text/plain").send(entry.promptText);
...
async function mountExperiments(app) {
    // Keep boot fast. Only ensureDirs() blocks — the rest runs in the
    // background so Azure Files SMB latency (tens of ms per op × hundreds
    // of manifests) doesn't delay the app coming up and taking traffic.
    await ensureDirs();
    app.use("/eval", buildRouter());
    console.log("[experiments] Tracker mounted at /eval");

    // Background: recover any stale "running" experiments from a prior crash.
    setImmediate(() => {
        recoverStaleExperiments()
            .catch((err) => console.warn(`[experiments] recoverStaleExperiments failed: ${err.message}`));
    });

    // Background: one-time migration of pre-sanitizer filenames. Skip if we've
    // already run it on this volume — the marker file prevents scanning every
    // run on every boot.
    const migrationMarker = path.join(EXPERIMENTS_DIR, ".filename-migration.done");
    setImmediate(async () => {
        try {
            await fsp.access(migrationMarker, fs.constants.F_OK);
            return; // Already migrated.
        } catch {}
        try {
            await migrateFilenamesToSanitized();
            await fsp.writeFile(migrationMarker, new Date().toISOString(), "utf8");
            console.log("[experiments] filename migration complete");
        } catch (err) {
            console.warn(`[experiments] migrateFilenamesToSanitized failed: ${err.message}`);
        }
    });
}
...
module.exports = {
    ...
    migrateFilenamesToSanitized, sanitizeFilenameSegment,
};
