...
function buildRouter() {
    ...
    router.post("/api/runs/:id/entries/:outputPath/retry", async (req, res) => {
        try {
            ...
            // Reset entry state. Clear retriedEntries so the runner's auto-retry-once
            // budget is refreshed for this manual retry attempt.
            ...
            // Bring the manifest back to running so the detail page auto-refreshes.
            ...
            await deps.ensurePhotoSceneCache(entry.photo);

            runEntry(entry, manifest, deps)
                .then(async () => {
                ...
                .catch((err) => console.error(`[experiments] Retry ${req.params.outputPath} crashed: ${err.message}`));
