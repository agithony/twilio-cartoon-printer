    ...
    router.get("/run/:id", async (req, res) => {
                    ...
                    const cells = entries.filter((e) => e.photo === p && e.variant === vn)
                        ...
                        .map((e) => {
                            if (e.status === "completed") {
                                ...
                                return `<button type="button" class="img-tile" data-img="${imgUrl}" data-prompt-b64="${Buffer.from(e.promptText || "", "utf8").toString("base64")}" data-caption="${escapeHtml(vn + " · " + p + " · rep " + e.rep + (durLabel ? " · " + durLabel : ""))}" style="border:none;padding:0;background:none;cursor:zoom-in;position:relative;">
        ...
        const body = `
          ...
          function openModal(tile) {
              const bytes = Uint8Array.from(atob(tile.dataset.promptB64 || ""), (c) => c.charCodeAt(0));
