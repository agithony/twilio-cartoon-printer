...
async function resolveRefsAndAnalyses(entry, manifest) {
    ...
    if (entry.brand && brandObj) {
        ...
        brandPrompt = bOverrides[entry.brand] || brandObj.wardrobe || brandObj.brandPrompt || settings.getForEvent("brandPrompt", ev);
    } else if (entry.brand) {
...
async function defaultBuildPromptForEntry(entry, manifest, photosSnapshot) {
    ...
    // Stash the refs on the entry so callImageEdit can attach them without
    // re-loading. This is a runtime-only field, not persisted.
    entry._refs = refs;
...
function buildRouter() {
    ...
    router.get("/new", async (req, res) => {
        ...
        // Background category toggles — at runtime we expand each combination
        // via resolveBackgroundMenu(style, brand), producing actual scene keys
        // (e.g. cartoon × chelsea-fc → stadium/original/plain-white).
        const bgCategoryOptions = [
            `<label class="inline"><input type="checkbox" name="bgCategories" value="brand-scenes" checked> Brand-specific scenes</label>`,
            `<label class="inline"><input type="checkbox" name="bgCategories" value="original" checked> Original photo background</label>`,
            `<label class="inline"><input type="checkbox" name="bgCategories" value="plain-white" checked> Plain white</label>`,
        ].join("");
        ...
        const body = `
          ...
          <form id="f" class="stack-lg">
            ...
            <fieldset>
              ...
              <div class="legend-hint">Categories expand per combination. For example, <span class="code">cartoon × chelsea-fc</span> with all three checked produces stadium + original + plain-white.</div>
              <div class="options-grid">${bgCategoryOptions}</div>
            </fieldset>
          ...
          document.getElementById("f").addEventListener("submit", async (e) => {
              ...
              const data = {
                  ...
                  bgCategories: [...f.querySelectorAll("[name=bgCategories]:checked")].map(x => x.value),
    ...
    router.post("/api/runs", express.json({ limit: "1mb" }), async (req, res) => {
        try {
            const {
                ...
                bgCategories, backgrounds, eventName, reviewFeedback,
            } = req.body || {};
            ...
            const categories = Array.isArray(bgCategories) && bgCategories.length
                ? bgCategories
                : (Array.isArray(backgrounds) && backgrounds.length ? null : ["brand-scenes", "original", "plain-white"]);
                ...
                for (const style of styles) {
                    ...
                    for (const brand of resolvedBrands) {
                        ...
                        if (categories) {
                            ...
                            if (!bgKeys.length) bgKeys = [null]; // no valid bg for this combo → "style default"
                        } else {
                            bgKeys = backgrounds.length ? backgrounds : [null];
                        }
