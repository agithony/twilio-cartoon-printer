...
async function sendHeldDelivery(phone, held) {
    ...
    if (held.templateKey && settings.getContentSid(held.templateKey, held.locale)) {
        const result = await messaging.send(phone, held.templateKey, held.vars || {}, { ...(adapter ? { adapter } : {}), locale: held.locale });
...
function getActiveSurveyFields(locale = "en") {
...
async function startSurvey(phone, appPhone, eventName, mode, pendingData, locale = "en") {
    const fields = getActiveSurveyFields(locale);
    ...
    if (fields.length === 0) {
        ...
        if (pendingData && mode === "after") {
            try { await sendHeldDelivery(phone, pendingData); } catch (err) {
    ...
    await messaging.send(phone, "_raw", {}, { _body: intro + fields[0].prompt });
...
async function processResponse(phone, messageBody) {
    ...
    const fields = state.fields || getActiveSurveyFields(state.locale);
...
async function completeSurvey(phone) {
    ...
    if (heldMms) {
        ...
        await messaging.send(phone, "_raw", {}, { _body: i18n.t(state.locale, "leadComplete", { firstName }, state.eventName) });
        try {
            await sendHeldDelivery(phone, heldMms);
        } catch (err) {
            console.error(`❌ Held MMS delivery failed for ${maskPhone(phone)}: ${err.message}`);
        }
        activeSurveys.delete(phone);
    ...
    await messaging.send(phone, "_raw", {}, { _body: i18n.t(state.locale, "leadCompleteWithCta", { firstName }, state.eventName) });
    activeSurveys.delete(phone);
    return { status: "completed", locale: state.locale };
}
