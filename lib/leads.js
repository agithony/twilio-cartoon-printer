...
async function completeSurvey(phone) {
    ...
    const summary = [
        `All set, ${answers.firstName}! Here's your info:`,
    ...
    if (heldMms) {
        // "After" mode: send summary then deliver held portrait
        const pickupLine = "\n\nShow this message at the booth to pick up your print!";
        await sendSms(phone, state.appPhone, summary + pickupLine);
        try {
    ...
    if (pendingImage) {
        // "Before" mode with image: summary + let caller enqueue
        await sendSms(phone, state.appPhone, summary);
    ...
    await sendSms(phone, state.appPhone, summary + "\n\nYou're all set! Now send us a selfie to get started.");
    activeSurveys.delete(phone);
    return { status: "completed" };
}
