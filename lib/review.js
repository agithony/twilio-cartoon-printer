...
function renderSingleCard(j) {
  ...
  h += '<div class="rv-img-wrap"><img src="'+origSrc+'" onclick="openSingleModal(\\''+origSrc+'\\',\\''+modalSrc+'\\')" title="Click to compare"><span class="rv-img-label">Original</span></div>';
  h += '<div class="rv-img-wrap"><img src="'+imgSrc+'" onclick="openSingleModal(\\''+origSrc+'\\',\\''+modalSrc+'\\')" title="Click to compare"><span class="rv-img-label">Generated</span></div>';
  h += '</div>';
...
function openSingleModal(origSrc, genSrc) {
  _modalContext = null;
  // Preload both images concurrently before swapping src — prevents the
  // "top-to-bottom reveal" effect that happens when the browser starts
  // decoding only after the <img> becomes visible.
