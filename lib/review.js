...
function renderMultiCard(j) {
  ...
  variants.forEach(function(v, idx){
    ...
    var stateCls = (failed ? " is-failed" : "") + (regenerating ? " is-regen" : "") + (isActive ? " is-active" : "");
    h += '<div class="rv-variant'+stateCls+'" data-variant-idx="'+idx+'" data-variant-id="'+vid+'" id="rvv-'+pid+'-'+idx+'">';
    h += '<div class="rv-v-imgwrap"'+(regenerating ? '' : ' onclick="openMultiModal(\\''+pid+'\\','+idx+')"')+'>';
    ...
    // When regenerating, the server has already deleted the staged image file,
    // so we just leave the img src blank and let the .is-regen stripe animation
    // + text overlay carry the visual state. Otherwise point at the output.
    if (regenerating) {
    ...
    if (failed) {
      ...
      // No Send while regenerating (staged file is gone), just a disabled Regen
      h += '<button class="rv-v-send" disabled title="Wait for regeneration to finish">Send</button>';
      h += '<button class="rv-v-regen" disabled title="'+regenTitle+'">↻ Regen</button>';
    } else {
      h += '<button class="rv-v-send" onclick="pickVariantClick(\\''+pid+'\\',\\''+vid+'\\')" title="Send this variant (variant '+(idx+1)+') to the user via MMS">Send</button>';
...
// ── Mobile swipe gestures on the variant carousel ──────────────────────────
// Event-delegated on document.body so handlers survive DOM re-renders without
// rebinding. Only engages when touch starts inside a .rv-variant-row of a
// multi-variant card AND the viewport is mobile-width.
(function setupSwipeNav() {
  var SWIPE_THRESHOLD_PX = 50;
  var touchStartX = 0, touchStartY = 0, touchActivePid = null, touchStartTime = 0;
  ...
  document.body.addEventListener("touchstart", function(e) {
    // Only meaningful at mobile widths where the carousel is actually stacked
  ...
  document.body.addEventListener("touchend", function(e) {
    ...
    // Swipe left (dx < 0) → next variant; right (dx > 0) → previous.
    cycleMobileVariant(pid, dx < 0 ? 1 : -1);
  }, { passive: true });
