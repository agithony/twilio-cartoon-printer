...
const { authenticateReviewPin, authenticateReviewRequest, getReviewCredentials } = require("./review-auth");
...
function hasReviewLoginCredential() {
    return Boolean(getReviewCredentials().reviewPin);
}

function adminLoginLink() {
    if (!hasAdminAuth()) return "";
    return `<a class="admin-login" href="/auth/login?next=${encodeURIComponent(reviewLandingPath())}">Admin login</a>`;
}

function renderPinPage(res, errorMsg = "", status = 200) {
    const errorHtml = errorMsg ? `<div class="error">${errorMsg}</div>` : "";
    const html = PIN_PAGE_HTML
        .replace("{{ERROR}}", errorHtml)
        .replace("{{ADMIN_LOGIN}}", adminLoginLink());
...
router.get("/", (req, res) => {
    ...
    if (authenticateReviewRequest(req)) {
        return res.redirect(reviewLandingPath());
    }

    if (!hasReviewLoginCredential()) {
        res.setHeader("Cache-Control", "no-store");
        return res.status(403).type("html").send(PIN_DISABLED_HTML.replace("{{ADMIN_LOGIN}}", adminLoginLink()));
    }
    ...
    return renderPinPage(res, errorMsg);
});

router.post("/auth", express.urlencoded({ extended: false, limit: "1kb" }), (req, res) => {
    if (!hasReviewLoginCredential()) return res.status(403).send("Review access PIN not configured");

    const state = pinFailureState(req, "review");
    if (state.count >= PIN_FAILURE_LIMIT) {
        res.setHeader("Retry-After", String(Math.max(1, Math.ceil((state.resetAt - state.now) / 1000))));
        return renderPinPage(res, "Too many failed attempts. Try again later.", 429);
    }

    const credential = authenticateReviewPin(req.body.pin);
    if (!credential) {
    ...
    const token = makeReviewToken(credential);
    const isSecure = (req.headers["x-forwarded-proto"] || req.protocol) === "https";
    const parts = [`review_token=${token}`, "HttpOnly", "SameSite=Lax", "Path=/", `Max-Age=${REVIEW_TOKEN_TTL}`];
    if (isSecure) parts.push("Secure");
    res.append("Set-Cookie", parts.join("; "));
    const legacyParts = ["review_token=", "HttpOnly", "SameSite=Lax", "Path=/review", "Max-Age=0"];
    if (isSecure) legacyParts.push("Secure");
    res.append("Set-Cookie", legacyParts.join("; "));
...
<style>
  ...
  .admin-login { display: inline-block; margin-top: 20px; color: var(--blue-300); font-size: 14px; }
</style>
</head>
<body>
<div class="msg">
  <h1>Review Not Available</h1>
  <p>A Review PIN has not been configured. An administrator can sign in or configure one in Review Settings.</p>
  {{ADMIN_LOGIN}}
</div>
</body></html>`;
...
<style>
  ...
  .admin-login { display: inline-block; margin-top: 20px; color: var(--blue-300); font-size: 13px; }
</style>
</head>
<body>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Placeholder slot for variants that failed and were cleaned up. Keeps
     the grid shape consistent so surviving variants stay thumbnail-sized. */
  ...
  /* Modal action bar (prev / approve / next). Single container so the
     approve button is always visible, independent of whether prev/next
     show for multi-variant modals. Mirrors the bar in /dashboard. */
  .rv-modal-actions {
...
<div id="reviewModal" class="rv-modal" onclick="closeModal(event)">
  <div class="rv-modal-content">
    ...
    <div class="rv-modal-actions" id="reviewModalActions">
      <button class="rv-modal-nav-btn" id="rvModalPrev" onclick="cycleModalVariant(-1)" title="Previous variant" style="display:none">‹</button>
      ...
      <button class="rv-modal-nav-btn" id="rvModalNext" onclick="cycleModalVariant(1)" title="Next variant" style="display:none">›</button>
    </div>
  </div>
</div>
...
function renderSingleCard(j) {
  ...
  // Use the same (already-resized) MMS jpg in the comparison modal —
  // the full-resolution .png is multi-MB and causes a visible top-to-
  // bottom decode when the modal opens.
  var modalSrc = imgSrc;
  ...
  h += '<div class="rv-img-wrap"><img src="'+origSrc+'" onclick="openSingleModal(\\''+origSrc+'\\',\\''+modalSrc+'\\',\\''+fn+'\\')" title="Click to compare"><span class="rv-img-label">Original</span></div>';
  h += '<div class="rv-img-wrap"><img src="'+imgSrc+'" onclick="openSingleModal(\\''+origSrc+'\\',\\''+modalSrc+'\\',\\''+fn+'\\')" title="Click to compare"><span class="rv-img-label">Generated</span></div>';
  h += '</div>';
...
function renderMultiCard(j) {
  ...
  // gridSlots = originally-requested variant count (from server). Drives the
  // grid track count so surviving variants stay thumbnail-sized even when
  // some siblings failed and were cleaned up. variantCount remains the
  // live count (used for carousel dots / mobile navigation).
  ...
  var h = "";
  h += '<div class="rv-card rv-multi'+(sel?" rv-selected":"")+actionsOpenCls+'" id="rv-'+pid+'" style="--variant-count:'+gridSlots+'">';
  ...
  // missing (e.g., permanent failures that got cleaned up).
  ...
  h += '<div class="rv-meta">'+escHtml(j.style||"unknown")+' &middot; '+timeAgo(j.reviewAt)+' &middot; '+variantCount+' of '+gridSlots+' variants</div>';
  ...
  h += '<div class="rv-feedback-hint">Leave blank to re-generate all '+gridSlots+' variants with fresh analysis.</div>';
  h += '<div class="rv-feedback-btns">';
...
// in-modal Approve button. Optional for back-compat.
function openSingleModal(origSrc, genSrc, filename) {
...
function openMultiModal(pid, idx) {
  ...
  // Preload every variant's modal-display image BEFORE showing the modal so
  // arrow-key navigation is instant. We display the MMS jpg (already resized
  // for delivery — ~400KB) rather than _output.png (~4MB full res) so paint
  // is fast and cached fully after the first visit.
...
function renderModalVariant() {
  ...
  var img = document.getElementById("reviewModalImg");
  // Force a visible transition so rapid arrow-keys don't leave the old image
  // painted while the new one loads. Also: blank the src to abort any in-
  // flight load before starting the next one — otherwise the previous load
  // can "win" and overwrite the newer one.
  img.src = "";
  ...
  // Approve is disabled unless the current variant is READY. Matches the
  // variant-card rule used elsewhere — you can only pick what's deliverable.
  var approve = document.getElementById("rvModalApprove");
  if (approve) {
    var canApprove = v && (v.status === "READY" || v.status === undefined);
...
// Approve whatever variant / image is currently visible in the modal.
// Modal closes on success so the reviewer lands back on the queue with
// one fewer card to scan. Delegates to the existing handlers
// (pickVariantClick for multi, doAction for single) so race guards,
// toasts, and optimistic card removal all work uniformly.
async function approveFromModal() {
  ...
  try {
    if (ctx.kind === "multi") {
      ...
      closeModal();
      await pickVariantClick(ctx.parentJobId, v.variantId);
    } else if (ctx.kind === "single") {
      console.log("Approving singleton " + ctx.filename);
      closeModal();
      await doAction(ctx.filename, "approve");
    }
  } finally {
