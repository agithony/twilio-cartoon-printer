...
<div class="wrap">
  <div class="header">
    ...
    <a href="/review/settings" class="header-btn" title="Settings">
      <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><circle cx="10" cy="10" r="2.5"/><path d="M10 1.5v2M10 16.5v2M1.5 10h2M16.5 10h2M3.4 3.4l1.4 1.4M15.2 15.2l1.4 1.4M3.4 16.6l1.4-1.4M15.2 4.8l1.4-1.4"/></svg>
      Settings
    </a>
  </div>
...
function renderMultiCard(j) {
  ...
  variants.forEach(function(v, idx){
    ...
    h += '<div class="rv-variant'+(failed?" is-failed":"")+(isActive?" is-active":"")+'" data-variant-idx="'+idx+'" data-variant-id="'+vid+'" id="rvv-'+pid+'-'+idx+'">';
    h += '<div class="rv-v-imgwrap" onclick="openMultiModal(\\''+pid+'\\','+idx+')">';
    ...
    if (failed) {
      h += '<button class="rv-v-regen" onclick="regenVariant(\\''+pid+'\\',\\''+vid+'\\')" title="Try generating this variant again">↻ Try Again</button>';
    } else {
      h += '<button class="rv-v-send" onclick="pickVariantClick(\\''+pid+'\\',\\''+vid+'\\')" title="Send variant '+(idx+1)+' to the user via MMS">Send</button>';
      h += '<button class="rv-v-regen" onclick="regenVariant(\\''+pid+'\\',\\''+vid+'\\')" title="Regenerate only this variant">↻ Regen</button>';
    }
    h += '</div>';
    h += '</div>';
  });
  ...
  h += '<button class="rv-card-actions-toggle" onclick="toggleCardActions(\\''+pid+'\\')" aria-expanded="false">Card-level actions ▾</button>';
