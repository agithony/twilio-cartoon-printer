...
// Negative results cache for a shorter window so a dub.co outage doesn't
// turn every dashboard poll into a fresh timeout. 15 seconds is enough to
// spare the dashboard while still retrying on a reasonable cadence.
const _analyticsCache = new Map(); // eventKey → { data, ts }
const ANALYTICS_CACHE_TTL = 60_000;
const ANALYTICS_NEGATIVE_TTL = 15_000;
...
async function getAnalytics(eventFilter) {
    ...
    if (cached) {
        ...
        const ttl = cached.data === null ? ANALYTICS_NEGATIVE_TTL : ANALYTICS_CACHE_TTL;
    ...
    try {
        ...
        if (countRes.status === "rejected" && topRes.status === "rejected") {
            ...
            // Cache the failure so the next few dashboard polls don't each
            // wait 5 seconds for the dub.co timeout to fire again.
            _analyticsCache.set(cacheKey, { data: null, ts: Date.now() });
            return null;
        }
