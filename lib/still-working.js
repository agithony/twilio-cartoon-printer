...
function arm(userPhone, appPhone, eventName, adapter, locale = "en") {
    if (!userPhone) return;
    cancel(userPhone);
    ...
    const timer = setTimeout(async () => {
        ...
        try {
            const msg = i18n.t(locale, "stillWorking", {}, eventName);
            if (msg) {
                await messaging.send(userPhone, "_raw", {}, { _body: msg, requiresSession: true, adapter });
