...
const BRB_OVERLAY_CSS = `
  ...
  html[data-theme="light"] #brbOverlay { background: var(--th-bg, #FFFFFF); }
  html[data-theme="light"] .brb-title {
    background: none; color: var(--th-text, #000D25);
    -webkit-text-fill-color: currentColor;
  }
  html[data-theme="light"] .brb-event,
  html[data-theme="light"] .brb-msg,
  html[data-theme="light"] .brb-dismiss { color: var(--th-text-muted, #656E87); }
`;

const BRB_OVERLAY_SCRIPT = `
var brbWakeLock = null;
function toggleBrb() {
  ...
  if (show) {
    ...
    if (navigator.wakeLock) {
      navigator.wakeLock.request("screen").then(function(s) { brbWakeLock = s; }).catch(function() {});
    }
  } else {
    if (brbWakeLock) { brbWakeLock.release().catch(function() {}); brbWakeLock = null; }
  }
}
`;
