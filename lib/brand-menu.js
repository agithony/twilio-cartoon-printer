...
function buildMenu(activeBrands, brandList, opts) {
    ...
    const lines = brandList.map((key, i) => `${i + 1}. ${optionI18n.localizeOption("brand", key, activeBrands[key], opts && opts.locale).name}`);
    if (includeNone) {
        lines.push(`${brandList.length + 1}. ${optionI18n.unbrandedOption(opts && opts.locale).name}`);
    }
...
function matchReply(body, activeBrands, brandList, opts) {
    ...
    if (includeNone && ["none", "unbranded", "nenhum", "nenhuma", "semmarca", "semmarcatwilio", "semtwilio"].includes(optionI18n.fold(text))) return "__none__";

    const norm = optionI18n.fold(text);
    for (const key of brandList) {
        if (optionI18n.optionAliases("brand", key, activeBrands[key]).includes(norm)) return key;
    }
    for (const key of brandList) {
        if (optionI18n.optionAliases("brand", key, activeBrands[key]).some((alias) => alias.length >= 4 && norm.includes(alias))) return key;
    }
