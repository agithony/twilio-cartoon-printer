...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    if (leadSurveyStarted) {
        ...
        const result = await messaging.send(job.userPhone, "delivery", deliveryVars, { adapter });
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => {
                ...
                const send = hasTemplate
                    ? messaging.send(job.userPhone, "promo", {}, { adapter })
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(() => {
                    ...
                    const send = hasRating
                        ? messaging.send(job.userPhone, "rating", {}, jobSendOptions(job))
