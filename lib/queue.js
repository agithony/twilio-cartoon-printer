...
async function toggleAdmin(phone, eventName) {
    ...
    if (targetEvent === activeEvent) {
        ...
        fs.writeFileSync(path.join(dir, "settings.json"), JSON.stringify(eventSettings, null, 2));
    }
    // Rebuild cache since admin status affects counting.
    // Wait for any in-progress build to finish first (its finally block
    // clears _buildingCache), then start a fresh build with updated admin data.
...
function enqueueJob(imageUrl, messageSid, userPhone, appPhone, style, baseUrl, background, brand) {
    ...
    const tmpPath = jobPath + ".tmp." + process.pid;
    ...
    try {
        ...
        if (reviewMode === "ai") {
            ...
            try {
                ...
                // AI review API error — soft fail, auto-approve
                console.error(`⚠️ AI review error, auto-approving: ${err.message}`);
            }
