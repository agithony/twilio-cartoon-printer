...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    if (!adminShortcut) {
        ...
        if (templateKey === "deliveryPrint") {
            // Print delivery: show remaining count or last portrait
            ...
            // Digital delivery: only show last portrait
    ...
    if (!leadSurveyStarted) {
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            const remaining = settings.getForEvent("maxPrints", ev) - (usageCache.get(usageKey(job.userPhone, ev)) || 0);
            const unlimited = isAdmin(job.userPhone) && testing;
            if (remaining <= 0 && !unlimited) {
...
async function getReviewQueue(eventFilter) {
    ...
    try {
        ...
        // First pass: load all jobs; partition into singletons and variant siblings
        const singletons = [];
        const variantGroups = new Map(); // parentJobId -> array of jobs
        for (const filename of files) {
            try {
                ...
                if (job.parentJobId) {
                    ...
                    variantGroups.get(job.parentJobId).push({ ...job, _filename: filename });
                } else {
        ...
        // Variant groups render as one parent-level card each. Only surface the
        // card when ALL expected siblings have reached a terminal state — i.e.,
        // every variant is present in REVIEW_DIR (either READY or FAILED).
        // If siblings are still generating, the group is hidden until complete.
        for (const [parentJobId, siblings] of variantGroups) {
            const expectedCount = siblings[0].variantCount || siblings.length;
            if (siblings.length < expectedCount) continue; // still generating
            ...
            result.push({
                ...
                filePrefix: first.filePrefix, // used by UI for per-variant image URLs via variants[]
...
async function _findVariantSiblings(parentJobId) {
    const out = [];
    try {
        const files = (await fsp.readdir(REVIEW_DIR)).filter((f) => f.endsWith(".json"));
        for (const filename of files) {
            try {
                const job = await readJobAsync(path.join(REVIEW_DIR, filename));
                if (job.parentJobId === parentJobId) {
                    out.push({ filename, job });
                }
            } catch {}
        }
    } catch {}
...
async function pickVariant(parentJobId, variantId) {
    ...
    // Supersede siblings first (cleanup their staged outputs, move their job
    // files out of REVIEW_DIR). Do this BEFORE promoting the winner so that
    ...
    for (const sib of siblings) {
        ...
        try {
            ...
            sib.job.variantStatus = "SUPERSEDED";
            sib.job.failReason = "superseded";
            sib.job.permanent = true;
            sib.job.stateChangedAt = now;
            const src = path.join(REVIEW_DIR, sib.filename);
...
async function regenerateVariant(parentJobId, variantId) {
    ...
    console.log(`🔄 Variant regenerate requested: ${target.filename}`);
}
...
async function rejectParent(parentJobId, message, reanalyze, feedback) {
    ...
    for (const sib of siblings) {
        try {
            await cleanupStaged(sib.job);
            sib.job.variantStatus = "REJECTED";
            sib.job.failReason = "review_rejected";
            sib.job.permanent = true;
            sib.job.stateChangedAt = now;
            const src = path.join(REVIEW_DIR, sib.filename);
            await writeJobAsync(src, sib.job);
