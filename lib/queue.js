...
async function processGeneration(filename, generatingPath) {
    try {
        ...
        const jobIsStickerA = !!(settings.getActiveStyles()[job.style] && settings.getActiveStyles()[job.style].sticker);
        if (shouldPrintJob({ eventPrintingEnabled: settings.getForEvent("enablePrinting", job.eventName), isSticker: jobIsStickerA })) {
...
async function approveJob(filename) {
    ...
    const ev = job.eventName;
        ...
        const jobIsStickerB = !!(settings.getActiveStyles()[job.style] && settings.getActiveStyles()[job.style].sticker);
    if (shouldPrintJob({ eventPrintingEnabled: settings.getForEvent("enablePrinting", ev), isSticker: jobIsStickerB })) {
        job.readyAt = now;
