...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    if (leadSurveyStarted) {
        ...
        await messaging.send(job.userPhone, "_raw", {}, { _body: mmsBody, mediaUrl: skipMms ? null : imageUrl })
    ...
    if (!leadSurveyStarted) {
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => messaging.send(job.userPhone, "promo", { eventName: ev }, { requiresSession: true }).catch((e) => console.error(`❌ Promo send failed: ${e.message}`)), 15000);
        }
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(() => {
                    ...
                    messaging.send(job.userPhone, "npsPrompt", {})
