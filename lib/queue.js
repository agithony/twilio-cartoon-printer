...
// would die in the MMS JPEG, which isn't even created for stickers) and
// never print (printers can't render alpha, and the feature is digital-only).
...
// Resolve whether a job's effective style is a sticker, using the SAME
// style-key fallback that delivery uses (job.style → event default → first
// active → built-in default). All sticker decisions (print gating + delivery
// URL) must agree on this, or a sticker could slip into the print queue.
function resolveJobStyleKey(job) {
    const activeStyles = settings.getActiveStyles();
    if (job.style && activeStyles[job.style]) return job.style;
    const activeStyleList = settings.getActiveStyleList();
    const configuredDefault = settings.getForEvent("defaultStyle", job.eventName) || DEFAULT_STYLE;
    return activeStyleList[0] || configuredDefault;
}

function jobIsSticker(job) {
    const activeStyles = settings.getActiveStyles();
    const key = resolveJobStyleKey(job);
    return !!(activeStyles[key] && activeStyles[key].sticker);
}
...
async function processGeneration(filename, generatingPath) {
    try {
        ...
        if (shouldPrintJob({ eventPrintingEnabled: settings.getForEvent("enablePrinting", job.eventName), isSticker: jobIsSticker(job) })) {
...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    const imageUrl = stickerDeliveryUrl(job, jobIsSticker(job));
...
async function approveJob(filename) {
    ...
    if (shouldPrintJob({ eventPrintingEnabled: settings.getForEvent("enablePrinting", ev), isSticker: jobIsSticker(job) })) {
        job.readyAt = now;
