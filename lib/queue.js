...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    const mmsBody = `${i18n.t(job.locale, templateKey, { styleName: jobStyleName }, ev)}${suffix}${shareLinks}`;
    const richDelivery = adapter && adapter.name === "whatsapp" && settings.getContentSid("delivery", job.locale) && job.baseUrl;
    const deliveryVars = {
        1: richStyleName,
