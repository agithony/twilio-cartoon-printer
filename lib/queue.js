...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    const adapter = jobAdapter(job);
    const richDelivery = adapter && adapter.name === "whatsapp" && settings.getContentSid("delivery", job.locale) && job.baseUrl && !skipMms;
    const deliveryVars = {
        1: richStyleName,
    ...
    if (!leadSurveyStarted) {
        ...
        if (richDelivery) {
            ...
            if (deliveryResult && deliveryResult.error) {
                deliveryResult = await messaging.send(job.userPhone, "_raw", {}, { _body: mmsBody, mediaUrl: skipMms ? null : imageUrl, adapter });
            }
        } else {
            deliveryResult = await messaging.send(job.userPhone, "_raw", {}, jobSendOptions(job, { _body: mmsBody, mediaUrl: skipMms ? null : imageUrl }));
        }
