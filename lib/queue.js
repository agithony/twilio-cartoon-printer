...
// Re-queues a job from DONE_DIR back into READY_DIR so a printer (local or
...
//
// Guards (in order):
//   404 — job not in done/ (only completed jobs can be reprinted)
...
function requeueDoneJobForReprint(filename, { targetPrinter = null } = {}) {
    ...
    const srcPath = path.join(DONE_DIR, safe);
    if (!fs.existsSync(srcPath)) return { ok: false, status: 404, error: "Job not found in done queue" };
