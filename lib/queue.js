...
async function processGeneration(filename, generatingPath) {
    try {
        ...
        if (settings.get("enablePrinting")) {
            ...
            const jobStyle = job.style && activeStyles[job.style] ? job.style : (activeStyleList[0] || DEFAULT_STYLE);
            ...
            try {
                await sendSms(
                    job.userPhone,
                    job.appPhone,
                    `✅ Your ${jobStyleName} portrait is ready!${suffix}${settings.getPromoReturning()}`,
                    imageUrl,
                );
            } catch (smsErr) {
...
async function processPrintQueue() {
    ...
    try {
        ...
        await sendSms(
            job.userPhone,
            job.appPhone,
            `✅ Your ${jobStyleName} portrait is ready! Pick it up at the Twilio booth.${suffix}${settings.getPromoReturning()}`,
            imageUrl,
        );
    } catch (error) {
