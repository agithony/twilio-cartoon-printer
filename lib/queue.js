...
function enqueueJob(imageUrl, messageSid, userPhone, appPhone, style, baseUrl, background, brand, extras) {
    ...
    const baseJob = {
        ...
        userPhone,
        appPhone,
        // Snapshot the messaging channel at enqueue so async sends (delivery,
        // still-working, NPS, promo) always reply on the channel the user used
        // when they submitted this job — not whatever channel they happen to be
        // on when the timer fires.
        channel: channel.get(userPhone),
        style,
        baseUrl: baseUrl || "",
        ...
        if (reviewMode === "ai") {
            ...
            try {
                ...
                if (result.passed) {
                    ...
                    if (!job.noDelivery) {
                        sendSms(job.userPhone, job.appPhone, settings.getMsgForEvent("reviewFailed", job.eventName), null, { channel: job.channel })
...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    if (leadSurveyStarted) {
        ...
        await sendSms(job.userPhone, job.appPhone, mmsBody, skipMms ? null : imageUrl, { channel: job.channel })
    ...
    if (!leadSurveyStarted) {
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => sendSms(job.userPhone, job.appPhone, promoText, null, { channel: job.channel }).catch((e) => console.error(`❌ Promo SMS failed: ${e.message}`)), 15000);
        }
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(() => {
                    ...
                    // NPS can land outside WhatsApp's 24h session window, so
                    // route WhatsApp recipients through an approved template.
                    sendSms(job.userPhone, job.appPhone, settings.getMsgForEvent("npsPrompt", ev), null, { templateKey: "npsPrompt", channel: job.channel })
...
async function rejectJob(filename, message, reanalyze, feedback) {
    ...
    if (reanalyze) {
        ...
        if (message && !job.noDelivery) {
            sendSms(job.userPhone, job.appPhone, message, null, { channel: job.channel })
...
async function rejectParent(parentJobId, message, reanalyze, feedback) {
    ...
    if (message && !firstJob.noDelivery) {
        sendSms(firstJob.userPhone, firstJob.appPhone, message, null, { channel: firstJob.channel })
            .catch((err) => console.error(`❌ Review SMS failed: ${err.message}`));
    }
