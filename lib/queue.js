    ...
    if (fs.existsSync(REVIEW_DIR)) {
        ...
        for (const filename of reviewJobs) {
            ...
            try {
                ...
                for (const [s, d] of [[staged.inputPath, final.inputPath], [staged.outputPath, final.outputPath], [staged.mmsPath, final.mmsPath]]) {
...
async function rejectJob(filename, message, reanalyze) {
    const reviewPath = path.join(REVIEW_DIR, filename);
    if (!fs.existsSync(reviewPath)) throw new Error("Job not found in review queue");
    ...
    // Clear cached scene analysis so next generation re-runs vision AI
    if (reanalyze) delete job.cachedScene;

    // Delete staged output files so generation starts fresh
