        ...
        if (reviewMode === "ai") {
            ...
            try {
                ...
                if (result.passed) {
                    ...
                    if (!job.noDelivery) {
                        messaging.send(job.userPhone, "_raw", {}, jobSendOptions(job, { _body: settings.getMsgForEvent("reviewFailed", job.eventName) }))
                ...
                if (jobRead.retries >= MAX_RETRIES) {
                    ...
                    if (isVariantSibling) {
                        ...
                        if (!jobRead.noDelivery) {
                            messaging.send(jobRead.userPhone, "_raw", {}, jobSendOptions(jobRead, { _body: "Sorry, we couldn't generate your AI portrait this time. Please try sending your photo again!" }))
...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    const adapter = jobAdapter(job);
    const richDelivery = adapter && adapter.name === "whatsapp" && settings.getContentSid("delivery") && job.baseUrl && !skipMms;
    const deliveryVars = {
        1: richStyleName,
    ...
    if (!leadSurveyStarted) {
        let deliveryResult;
        if (richDelivery) {
            deliveryResult = await messaging.send(job.userPhone, "delivery", deliveryVars, { adapter, allowOutOfSession: true });
    ...
    if (!leadSurveyStarted) {
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => {
                const hasTemplate = adapter && adapter.name === "whatsapp" && settings.getContentSid("promo");
                const send = hasTemplate
                    ? messaging.send(job.userPhone, "promo", {}, { adapter, allowOutOfSession: true })
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(async () => {
                    const hasRating = adapter && adapter.name === "whatsapp" && settings.getContentSid("rating");
                    try {
                        const result = await messaging.send(job.userPhone, hasRating ? "rating" : "npsPrompt", {}, jobSendOptions(job, hasRating ? { allowOutOfSession: true } : {}));
