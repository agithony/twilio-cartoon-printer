...
function jobAdapter(job) {
    return job.channel ? channels.ADAPTERS[job.channel] : null;
}
...
async function readClaimedJobWithRetry(filepath, attempts = 8) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
            ...
            lastError = err;
            if (!["ENOENT", "EACCES", "EBUSY"].includes(err.code) || attempt === attempts) throw err;
...
async function processGenerationQueue() {
    ...
    for (const filename of toProcess) {
        ...
        try {
            ...
            // Files can briefly hide the rename destination; the next poll can
            // safely retry if the source remains visible.
            continue;
        }
    ...
    try {
        ...
        if (job.parentJobId) {
            try {
                ...
                // ENOENT is the expected race when the file was already
                // renamed out of GENERATING_DIR — silent to keep log noise
                // down; anything else is worth flagging.
    ...
    for (const filename of printing) {
        ...
        try {
            ...
            if (job.staleRecoveries >= MAX_STALE_RECOVERIES) {
                ...
                await renameWithRetry(filePath, path.join(FAILED_DIR, filename));
...
// Re-queues a job from DONE_DIR or FAILED_DIR back into READY_DIR so a printer (local or
...
function requeueDoneJobForReprint(filename, { targetPrinter = null } = {}) {
    ...
    try {
        ...
        // Do NOT incrementUsage — reprints don't count against quota.
...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    const mmsBody = `${i18n.t(job.locale, templateKey, { styleName: jobStyleName, pickupLocation }, ev)}${suffix}${shareLinks}`;
    ...
    const richDelivery = templateKey !== "deliveryPrint" && adapter && adapter.name === "whatsapp" && settings.getContentSid("delivery", job.locale) && job.baseUrl;
    const deliveryVars = {
        1: richStyleName,
        2: `images/${job.filePrefix}_output_mms.jpg`,
...
async function _markSiblingTerminal(sib, failReason, now) {
    ...
    await renameWithRetry(src, path.join(FAILED_DIR, sib.filename));
}
