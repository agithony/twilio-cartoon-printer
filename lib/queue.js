...
const { atomicWriteFile, atomicWriteFileSync, cleanupTempDirectory } = require("./atomic-write");
        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    if (job.failureEffectsPending && job.usageReleasePending) continue;
                    if (dir === FAILED_DIR && !job.countUsageOnRebuild) continue;
...
async function settlePendingRelayTransition(filename) {
    const printingPath = path.join(PRINTING_DIR, filename);
    let job;
    try {
        job = await readJobAsync(printingPath);
    } catch (err) {
    ...
    try {
        await renameWithRetry(printingPath, terminalPath);
    } catch (err) {
        if (err.code !== "ENOENT") throw err;
    }
...
async function sweepPendingPrintingTransitions() {
    let printing = [];
    try { printing = (await fsp.readdir(PRINTING_DIR)).filter((f) => f.endsWith(".json")); } catch {}
...
async function sweepPendingRelayEffects() {
    await sweepPendingPrintingTransitions();
    return sweepPendingTerminalEffects();
}
...
function requeueDoneJobForReprint(filename, { targetPrinter = null } = {}) {
    ...
    try {
        ...
        fs.renameSync(srcPath, path.join(READY_DIR, safe));
