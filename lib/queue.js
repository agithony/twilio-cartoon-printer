        ...
        if (settings.get("enablePrinting")) {
            ...
            if (adminShortcut) {
                ...
                if (remaining > 0 || unlimited) {
                    ...
                    suffix = ` That was your last one -- thanks for visiting!`;
                }
            }

            const imageUrl = job.baseUrl ? `${job.baseUrl}/images/${job.filePrefix}_output_mms.jpg` : null;
            const mmsBody = `Here's your ${jobStyleName} portrait!${suffix}`;
            ...
            const promoText = settings.get("promoMessage");
...
async function processPrintQueue() {
    ...
    try {
        ...
        if (adminShortcut) {
            ...
            const unlimited = isAdmin(job.userPhone) && testing;
            if (remaining > 0 || unlimited) {
                suffix = ` You have ${remaining} print${remaining === 1 ? "" : "s"} remaining -- try a different style next time: ${otherStyles}.`;
            } else {
                suffix = ` That was your last one -- thanks for visiting!`;
            }
        }
        ...
        const imageUrl = job.baseUrl ? `${job.baseUrl}/images/${job.filePrefix}_output_mms.jpg` : null;
        const mmsBody = `Your ${jobStyleName} portrait has been sent to the printer! Head to the Twilio booth to pick it up.${suffix}`;
        const leadMode = settings.get("leadCaptureMode");
