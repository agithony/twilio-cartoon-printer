        ...
        if (reviewMode === "ai") {
            ...
            try {
                ...
                if (result.passed) {
                    ...
                    if (!job.noDelivery) {
                        messaging.send(job.userPhone, "_raw", {}, { _body: i18n.t(job.locale, "reviewFailed", {}, job.eventName) })
                ...
                if (jobRead.retries >= MAX_RETRIES) {
                    ...
                    if (isVariantSibling) {
                        ...
                        if (!jobRead.noDelivery) {
                            messaging.send(jobRead.userPhone, "_raw", {}, { _body: "Sorry, we couldn't generate your AI portrait this time. Please try sending your photo again!" })
...
async function sendDigitalDelivery(job, filename, templateKey, jobDir) {
    ...
    } else if (richDelivery) {
        const result = await messaging.send(job.userPhone, "delivery", deliveryVars, { adapter, locale: job.locale });
        if (result && result.error) {
            await messaging.send(job.userPhone, "_raw", {}, { _body: mmsBody, mediaUrl: skipMms ? null : imageUrl, adapter });
        }
    } else {
        await messaging.send(job.userPhone, "_raw", {}, jobSendOptions(job, { _body: mmsBody, mediaUrl: skipMms ? null : imageUrl }))
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => {
                ...
                const send = hasTemplate
                    ? messaging.send(job.userPhone, "promo", {}, { adapter, locale: job.locale })
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(() => {
                    ...
                    const send = hasRating
                        ? messaging.send(job.userPhone, "rating", {}, jobSendOptions(job, { locale: job.locale }))
                        : messaging.send(job.userPhone, "_raw", {}, jobSendOptions(job, { _body: i18n.t(job.locale, "npsPrompt", {}, ev) }));
                    send
...
async function rejectJob(filename, message, reanalyze, feedback) {
    ...
    if (reanalyze) {
        ...
        if (message && !job.noDelivery) {
            messaging.send(job.userPhone, "_raw", {}, { _body: message })
...
async function rejectParent(parentJobId, message, reanalyze, feedback) {
    ...
    if (message && !firstJob.noDelivery) {
        messaging.send(firstJob.userPhone, "_raw", {}, { _body: message })
            .catch((err) => console.error(`❌ Review SMS failed: ${err.message}`));
    }
