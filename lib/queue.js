...
async function processGeneration(filename, generatingPath) {
    try {
        ...
        if (reviewMode === "ai") {
            ...
            const reviewedJob = { ...job, filename };
            slack.postReviewRequest(reviewedJob).then(() => {
                if (reviewedJob.slackTs) {
                    const reviewPath = path.join(REVIEW_DIR, filename);
                    if (fs.existsSync(reviewPath)) {
                        try {
                            const stored = JSON.parse(fs.readFileSync(reviewPath, "utf8"));
                            stored.slackTs = reviewedJob.slackTs;
                            stored.slackChannel = reviewedJob.slackChannel;
                            fs.writeFileSync(reviewPath, JSON.stringify(stored));
                        } catch (e) { /* job may have been actioned already */ }
                    }
                }
            }).catch((e) => console.error(`[Slack] review post error: ${e.message}`));
            return;
        }
        ...
        if (settings.getForEvent("enablePrinting", job.eventName)) {
            ...
            try {
                ...
                slack.postPortraitFeed(job).catch((e) => console.error(`[Slack] feed error: ${e.message}`));
            } catch (smsErr) {
...
async function approveJob(filename) {
    ...
    slack.notifyJobActioned(job, "approve", "web-ui").catch((e) => console.error(`[Slack] notify error: ${e.message}`));
}
...
async function rejectJob(filename, message, reanalyze, feedback) {
    ...
    const rejectAction = reanalyze ? "reanalyze" : (message ? "reject_notify" : "reject");
    slack.notifyJobActioned(job, rejectAction, "web-ui").catch((e) => console.error(`[Slack] notify error: ${e.message}`));
}
...
async function pickVariant(parentJobId, variantId) {
    ...
    slack.notifyJobActioned({ ...pickedJob, slackTs: pickedJob.slackTs, slackChannel: pickedJob.slackChannel }, "pick_variant", "web-ui").catch((e) => console.error(`[Slack] notify error: ${e.message}`));
}
...
async function rejectParent(parentJobId, message, reanalyze, feedback) {
    ...
    if (reanalyze) {
        ...
        slack.notifyJobActioned({ ...firstJob, slackTs: firstJob.slackTs, slackChannel: firstJob.slackChannel }, "reanalyze", "web-ui").catch((e) => console.error(`[Slack] notify error: ${e.message}`));
        return;
    }
    ...
    slack.notifyJobActioned({ ...firstJob, slackTs: firstJob.slackTs, slackChannel: firstJob.slackChannel }, reanalyze ? "reanalyze" : "reject_parent", "web-ui").catch((e) => console.error(`[Slack] notify error: ${e.message}`));
}
