        ...
        if (reviewMode === "ai") {
            ...
            try {
                ...
                if (result.passed) {
                    ...
                    if (!job.noDelivery) {
                        messaging.send(job.userPhone, "_raw", {}, { _body: settings.getMsgForEvent("reviewFailed", job.eventName) })
        ...
        if (promoText && !adminShortcut) {
            setTimeout(() => {
                ...
                const send = hasTemplate
                    ? messaging.send(job.userPhone, "promo", { 1: promoText }, { adapter })
        ...
        if (settings.getForEvent("enableNps", ev) && !adminShortcut && !nps.hasCompleted(job.userPhone, ev)) {
            ...
            if (remaining <= 0 && !unlimited) {
                ...
                setTimeout(() => {
                    ...
                    messaging.send(job.userPhone, hasRating ? "rating" : "npsPrompt", {}, jobSendOptions(job))
