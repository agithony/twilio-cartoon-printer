...
function computeDurations(eventFilter) {
    const isAdmin = (phone) => settings.get("adminPhones").includes(phone);
...
router.get("/api/stats", async (req, res) => {
    ...
    const durations = computeDurations(eventFilter);
...
function eventParam() {
  return "?event=" + encodeURIComponent(selectedEvent);
}
...
async function fetchStats() {
  ...
  try {
    ...
    if (selectedEvent !== forEvent) return; // user changed event while request was in flight
    render(d);
  } catch (e) {
