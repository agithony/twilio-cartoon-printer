...
const { getPrinterBusyState, incrementUsage, getReviewQueue, approveJob, rejectJob, isAdmin, getAllUsers, resetUsage, toggleAdmin, pickVariant, regenerateVariant, rejectParent, requeueDoneJobForReprint } = require("./queue");
...
const { shapeActiveJob } = require("./relay-jobs");
const { loadOpenAIModelCatalog } = require("./openai-models");
const { getEventSummary } = require("./event-summary");

const router = express.Router();
router.use(express.json({ limit: "1mb" }));

const TEMPLATE_EXTENSIONS = new Set([".png", ".jpg", ".jpeg", ".gif"]);
...
const MAX_TEMPLATE_EDGE = 10_000;
const TEMPLATE_SETTING_KEYS = new Set([
    "templateFile",
    "templateFilesByOrientation",
    "templateCompatibilityOverrides",
    "templateDeletionTombstones",
]);
let _templateOperationCounter = 0;
let _templateOperationTail = Promise.resolve();

async function withTemplateOperationLock(operation) {
    let release;
    const previous = _templateOperationTail;
    _templateOperationTail = new Promise((resolve) => { release = resolve; });
    await previous;
    try {
...
function validExistingTemplateName(filename) {
    return typeof filename === "string"
        && filename.length > 0
        && filename === path.basename(filename)
        && !/[\\/\0]/.test(filename)
        && TEMPLATE_EXTENSIONS.has(path.extname(filename).toLowerCase());
}

function validManagedTemplateName(filename) {
    return typeof filename === "string"
        && filename.length > 0
        && filename === path.basename(filename)
        && !/[\\/\0]/.test(filename)
        && [...TEMPLATE_EXTENSIONS, ".svg"].includes(path.extname(filename).toLowerCase());
}

function sanitizedRenamedTemplateName(name, sourceFilename) {
    if (typeof name !== "string" || /[\\/\0]/.test(name)) return "";
    const requested = name.trim();
    if (!requested) return "";
    const requestedExt = path.extname(requested);
    const stem = (requestedExt ? requested.slice(0, -requestedExt.length) : requested)
        .replace(/[^a-zA-Z0-9._-]/g, "_")
        .replace(/^\.+/, "")
        .replace(/_+/g, "_");
    return stem ? stem + path.extname(sourceFilename).toLowerCase() : "";
}

function sanitizeTemplateSettingsChanges(changes) {
    settings.refreshTemplateStateFromDisk();
    const sanitized = settings.sanitizeTemplateReferenceChanges(changes);
    // Frame-library globals are changed only by their dedicated locked APIs.
    delete sanitized.templateCompatibilityOverrides;
    delete sanitized.templateDeletionTombstones;
    return sanitized;
}

async function validateTemplateFile(filePath, filename) {
    const image = sharp(filePath, {
        animated: true,
        failOn: "error",
        limitInputPixels: MAX_TEMPLATE_PIXELS,
    });
    const metadata = await image.metadata();
    if (!metadata.width || !metadata.height || metadata.format !== TEMPLATE_FORMATS[path.extname(filename).toLowerCase()]) {
        throw new Error("Template data does not match its image extension");
    }
    if ((metadata.pages || 1) !== 1) {
        throw new Error("Animated or multipage templates are not supported");
    }
    if (metadata.width > MAX_TEMPLATE_EDGE || metadata.height > MAX_TEMPLATE_EDGE) {
        throw new Error("Template dimensions exceed the maximum edge length");
    }
    if (metadata.width * metadata.height > MAX_TEMPLATE_PIXELS) {
        throw new Error("Template dimensions exceed the maximum pixel count");
    }
    await image.toBuffer();
}

async function withTemplateFiles(payload) {
    try {
        payload.files = await settings.listTemplates();
    } catch (err) {
        payload.filesUnavailable = true;
        console.error(`⚠️  Template list refresh failed: ${err.message}`);
    }
    return payload;
}
...
// Cold-cache-aware variants. The sync readJobs()/countFiles() above return
// []/0 when NOTHING is cached yet (fresh process, or before the module-load
// pre-warm resolves) — which made the Jobs table render empty and the queue
// counts read 0 for the first ~second after a restart. These async variants
// await a single real read ONLY on a cold cache; once warm (even if stale)
// they fall straight back to the non-blocking stale-while-revalidate path, so
// we never reintroduce the event-loop stall the sync version once caused on
// Azure Files. Awaiting here is async I/O — it yields, it does not block.
async function _readDirJobsNow(dir) {
    const all = await fsp.readdir(dir);
    const files = all.filter((f) => f.endsWith(".json"));
    _dirCache.set(dir, { files, ts: Date.now() });
    const jobs = [];
    for (const f of files) {
        try { jobs.push(JSON.parse(await fsp.readFile(path.join(dir, f), "utf-8"))); } catch {}
    }
    _jobsCache.set(dir, { jobs, ts: Date.now() });
    return jobs;
}

async function readJobsAsync(dir) {
    const now = Date.now();
    const cached = _jobsCache.get(dir);
    if (cached && (now - cached.ts) < DIR_CACHE_TTL) return cached.jobs;
    if (cached) { _refreshJobs(dir); return cached.jobs; } // stale: serve + refresh, never block
    try { return await _readDirJobsNow(dir); } catch { return []; } // cold: one awaited read
}
...
async function countFilesAsync(dir) {
    ...
    const cached = _dirCache.get(dir);
    if (cached && (now - cached.ts) < DIR_CACHE_TTL) return cached.files.length;
    if (cached) { _refreshReaddir(dir); return cached.files.length; } // stale: serve + refresh
    try { return (await fsp.readdir(dir)).filter((f) => f.endsWith(".json")).length; } catch { return 0; }
}
...
function getStuckJobs(eventFilter) {
    ...
    // Review items are NOT reported as stuck. They show up in their own
    // Review Queue panel with timestamps, and "stuck in review" just means
    // "no reviewer has clicked yet" — that's a staffing fact, not a system
    // fault worth raising a red banner over. Reserve the STUCK alert for
    // cases that need an admin click (hung generation, offline printer).
    const checks = [
...
router.get("/api/stats", async (req, res) => {
    try {
        ...
        // Admin phone numbers are ALWAYS excluded from dashboard stats now —
        // the old xa=1 toggle let ops forget to enable it and then misread
        // metrics inflated by their own test traffic. Keeping the variable
        // so the downstream compute helpers still receive a boolean.
        const excludeAdmin = true;
        const showPii = req.query.pii === "1";
        ...
        const cacheKey = `${eventFilter}:${showPii}`;
        const now = Date.now();
        if (_statsCache.key === cacheKey && (now - _statsCache.ts) < STATS_CACHE_TTL) {
            return res.json(_statsCache.data);
        }

        // Fire Dub analytics early (non-blocking) — runs in parallel with
        // computeStats. We race it against a short deadline and drop the
        // result on the floor if dub.co is slow or down; the dashboard
        // still renders without those numbers instead of hanging on dub.
        const DUB_DASHBOARD_DEADLINE_MS = 1500;
        const dubAnalyticsP = Promise.race([
            dub.getAnalytics(eventFilter).catch(() => null),
            new Promise((resolve) => setTimeout(() => resolve(null), DUB_DASHBOARD_DEADLINE_MS)),
        ]);

        const stats = await computeStats(eventFilter, excludeAdmin);
        const queue = {
            pending: await countFilesAsync(PENDING_DIR),
            generating: await countFilesAsync(GENERATING_DIR),
            review: (await getReviewQueue(eventFilter)).length,
            ready: await countFilesAsync(READY_DIR),
            printing: await countFilesAsync(PRINTING_DIR),
        };
        ...
        // Which relay printers are actively printing (have a job in
        // PRINTING_DIR). Cached 10s because it only drives a cosmetic
        // status badge on the printer chip. Uses the shared stale-while-
        // revalidate readJobs() helper so we never block the event loop
        // on Azure Files readdir/readFile under load — a SIGNAL2026-scale
        // review queue made the old sync version stall the dashboard.
        const relayBusy = new Set();
        const printingJobs = readJobs(PRINTING_DIR);
        for (const j of printingJobs) {
            if (j && j.printerName) relayBusy.add(j.printerName);
        }
...
router.get("/api/failed-jobs", async (req, res) => {
    const eventFilter = req.query.e || "all";
    const candidates = (await readJobsAsync(FAILED_DIR)).filter((j) => {
        if (eventFilter !== "all" && j.eventName !== eventFilter) return false;
        // Hide "superseded" (losing sibling of a multi-variant pick) and
        // "reanalyzed" (replaced by a new parent job). Both live in FAILED_DIR
        // for audit trail but are not user-facing failures.
        return j.failReason !== "superseded" && j.failReason !== "reanalyzed";
    });
    // Parallel file-existence checks for thumbnails. Each failed job MAY have
    // an _input.jpg on disk (moderation/face/multi-subject fails keep it;
    // download fails don't). Previously the browser discovered missing files
    // via a 404 per row, which flooded the network tab and slowed paint.
    const jobs = await Promise.all(candidates.map(async (j) => {
        const filename = `${j.filePrefix}.json`;
        const evName = j.eventName || eventFilter;
        let hasInputThumb = false;
        let canReprint = false;
        if (j.filePrefix && evName) {
            const inputPath = path.join(settings.getDownloadDir(evName), `${j.filePrefix}_input.jpg`);
            const { outputPath } = jobPaths(j);
            await Promise.all([
                fsp.access(inputPath).then(() => { hasInputThumb = true; }).catch(() => {}),
                // A failed job whose print-resolution image still exists can be
                // sent straight back to the printer (no re-generation). Drives
                // the Reprint button on failed rows. Moderation fails are never
                // reprintable — the image must not be re-surfaced.
                fsp.access(outputPath).then(() => { canReprint = true; }).catch(() => {}),
            ]);
        }
        return {
            filename,
            ...
            canReprint: canReprint && j.failReason !== "moderation",
...
router.post("/api/retry-job", (req, res) => {
    ...
    try {
        ...
        const existingOutput = jobPaths(job).outputPath;
        if (fs.existsSync(existingOutput) && settings.getForEvent("enablePrinting", job.eventName)) {
            const result = requeueDoneJobForReprint(filename, { targetPrinter });
            if (!result.ok) return res.status(result.status).json({ error: result.error });
            return res.json({ ok: true, target: "print queue" });
        }
...
router.get("/api/done-jobs", async (req, res) => {
    const eventFilter = req.query.e || "all";
    const candidates = (await readJobsAsync(DONE_DIR)).filter((j) => {
        if (eventFilter !== "all" && j.eventName !== eventFilter) return false;
        return true;
    });
    // One output check supplies both reprint availability and the final-image thumbnail.
    const jobs = await Promise.all(candidates.map(async (j) => {
        const filename = `${j.filePrefix}.json`;
        const evName = j.eventName || eventFilter;
        const { outputPath } = jobPaths(j);
        let canReprint = false;
        if (j.filePrefix && evName) {
            try { await fsp.access(outputPath); canReprint = true; } catch {}
        }
        const hasOutputThumb = canReprint;
        return {
            filename,
            filePrefix: j.filePrefix,
            eventName: evName,
...
// ── Active (in-flight) Jobs API ───────────────────────────────────────────────
// Jobs sitting in READY_DIR (generated, waiting for a relay/printer to claim
// them) and PRINTING_DIR (actively printing) used to surface ONLY as the
// numeric "Ready to Print" / "Printing" counters in the queue strip — never as
// rows in the Jobs table. So when the relay stopped printing (e.g. the venue
// network dropped), portraits piled up in ready/ and silently never appeared
// as job rows: the table only ever read done/ and failed/. This endpoint
// exposes those in-flight jobs so they render as live rows.
router.get("/api/active-jobs", async (req, res) => {
    const eventFilter = req.query.e || "all";
    const out = [];
    for (const [dir, stage] of [[READY_DIR, "ready"], [PRINTING_DIR, "printing"]]) {
        for (const job of await readJobsAsync(dir)) {
            if (!job || !job.filePrefix) continue;
            if (eventFilter !== "all" && job.eventName !== eventFilter) continue;
            out.push(shapeActiveJob(job, stage, maskPhone));
        }
    }
    // Most-recently-entered first, matching the done/failed tabs' ordering.
    out.sort((a, b) => (b.enteredAt || 0) - (a.enteredAt || 0));
    res.json(out);
});

// POST /api/release-ready-job — un-strand a job stuck in READY_DIR.
// A ready job can be invisible to the booth's relay because it carries a
// targetPrinter that's offline, or a failedPrinters list containing the only
// printer (the single-printer failover trap). This clears both so the next
// poll from ANY printer picks it up. The job stays in ready/ (not moved), so
// there's no double-print risk — it was never claimed/printed. No-op safe:
// returns ok even if there was nothing to clear.
router.post("/api/release-ready-job", (req, res) => {
    const filename = path.basename((req.body || {}).filename || "");
    if (!filename || !filename.endsWith(".json")) return res.status(400).json({ error: "filename required" });
    const readyPath = path.join(READY_DIR, filename);
    if (!fs.existsSync(readyPath)) return res.status(404).json({ error: "Job not in the ready queue (it may have started printing or completed)" });
    try {
        ...
        job.retries = 0;
        job.stateChangedAt = Date.now();
        const tmpPath = readyPath + `.tmp.${process.pid}.${Date.now()}`;
        fs.writeFileSync(tmpPath, JSON.stringify(job, null, 2));
        fs.renameSync(tmpPath, readyPath);
        console.log(`🔧 Released ready job for any printer: ${filename}`);
        res.json({ ok: true });
    } catch (err) {
        if (err.code === "ENOENT") return res.status(409).json({ error: "Job was claimed by a printer just now" });
        console.error(`Failed to release ready job ${filename}:`, err);
        res.status(500).json({ error: err.message });
    }
});
...
router.post("/api/reprint-job", (req, res) => {
    ...
    // Shared with the relay's reprint endpoint so both surfaces re-queue
    // identically. The helper returns an HTTP-style status we pass through.
    const result = requeueDoneJobForReprint(filename, { targetPrinter });
    if (!result.ok) return res.status(result.status).json({ error: result.error });
    res.json({ ok: true });
});
...
router.get("/api/style-preview", (req, res) => {
    ...
    for (let i = doneJobs.length - 1; i >= 0; i--) {
        ...
        const outputPath = path.join(settings.getDownloadDir(j.eventName), `${j.filePrefix}_output.png`);
        if (!fs.existsSync(outputPath)) continue;
        if (!match || (j.createdAt || 0) > (match.createdAt || 0)) match = j;
    }
    if (!match) return res.json({ image: null });
    res.json({ image: `/photogallery/img/${encodeURIComponent(match.eventName)}/${encodeURIComponent(match.filePrefix)}_output.png` });
});
...
router.get("/api/openai-models", async (_req, res) => {
...
router.post("/api/settings", async (req, res) => {
    const saveChanges = (changes) => {
        ...
        const result = settings.updateForUser(req.user.email, changes);

        if (changes.backgroundChoices) {
            const savedBg = settings.get("backgroundChoices") || [];
            console.log(`⚙️  Backgrounds saved: ${savedBg.length} choices (received ${changes.backgroundChoices.length})`);
        }
    ...
    try {
        ...
        const hasTemplateSettings = Object.keys(changes).some((key) => TEMPLATE_SETTING_KEYS.has(key));
        const result = hasTemplateSettings
            ? await withTemplateOperationLock(() => saveChanges(sanitizeTemplateSettingsChanges(changes)))
            : saveChanges(changes);

        // Turning review off only affects new items. Existing review items
        // remain queued until a human explicitly approves or rejects them.
        res.json(result);
    } catch (err) {
...
router.get("/api/settings/files", async (req, res) => {
    try {
        ...
        console.error(`❌ Settings file discovery failed: ${err.message}`);
        res.status(503).json({ error: "Template files are temporarily unavailable" });
    }
});

router.post("/api/settings/template/compatibility", async (req, res) => {
    const { filename, compatibility } = req.body || {};
    if (!validExistingTemplateName(filename)) {
        return res.status(400).json({ error: "Invalid template filename" });
    }
    if (!["native", "portrait", "landscape", "both"].includes(compatibility)) {
        return res.status(400).json({ error: "Invalid template compatibility" });
    }

    try {
        await withTemplateOperationLock(async () => {
            settings.refreshTemplateStateFromDisk();
            if (!settings.isTemplateUsable(filename)) {
                return res.status(404).json({ error: "Template not found" });
            }
            try {
                const stat = await fsp.stat(path.join(settings.TEMPLATES_DIR, filename));
                if (!stat.isFile()) return res.status(404).json({ error: "Template not found" });
            } catch (err) {
                if (err.code === "ENOENT") return res.status(404).json({ error: "Template not found" });
                throw err;
            }
            settings.setTemplateCompatibility(filename, compatibility);
            res.json({ success: true, filename, compatibility });
        });
    } catch (err) {
        console.error(`❌ Template compatibility update failed: ${err.message}`);
        res.status(500).json({ error: "Template compatibility update failed" });
    }
});

router.post("/api/settings/template/rename", async (req, res) => {
    const { filename, newName } = req.body || {};
    if (!validExistingTemplateName(filename)) {
        return res.status(400).json({ error: "Invalid template filename" });
    }
    const safeName = sanitizedRenamedTemplateName(newName, filename);
    if (!safeName) return res.status(400).json({ error: "Invalid new template name" });
    ...
    await withTemplateOperationLock(async () => {
        ...
        const sourcePath = path.join(settings.TEMPLATES_DIR, filename);
        const targetPath = path.join(settings.TEMPLATES_DIR, safeName);
        try {
            const sourceStat = await fsp.stat(sourcePath);
            if (!sourceStat.isFile()) return res.status(404).json({ error: "Template not found" });
        } catch (err) {
            if (err.code === "ENOENT") return res.status(404).json({ error: "Template not found" });
            return res.status(500).json({ error: "Could not read template" });
        }
        if (safeName === filename) {
            return res.json(await withTemplateFiles({ success: true, filename: safeName }));
        }
        try {
            await fsp.access(targetPath);
            return res.status(409).json({ error: "A template with that name already exists" });
        } catch (err) {
            if (err.code !== "ENOENT") return res.status(500).json({ error: "Could not check template name" });
        }
        ...
        try {
            ...
            settings.migrateTemplateReferences(filename, safeName);
            migrated = true;
            try {
                await fsp.unlink(sourcePath);
            } catch (err) {
                if (err.code !== "ENOENT") {
                    console.error(`⚠️  Renamed template cleanup failed; tombstone will hide the old file: ${err.message}`);
                }
            }
            res.json(await withTemplateFiles({ success: true, filename: safeName }));
        } catch (err) {
            if (copied && !migrated) {
                try { await fsp.unlink(targetPath); } catch (cleanupErr) {
                    console.error(`⚠️  Failed to roll back template copy: ${cleanupErr.message}`);
                }
            }
            console.error(`❌ Template rename failed: ${err.message}`);
            res.status(500).json({ error: "Template rename failed" });
        }
    });
});

router.delete("/api/settings/template", async (req, res) => {
    ...
    await withTemplateOperationLock(async () => {
        ...
        const filePath = path.join(settings.TEMPLATES_DIR, filename);
        try {
            const stat = await fsp.stat(filePath);
            if (!stat.isFile()) return res.status(404).json({ error: "Template not found" });
            settings.migrateTemplateReferences(filename, "");
            try { await fsp.unlink(filePath); } catch (err) {
                if (err.code !== "ENOENT") {
                    console.error(`⚠️  Deleted template cleanup failed; tombstone will hide the file: ${err.message}`);
                }
            }
            res.json(await withTemplateFiles({ success: true }));
        } catch (err) {
            if (err.code === "ENOENT") return res.status(404).json({ error: "Template not found" });
...
// ── Reset event data ─────────────────────────────────────────────────────────
// Wipes all generated data for a single event: queue files (except actively
// processing), images, leads, contacts, and kiosk submissions. Does NOT
// touch audit-log.jsonl (audit trail preserved by design) and does NOT
// reset settings — those are separate concerns.
router.post("/api/events/:eventName/reset-data", async (req, res) => {
    const eventName = req.params.eventName;
    // Reject any separator/traversal/null-byte tricks, plus anything that
    // doesn't look like a normal event name we'd accept on creation. This
    // is a destructive endpoint — stay strict.
    if (!eventName || !/^[A-Za-z0-9_\- ]{1,64}$/.test(eventName)) {
        return res.status(400).json({ error: "Invalid eventName" });
    }

    const summary = { eventName, queueJobs: 0, images: 0, leads: 0, contacts: 0, kioskSubmissions: 0 };

    // Queue files — skip GENERATING_DIR and PRINTING_DIR so we don't yank
    // a file a worker is actively holding a reference to.
    ...
    for (const dir of queueDirs) {
        try {
            ...
            for (const f of files) {
                ...
                try {
                    const job = JSON.parse(await fsp.readFile(full, "utf8"));
    ...
    // Download dir — images (final + .staging). Remove only files, not the
    // directory itself, so the pipeline can keep writing into it afterwards.
    const dlRoot = path.join(__dirname, "..", "downloads");
    for (const evDir of [path.join(dlRoot, eventName), path.join(dlRoot, eventName, ".staging")]) {
        try {
            const files = await fsp.readdir(evDir);
            for (const f of files) {
                const full = path.join(evDir, f);
                try {
    ...
    summary.leads = leads.deleteByEvent(eventName);
    summary.contacts = contacts.deleteByEvent(eventName);
    summary.kioskSubmissions = kioskSubmissions.deleteByEvent(eventName);

    // Invalidate caches so the dashboard immediately reflects the wipe
    // instead of serving stale counts until STATS_CACHE_TTL expires.
    // _dirCache feeds the count badges; _reviewCache feeds the review queue.
    // Miss either and post-reset UI shows stale numbers where the admin is
    // specifically looking to confirm the wipe.
    _statsCache.ts = 0;
    _jobsCache.clear();
    _dirCache.clear();
    try { require("./photogallery").invalidatePhotoGalleryCache(eventName, { styles: true }); } catch {}
    // Rebuild the usage cache since we deleted jobs it counted. Await so
    // the response only returns once the cache is ready — otherwise the
    // dashboard's next poll can still hit a half-built cache.
    try {
        const { buildUsageCache, invalidateReviewCache } = require("./queue");
        invalidateReviewCache();
        await buildUsageCache();
    } catch (err) {
        console.error(`⚠️  Usage cache rebuild after reset failed: ${err.message}`);
    }

    console.log(`🗑️  Reset event data for "${eventName}":`, summary);
...
router.get("/api/audit", async (req, res) => {
    try {
        ...
        // The "Recent Settings Changes" panel passes type=settings to hide
        // review/variant events. Left unset = show everything (for callers
        // that want the full activity log).
        const filter = req.query.type || null;
        const result = await audit.getAuditLog(limit, offset, filter);
        res.json(result);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// File upload for templates and videos (streams to disk without buffering the body)
router.post("/api/settings/upload", (req, res) => {
    const filename = req.query.filename;
    const type = req.query.type;
    const replaceFilename = req.query.replaceFilename;
    ...
    if (replaceFilename && (type !== "template" || req.query.replace !== "1" || !validManagedTemplateName(replaceFilename))) {
        return res.status(400).json({ error: "Invalid replacement template filename" });
    }
    ...
    const maxBytes = type === "template" ? MAX_TEMPLATE_UPLOAD_BYTES : MAX_UPLOAD_BYTES;
    ...
    const targetDir = type === "template"
        ...
        : path.join(__dirname, "..", "booth-uploads"); // video + booth-image live here so they persist across container restarts (assets/ is in-image only)

    if (!fs.existsSync(targetDir)) fs.mkdirSync(targetDir, { recursive: true });

    const targetPath = path.join(targetDir, safeName);
    if (type === "template" && req.query.replace !== "1" && fs.existsSync(targetPath)) {
        return res.status(409).json({ error: "A template with that name already exists" });
    }
    const uploadPath = path.join(targetDir, `.${safeName}.upload-${process.pid}-${Date.now()}-${_templateOperationCounter++}`);
    ...
    req.on("data", (chunk) => {
        ...
        size += chunk.length;
        if (size > maxBytes) {
            fail(413, `File too large (max ${maxBytes / 1024 / 1024}MB)`);
            return;
        }
    ...
    req.on("end", () => {
        ...
        writeStream.end(async () => {
            ...
            try {
                if (type === "template") {
                    try {
                        await validateTemplateFile(uploadPath, safeName);
                    } catch {
                        finished = true;
                        try { await fsp.unlink(uploadPath); } catch {}
                        return res.status(400).json({ error: "Invalid template image data" });
                    }
                }
                if (type === "template") {
                    await withTemplateOperationLock(async () => {
                        settings.refreshTemplateStateFromDisk();
                        const replacing = req.query.replace === "1";
                        const replacedFilename = replaceFilename || safeName;
                        const replacedPath = path.join(targetDir, replacedFilename);
                        let targetExists = false;
                        try {
                            targetExists = (await fsp.stat(targetPath)).isFile();
                        } catch (err) {
                            if (err.code !== "ENOENT") throw err;
                        }
                        let replacedFileExists = targetExists;
                        if (replacedFilename !== safeName) {
                            try {
                                replacedFileExists = (await fsp.stat(replacedPath)).isFile();
                            } catch (err) {
                                if (err.code !== "ENOENT") throw err;
                                replacedFileExists = false;
                            }
                        }
                        if (!replacing && targetExists) {
                            const err = new Error("A template with that name already exists");
                            err.status = 409;
                            throw err;
                        }
                        if (replacing && !replacedFileExists) {
                            const err = new Error("Template not found");
                            err.status = 404;
                            throw err;
                        }
                        if (replacing && replacedFilename !== safeName && targetExists) {
                            const err = new Error("A template with the replacement name already exists");
                            err.status = 409;
                            throw err;
                        }

                        const backupPath = path.join(targetDir, `.${safeName}.backup-${process.pid}-${Date.now()}-${_templateOperationCounter++}`);
                        let installed = false;
                        try {
                            if (targetExists) await fsp.copyFile(targetPath, backupPath, fs.constants.COPYFILE_EXCL);
                            await fsp.rename(uploadPath, targetPath);
                            installed = true;
                            if (replacedFilename !== safeName) {
                                settings.migrateTemplateReferences(replacedFilename, safeName);
                                try { await fsp.unlink(replacedPath); } catch (err) {
                                    if (err.code !== "ENOENT") {
                                        console.error(`⚠️  Replaced template cleanup failed; tombstone will hide the old file: ${err.message}`);
                                    }
                                }
                            } else {
                                settings.restoreTemplate(safeName);
                            }
                        } catch (err) {
                            if (installed) {
                                try {
                                    if (targetExists) await fsp.rename(backupPath, targetPath);
                                    else await fsp.unlink(targetPath);
                                } catch (rollbackErr) {
                                    err.message += ` (template rollback failed: ${rollbackErr.message})`;
                                }
                            }
                            throw err;
                        } finally {
                            try { await fsp.unlink(backupPath); } catch (err) {
                                if (err.code !== "ENOENT") console.error(`⚠️  Template backup cleanup failed: ${err.message}`);
                            }
                        }
                    });
                } else {
                ...
                if (type === "template") {
                    ...
                    const files = type === "brand-reference" ? settings.listBrandReferences() : type === "style-reference" ? settings.listStyleReferences() : type === "background-reference" ? settings.listBackgroundReferences() : type === "video" ? settings.listVideos() : [];
                ...
                finished = true;
                res.status(err.status || 500).json({ error: "Upload failed: " + err.message });
            }
        });
    });
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .audit-detail .audit-meta { color: var(--th-text-muted); font-size: 12px; }
  ...
  /* Grid columns: thumb, phone, name, style, status, printer, time, action.
     Fixed widths for alignment; name flexes. Each row shares this grid so
     data in one row is always visually under the same header cell, even
     when some cells are empty. */
  .jb-grid-cols {
    display: grid;
    grid-template-columns: 44px 110px minmax(80px, 1fr) 90px 110px 90px 60px auto;
    align-items: center; gap: 8px 10px;
  }
  .jb-header {
    padding: 6px 12px 8px; margin-bottom: 2px;
    font-size: 10px; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.6px; color: var(--th-text-muted);
    font-family: 'Twilio Sans Mono', monospace;
    border-bottom: 1px solid var(--th-card-border); position: sticky; top: 0;
    background: var(--th-card); z-index: 1;
  }
  .jb-header > span:last-child { text-align: right; }
  .jb-row {
    padding: 10px 12px; margin-bottom: 4px;
    background: var(--th-bg); border-radius: 8px; border: 1px solid var(--th-card-border); font-size: 13px;
  }
  .jb-row.has-detail { grid-template-rows: auto auto; }
  .jb-thumb-link, .jb-thumb-empty {
    flex-shrink: 0; width: 36px; height: 36px; border-radius: 4px; display: inline-block;
  }
  .jb-thumb {
  ...
  .jb-thumb:hover { transform: scale(1.08); }
  .jb-thumb-empty {
    background: var(--th-bg-subtle); border: 1px dashed var(--th-card-border);
  }
  .jb-phone { color: var(--th-text-secondary); font-family: 'Twilio Sans Mono', monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .jb-name { color: var(--th-text); font-weight: 600; font-size: 12px; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .jb-style { color: var(--th-text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .jb-printer { color: var(--th-text-muted); font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .jb-time { color: var(--th-text-muted); font-size: 11px; text-align: right; white-space: nowrap; }
  .jb-status {
  ...
  .jb-status.ready { background: rgba(107,117,133,.12); color: var(--th-text-secondary); }
  .jb-status.printing { background: rgba(24,102,238,.12); color: var(--blue-500); }
  .jb-bounce { font-size: 10px; font-weight: 700; color: #ce9325; background: rgba(206,147,37,.09); padding: 2px 6px; border-radius: 5px; white-space: nowrap; cursor: help; }
  .jb-btn {
  ...
  .jb-detail {
    grid-column: 1 / -1; font-size: 11px; color: var(--th-text-muted); line-height: 1.5;
    padding: 4px 0 0; border-top: 1px solid var(--th-card-border); margin-top: 2px;
  }
  .jb-action { text-align: right; display: flex; gap: 4px; justify-content: flex-end; align-items: center; }
  ...
  /* Review grid: fit 3 multi-variant cards per row. Each multi-card spans
     2 base columns (rv-card.rv-multi { grid-column: span 2 }), so we need
     6 base tracks of 1fr to get 3 cards across. auto-fit collapses unused
     tracks so the layout stays tight instead of leaving trailing white-
     space on the right. At narrower widths the grid falls back to 2 → 1
     cards per row automatically. */
  .review-grid {
  ...
  /* Placeholder for missing variants — keeps the grid shape consistent when
     some siblings failed and were cleaned up. Non-interactive; shows a
     dim "Unavailable" label where the image would be. */
  .rv-variant.is-empty .rv-v-imgwrap {
    background: var(--th-bg); border: 1px dashed var(--th-card-border);
    position: relative; aspect-ratio: 1 / 1; border-radius: 4px;
    display: flex; align-items: center; justify-content: center;
    cursor: default;
  }
  .rv-variant.is-empty .rv-v-regen-label {
    color: var(--th-text-muted); font-size: 11px; font-weight: 600;
    text-transform: uppercase; letter-spacing: 0.5px;
  }
  .rv-variant.is-empty .rv-v-actions { min-height: 0; }
  ...
  /* Modal action bar (prev / approve / next). Single container so the
     approve button always shows — whether the card is a singleton (no
     prev/next) or multi-variant. Inner buttons toggle display independently. */
  .rv-modal-actions {
  ...
  /* Shown state — toggled by JS (display:none inline → display:inline-flex). */
  .rv-modal-nav-btn.shown { display: inline-flex; }
  ...
  /* Approve button sits between prev/next, styled like the card-level primary
     approve so the visual is consistent. Wider pill, not a circle. */
  .rv-modal-approve {
  ...
  .rv-modal-approve:hover:not(:disabled) { background: var(--blue-500); box-shadow: 0 2px 10px rgba(33,136,239,.35); }
  .rv-modal-approve:active:not(:disabled) { transform: translateY(1px); }
  .rv-modal-approve:disabled { opacity: .5; cursor: not-allowed; }
...
<div class="header">
  ...
  <div class="header-controls">
    ...
    <label class="hdr-item" title="Reveal user names, emails, and companies in the Share Platform Funnel"><input type="checkbox" id="showPiiCb" onchange="onPiiToggle()">Show contact info</label>
...
<div id="reviewModal" class="rv-modal" onclick="closeReviewModal(event)">
  <div class="rv-modal-content">
    ...
    <div class="rv-modal-actions" id="reviewModalActions">
      <button class="rv-modal-nav-btn" id="rvModalPrev" onclick="cycleModalVariant(-1)" title="Previous variant" style="display:none">&lsaquo;</button>
      <button class="rv-modal-approve" id="rvModalApprove" onclick="approveFromModal()" title="Approve this image (A or Enter)">Approve</button>
      <button class="rv-modal-nav-btn" id="rvModalNext" onclick="cycleModalVariant(1)" title="Next variant" style="display:none">&rsaquo;</button>
    </div>
  </div>
</div>
  ...
  <div class="panel" style="margin-top:14px">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
      ...
      <div class="jb-tabs" id="jobTabs">
        ...
        <button class="jb-tab" data-filter="active" onclick="setJobFilter(&apos;active&apos;)">In&nbsp;Progress</button>
...
function render(d) {
  ...
  // Populate event dropdown (preserve selection). Only rebuild the <options>
  // when the events list actually changed — the poll fires every 3s and the
  // innerHTML swap was (a) wasteful and (b) caused a brief deselect flicker
  // mid-rebuild that could persist if the user was hovering the dropdown.
  const sel = document.getElementById("eventSelect");
  const prev = sel.value;
  const nextSig = "all|" + (d.events || []).join("|");
  if (sel.dataset.sig !== nextSig) {
    sel.innerHTML = '<option value="all">All Events</option>';
    if (d.events) {
      for (const e of d.events) {
        sel.innerHTML += '<option value="'+escHtml(e)+'">'+escHtml(e)+'</option>';
      }
    }
  ...
  firstLoad = false;
  // Restore previous selection if the user had picked something non-default.
  // (Without this, rebuilding the dropdown on an events-list change would
  // reset to "all".)
  if (prev && prev !== sel.value) sel.value = prev;
...
async function resetUserUsage(id) {
  ...
  var ok = await twModal.confirm({
    title: "Reset portrait count?",
    message: "Reset this user's portrait count to 0 for " + ev + "?",
    confirmText: "Reset",
    danger: true,
  });
  if (!ok) return;
...
async function resetUserSurvey(id) {
  if (selectedEvent === "all") { showToast("Select a specific event to reset survey"); return; }
  var ev = selectedEvent;
  var ok = await twModal.confirm({
    title: "Clear lead capture survey?",
    message: "Clear lead capture survey for this user on " + ev + "? They will be asked to fill it out again.",
    confirmText: "Clear",
    danger: true,
  });
  if (!ok) return;
...
async function fetchJobs() {
  ...
  // Fire all requests in parallel — previously these were sequential, which
  // multiplied the wait before jobs could render.
  var p1 = fetch("api/failed-jobs" + evParam).then(function(r){ return r.json(); }).then(function(d){ _failedJobs = d; }).catch(function(){});
  var p2 = fetch("api/done-jobs" + evParam).then(function(r){ return r.json(); }).then(function(d){ _doneJobs = d; }).catch(function(){});
  // In-flight jobs (ready/printing). Without this the Jobs table only ever
  // showed terminal states, so a portrait waiting on a stalled relay was
  // invisible as a row even though "Ready to Print" climbed.
  var p3 = fetch("api/active-jobs" + evParam).then(function(r){ return r.json(); }).then(function(d){ _activeJobs = d; }).catch(function(){});
...
function renderJobs() {
  ...
  var all = [];
  // In-flight jobs (ready/printing) first so an operator scanning the All tab
  // sees what's still moving through the pipeline, not just terminal rows.
  for (var j of _activeJobs) {
    all.push({ type: "active", ts: j.enteredAt || 0, data: j });
  }
  ...
  // Apply filter
  var filtered = all;
  if (_jobFilter === "active") filtered = all.filter(function(x) { return x.type === "active"; });
  else if (_jobFilter === "failed") filtered = all.filter(function(x) { return x.type === "failed"; });
  else if (_jobFilter === "done") filtered = all.filter(function(x) { return x.type === "done"; });
  filtered.sort(function(a, b) { return b.ts - a.ts; });

  // Update tab counts
  var activeCount = _activeJobs.length;
  ...
  for (var t of tabs) {
    ...
    else if (f === "active") t.textContent = "In Progress (" + activeCount + ")";
    else if (f === "failed") t.textContent = "Failed (" + failCount + ")";
    else if (f === "done") t.textContent = "Completed (" + doneCount + ")";
  }

  if (!filtered.length) {
    var msg = _jobFilter === "active" ? "No jobs in progress" : _jobFilter === "failed" ? "No failed jobs" : _jobFilter === "done" ? "No completed jobs" : "No jobs";
    el.innerHTML = '<div class="jb-empty">' + msg + '</div>';
    return;
  }
  var oldList = el.querySelector(".jb-list");
  var scrollTop = oldList ? oldList.scrollTop : 0;
  // Sticky column header sits inside .jb-list so it stays pinned to the top
  // as rows scroll beneath it. Grid template matches data rows exactly so
  // every header cell lines up with its column.
  var h = '<div class="jb-list">';
  h += '<div class="jb-header jb-grid-cols">'
    + '<span></span>'
    + '<span>Phone</span>'
    + '<span>Name</span>'
    + '<span>Style</span>'
...
function renderActiveRow(j) {
  // In-flight job (ready or printing). No Reprint/Retry action — it isn't
  // terminal yet — but we DO surface a hint when a ready job has already been
  // rejected by one or more printers, since that's the signature of a job
  // bouncing in the queue (e.g. its output image is missing, or every printer
  // is offline). The thumbnail uses the MMS-sized jpg like the Done rows.
  var statusLabel = j.status === "printing" ? "Printing…" : "Ready to print";
  var statusCls = j.status === "printing" ? "printing" : "ready";
  var h = '<div class="jb-row jb-grid-cols" id="jb-'+escHtml(j.filename)+'">';
  if (j.filePrefix && j.eventName) {
    var thumbUrl = "/photogallery/img/" + encodeURIComponent(j.eventName) + "/" + encodeURIComponent(j.filePrefix) + "_output.png";
    h += '<a class="jb-thumb-link" href="'+thumbUrl+'" target="_blank" rel="noopener" title="Portrait"><img class="jb-thumb" src="'+thumbUrl+'" loading="lazy" alt="" onerror="this.style.display=&apos;none&apos;"></a>';
  } else {
    h += '<span class="jb-thumb-empty"></span>';
  }
  ...
  h += '<span class="jb-status '+statusCls+'">'+statusLabel+'</span>';
  h += '<span class="jb-printer">'+(j.printerName ? escHtml(j.printerName) : '')+'</span>';
  h += '<span class="jb-time">'+timeAgo(j.enteredAt)+'</span>';
  h += '<span class="jb-action">';
  if (j.status === "ready") {
    // A ready job that's been rejected by a printer (or targeted at one that's
    // offline) can sit unclaimed. "Release" clears the printer history/target
    // so any connected printer picks it up on its next poll — the operator
    // doesn't have to uncheck printers in the relay app. Shown with the bounce
    // hint when there's failure history so the cause is visible.
    if (j.failedPrinters && j.failedPrinters.length) {
      h += '<span class="jb-bounce" title="Rejected by: '+escHtml(j.failedPrinters.join(", "))+'">retry '+(j.retries||0)+'</span>';
    }
    h += '<button class="jb-btn" onclick="releaseReadyJob(&apos;'+escHtml(j.filename)+'&apos;)" title="Clear printer history so any connected printer prints this">Release</button>';
  }
  h += '</span>';
  h += '</div>';
  return h;
...
function renderFailedRow(j) {
  ...
  var rowCls = "jb-row jb-grid-cols" + (detail ? " has-detail" : "");
  var h = '<div class="'+rowCls+'" id="jb-'+escHtml(j.filename)+'">';
  if (j.hasInputThumb && j.filePrefix && j.eventName) {
    var thumbUrl = "/photogallery/img/" + encodeURIComponent(j.eventName) + "/" + encodeURIComponent(j.filePrefix) + "_input.jpg";
    h += '<a class="jb-thumb-link" href="'+thumbUrl+'" target="_blank" rel="noopener" title="Original selfie"><img class="jb-thumb" src="'+thumbUrl+'" loading="lazy" alt=""></a>';
  } else {
    h += '<span class="jb-thumb-empty"></span>';
  }
  h += '<span class="jb-phone">'+escHtml(j.phone)+'</span>';
  ...
  h += '<span class="jb-action">';
  if (j.canReprint) {
    // Output image still exists — offer a direct Reprint (re-queue straight to
    // the print queue, no re-generation). Broader than the old printer-fail-
    // only gate: any non-moderation failure whose image survived can reprint.
    h += printerDropdown("jb-ptr-"+escHtml(j.filename), "Any printer");
    h += '<button class="jb-btn" onclick="retryJob(&apos;'+escHtml(j.filename)+'&apos;,true)" title="Print the existing portrait again — no re-generation, user is not notified">Reprint</button>';
  } else if (j.canRetry) {
...
function renderDoneRow(j) {
  var h = '<div class="jb-row jb-grid-cols" id="jb-'+escHtml(j.filename)+'">';
  if (j.hasOutputThumb && j.filePrefix && j.eventName) {
    var thumbUrl = "/photogallery/img/" + encodeURIComponent(j.eventName) + "/" + encodeURIComponent(j.filePrefix) + "_output.png";
    h += '<a class="jb-thumb-link" href="'+thumbUrl+'" target="_blank" rel="noopener" title="Portrait"><img class="jb-thumb" src="'+thumbUrl+'" loading="lazy" alt=""></a>';
  } else {
    h += '<span class="jb-thumb-empty"></span>';
  }
  h += '<span class="jb-phone">'+escHtml(j.phone)+'</span>';
...
async function releaseReadyJob(filename) {
  try {
    var r = await fetch("api/release-ready-job", {
      method: "POST", headers: {"Content-Type":"application/json"},
      body: JSON.stringify({ filename })
    });
    var d = await r.json();
    if (d.ok) {
      showToast("Released — any connected printer will pick it up");
      fetchJobs();
      fetchStats();
    } else {
      showToast(d.error || "Release failed");
    }
...
// Per-card signature used to decide whether to re-render a card on poll.
// Two cards with the same sig render identically, so we skip the rebuild
// and preserve the card's images, event listeners, and open sub-panels
// (regen feedback textareas, etc).
function _rvCardSig(j) {
  if (j.parentJobId) {
    // Variant status (READY / REGENERATING / FAILED / SUPERSEDED) changes
    // as regens complete; regenCount goes up per variant. Both must be in
    // the signature so we catch those transitions.
    var vSig = (j.variants || []).map(function(v){
      return (v.variantId||v.filePrefix) + ":" + (v.status||"?") + ":" + (v.regenCount||0);
    }).join("|");
    return "M:" + j.parentJobId + "|" + vSig;
  }
  return "S:" + j.filename;
}
...
function renderReviewQueue(jobs) {
  ...
  // Skip re-render if the user has a feedback textarea open — replacing the
  // card would blow away their typed text. Same guard as before.
  if (Object.keys(_openFeedback).length > 0) { badge.textContent = jobs.length; return; }

  // Incremental update. Previously this rebuilt grid.innerHTML on every 3s
  // poll, which forced every card's images to re-decode and caused a visible
  // flicker on approve. Now we:
  //   1. Remove cards no longer in the queue (approved / rejected elsewhere)
  //   2. Update cards whose signature changed (status / regenCount)
  //   3. Append new cards at the end
  //   4. Reorder if needed (oldest first from server)
  var existingById = {};
  ...
  for (var k = 0; k < jobs.length; k++) {
    var j = jobs[k];
    var id = "rv-" + (j.parentJobId || j.filename);
    seenIds[id] = true;
    var sig = _rvCardSig(j);
    var existing = existingById[id];
    var needsRender = !existing || existing.dataset.sig !== sig;
    if (needsRender) {
      var html = j.parentJobId ? renderMultiReviewCard(j) : renderSingleReviewCard(j);
      var tmp = document.createElement("div");
      tmp.innerHTML = html; // eslint-disable-line no-unsanitized/property
      var newEl = tmp.firstElementChild;
      newEl.dataset.sig = sig;
      if (existing) {
        grid.replaceChild(newEl, existing);
      } else if (prevCardEl && prevCardEl.nextSibling) {
        grid.insertBefore(newEl, prevCardEl.nextSibling);
      } else {
        grid.appendChild(newEl);
      }
      prevCardEl = newEl;
    } else {
      // Order might have shifted (e.g., a card ahead got approved). Move
      // into position only if needed.
      if (prevCardEl ? existing.previousSibling !== prevCardEl : grid.firstChild !== existing) {
        if (prevCardEl) prevCardEl.after(existing);
        else grid.prepend(existing);
      }
      prevCardEl = existing;
    }
  }
  // Remove cards that are no longer in the queue (server-side gone).
  for (var gid in existingById) {
    if (!seenIds[gid]) existingById[gid].remove();
  }
...
function renderSingleReviewCard(j) {
  ...
  var eventPath = encodeURIComponent(j.eventName || "");
  var imgSrc = "/photogallery/staging/" + eventPath + "/" + fp + "_output.png";
  var origSrc = "/photogallery/staging/" + eventPath + "/" + fp + "_input.jpg";
  var fullSrc = imgSrc;
  ...
  h += '<div class="rv-img-wrap"><img src="'+origSrc+'" onclick="openSingleReviewModal(\\''+origSrc+'\\',\\''+fullSrc+'\\',\\''+fn+'\\')" title="Click to compare"><span class="rv-img-label">Original</span></div>';
  h += '<div class="rv-img-wrap"><img src="'+imgSrc+'" onclick="openSingleReviewModal(\\''+origSrc+'\\',\\''+fullSrc+'\\',\\''+fn+'\\')" title="Click to compare"><span class="rv-img-label">Generated</span></div>';
  h += '</div>';
...
function renderMultiReviewCard(j) {
  ...
  var eventPath = encodeURIComponent(j.eventName || "");
  // gridSlots = originally-requested variant count (from server). Always use
  // this for the grid-template-columns track count so thumbnails stay the
  // same size even when some siblings are missing (e.g., 2 of 3 failed and
  // got cleaned up). Missing slots render as empty placeholders rather
  // than stretching the surviving variants across the row.
  var gridSlots = j.variantCount || variants.length || 1;
  var firstPrefix = variants[0] ? escHtml(variants[0].filePrefix) : escHtml(j.filePrefix);
  var origSrc = "/photogallery/staging/" + eventPath + "/" + firstPrefix + "_input.jpg";
  ...
  _cardVariants[cardKey] = {
    ...
    variants: variants.map(function(v){
      return {
        ...
        status: v.status,
        mmsSrc: "/photogallery/staging/" + eventPath + "/" + v.filePrefix + "_output.png",
        fullSrc: "/photogallery/staging/" + eventPath + "/" + v.filePrefix + "_output.png",
      };
    }),
  };
  ...
  var h = "";
  h += '<div class="rv-card rv-multi'+(sel?" rv-selected":"")+'" id="rv-'+pid+'" style="--variant-count:'+gridSlots+'">';
  ...
  variants.forEach(function(v, idx){
    ...
    var stateCls = (failed ? " is-failed" : "") + (inFlight ? " is-regen" : "");
    h += '<div class="rv-variant'+stateCls+'" data-variant-idx="'+idx+'" data-variant-id="'+vid+'" id="rvv-'+pid+'-'+idx+'">';
    h += '<div class="rv-v-imgwrap"'+(inFlight ? '' : ' onclick="openMultiReviewModal(\\''+pid+'\\','+idx+')"')+'>';
    h += '<span class="rv-v-num">'+(idx+1)+'/'+gridSlots+'</span>';
    if (regenCount > 0) {
    ...
    if (inFlight) {
      ...
      h += '<img src="/photogallery/staging/'+eventPath+'/'+vfp+'_output.png" alt="Variant '+(idx+1)+'">';
    }
    h += '</div>';
  ...
  // Fill empty slots so the grid keeps its shape when some siblings are
  // missing. Placeholder is non-interactive with a "gone" label.
  for (var slotIdx = variants.length; slotIdx < gridSlots; slotIdx++) {
    h += '<div class="rv-variant is-empty" aria-hidden="true">';
    h += '<div class="rv-v-imgwrap"><span class="rv-v-num">'+(slotIdx+1)+'/'+gridSlots+'</span>';
    h += '<div class="rv-v-regen-label">Unavailable</div></div>';
    h += '<div class="rv-v-actions"></div>';
    h += '</div>';
  }
  h += '</div></div>';

  h += '<div class="rv-meta">'+escHtml(j.style||"unknown")+' &middot; '+timeAgo(j.reviewAt)+' &middot; '+variants.length+' of '+gridSlots+' variants</div>';
  ...
  h += '<div class="rv-feedback-hint">Leave blank to regenerate all '+gridSlots+' variants with fresh analysis.</div>';
  h += '<div class="rv-feedback-btns">';
...
async function bulkReviewAction(action) {
  ...
  var confirmed = await twModal.confirm({
    ...
    message: "This will " + label + " " + count + " item" + (count>1?"s":"") + ".",
...
async function reviewAction(filename, action, opts) {
  try {
    ...
    if (d.ok) {
      ...
      showToast(msg);
      // No full re-fetch — same reasoning as pickVariantDash. Reflect the
      // removal in the badge + bulk bar; the next 3s poll picks up any
      // concurrent changes (other admins, regen completions).
      var badge = document.getElementById("reviewCount");
      if (badge) {
        var n = document.querySelectorAll("#reviewGrid .rv-card").length;
        badge.textContent = n;
        if (n === 0) document.getElementById("reviewPanel").style.display = "none";
      }
      updateBulkBar(document.querySelectorAll("#reviewGrid .rv-card").length);
      refreshBulkApproveVisibility();
      fetchStats();
    } else {
...
// filename is the singleton job's .json filename — needed to wire up the
// in-modal Approve button. Optional for back-compat (old callers that didn't
// pass it still get a working compare-view modal, just without Approve).
function openSingleReviewModal(origSrc, genSrc, filename) {
  _modalContext = filename ? { kind: "single", filename: filename } : null;
  [origSrc, genSrc].forEach(function(s) { if (s) { var p = new Image(); p.src = s; } });
  var img = document.getElementById("reviewModalImg");
  img.src = "";
  ...
  // Hide prev/next for singletons. Show/enable the Approve button only if
  // the caller gave us a filename — old callers (compare-view on Done jobs,
  // say) get the modal with Approve hidden instead of a broken button.
  var prev = document.getElementById("rvModalPrev");
  var next = document.getElementById("rvModalNext");
  if (prev) { prev.classList.remove("shown"); prev.style.display = "none"; }
  if (next) { next.classList.remove("shown"); next.style.display = "none"; }
  var approve = document.getElementById("rvModalApprove");
  if (approve) { approve.style.display = filename ? "inline-flex" : "none"; approve.disabled = false; }
...
function openMultiReviewModal(pid, idx) {
  ...
  if (!ctx) return;
  _modalContext = { kind: "multi", parentJobId: pid, idx: Math.max(0, idx), variantCount: ctx.variants.length };
  // Preload every variant's modal image before showing the modal — we use
  // the MMS jpg (already resized for delivery) instead of the multi-MB
  // _output.png so paint is instant after cache warm-up.
  for (var i = 0; i < ctx.variants.length; i++) {
    var src = ctx.variants[i] && ctx.variants[i].mmsSrc;
...
function renderModalVariant() {
  if (!_modalContext || _modalContext.kind !== "multi") return;
  var ctx = _cardVariants[_modalContext.parentJobId];
  if (!ctx) return;
  var v = ctx.variants[_modalContext.idx];
  // Blank the src before assigning the new one to abort any in-flight load —
  // otherwise the previous variant's image can finish loading AFTER the new
  // one is requested and end up painted on top of it.
  var img = document.getElementById("reviewModalImg");
  img.src = "";
  ...
  var prev = document.getElementById("rvModalPrev");
  var next = document.getElementById("rvModalNext");
  // Show prev/next only when there are multiple variants to cycle through.
  var showNav = _modalContext.variantCount > 1;
  if (prev) {
    prev.style.display = showNav ? "inline-flex" : "none";
    if (showNav) prev.classList.add("shown"); else prev.classList.remove("shown");
    prev.disabled = _modalContext.idx === 0;
  }
  if (next) {
    next.style.display = showNav ? "inline-flex" : "none";
    if (showNav) next.classList.add("shown"); else next.classList.remove("shown");
    next.disabled = _modalContext.idx === _modalContext.variantCount - 1;
  }
  // Approve button: disabled if the current variant is still generating /
  // regenerating / failed / superseded — only READY variants can be picked.
  var approve = document.getElementById("rvModalApprove");
  if (approve) {
    var canApprove = v && (v.status === "READY" || v.status === undefined); // undefined = legacy shape
    approve.style.display = "inline-flex";
    approve.disabled = !canApprove;
    approve.title = canApprove
      ? "Approve this variant (A or Enter)"
      : "Variant not ready (" + (v && v.status || "unknown") + ")";
  }
}

function closeReviewModal(e) {
  // Only close when clicking the backdrop itself, not any child (nav buttons,
  // images, labels) — previously BUTTON clicks bubbled up and closed the
  // modal, breaking the Prev/Next arrow buttons.
...
function _modalEsc(e) {
  ...
  if (_modalContext && _modalContext.kind === "multi") {
    if (e.key === "ArrowLeft") { e.preventDefault(); return cycleModalVariant(-1); }
    if (e.key === "ArrowRight") { e.preventDefault(); return cycleModalVariant(1); }
  }
  // A / Enter approves the currently-displayed image (single or multi).
  // Lowercase-a ensures a bare letter press works regardless of caps lock.
  if (_modalContext && (e.key === "a" || e.key === "A" || e.key === "Enter")) {
    e.preventDefault();
    approveFromModal();
  }
}

// Approve whatever's currently on screen in the modal. Works for both
// single (_modalContext.filename set) and multi (parentJobId + idx). Closes
// the modal after the server confirms the approval succeeded so the admin
// lands back on the review grid with one fewer card.
async function approveFromModal() {
  var ctx = _modalContext;
  if (!ctx) return;
  var btn = document.getElementById("rvModalApprove");
  if (btn && btn.disabled) return;
  if (btn) btn.disabled = true;
  try {
    if (ctx.kind === "multi") {
      var vctx = _cardVariants[ctx.parentJobId];
      if (!vctx) return;
      var v = vctx.variants[ctx.idx];
      if (!v || (v.status && v.status !== "READY")) { showToast("Variant not ready"); return; }
      // Log what we're about to approve so the admin can verify in the
      // browser console that the right variant went through (paranoia check:
      // ctx.idx should always match what the label shows on screen).
      console.log("Approving variant " + (ctx.idx + 1) + "/" + ctx.variantCount + " (" + v.variantId + ") of parent " + ctx.parentJobId);
      closeReviewModal();
      // Delegate to the existing card-level handler — it already handles the
      // optimistic card.remove(), 409 races, toast, and badge refresh.
      await pickVariantDash(ctx.parentJobId, v.variantId);
    } else if (ctx.kind === "single") {
      console.log("Approving singleton " + ctx.filename);
      closeReviewModal();
      await reviewAction(ctx.filename, "approve");
    }
  } finally {
    // _modalContext is cleared by closeReviewModal; if the approve failed
    // mid-flight the card still exists and its buttons work normally.
    if (btn && document.getElementById("reviewModal").classList.contains("open")) {
      btn.disabled = false;
    }
  }
}
...
async function pickVariantDash(parentJobId, variantId) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Don't call fetchReviewQueue() here — that would trigger a full grid
      // rebuild which makes every OTHER card re-paint from scratch. The
      // approved card is already gone (card.remove()); the next 3s poll
      // picks up any other changes. Update the badge manually for instant
      // feedback. Bulk-bar visibility follows the card count.
      var badge = document.getElementById("reviewCount");
      if (badge) {
        var n = document.querySelectorAll("#reviewGrid .rv-card").length;
        badge.textContent = n;
      ...
      // Server says it's already been decided (another admin / AI picked it).
      // Remove our card silently and let the next poll reconcile.
      if (card) card.remove();
...
function renderAuditEntry(e) {
  ...
  } else if (e.action === "variant.pick") {
    // Reviewer picked a winning variant from a multi-variant job group
    var pickedIdx = e.pickedVariantIndex != null ? e.pickedVariantIndex : "?";
    var total = e.variantCount != null ? e.variantCount : "?";
    var styleSuffix = e.style ? ' <span class="audit-meta">(' + esc(e.style) + (e.brand ? " · " + esc(e.brand) : "") + ')</span>' : "";
    changes = '<span class="key">Variant picked</span>: chose #' + esc(String(pickedIdx)) + ' of ' + esc(String(total)) + styleSuffix;
  } else if (e.action === "variant.regenerate") {
    // Reviewer kicked off a regeneration of a single variant
    var vIdx = e.variantIndex != null ? e.variantIndex : "?";
    var regen = e.regenerationIndex != null ? e.regenerationIndex : "?";
    var limit = e.regenerationLimit != null ? e.regenerationLimit : "?";
    changes = '<span class="key">Variant regenerated</span>: #' + esc(String(vIdx)) + ' <span class="audit-meta">(attempt ' + esc(String(regen)) + '/' + esc(String(limit)) + ')</span>';
  } else if (e.action === "parent.reanalyze") {
    // Reviewer rejected all variants and requested a fresh generation with
    // different scene analysis (and optional feedback prompt)
    var fbSuffix = e.feedback ? ' <span class="audit-meta">feedback: "' + esc(e.feedback) + '"</span>' : "";
    changes = '<span class="key">Reanalyzed</span>: all variants rejected, requeued with fresh analysis' + fbSuffix;
  } else if (e.action === "parent.reject") {
    // Reviewer rejected the entire variant group (no regeneration)
    var notified = e.notified === true ? "user notified" : e.notified === false ? "silently rejected" : "notify status unknown";
    changes = '<span class="key">Rejected</span>: ' + esc(String(e.variantCount || "?")) + ' variants — ' + notified;
  } else if (e.action === "variant.ai_pick") {
    // AI automatically picked the best variant (no human reviewer)
    if (e.outcome === "all_failed") {
      changes = '<span class="key">AI pick</span>: all ' + esc(String(e.variantCount || "?")) + ' variants failed review' + (e.reason ? ' <span class="audit-meta">(' + esc(e.reason) + ')</span>' : "");
    } else {
      var aiIdx = e.pickedVariantIndex != null ? e.pickedVariantIndex : "?";
      changes = '<span class="key">AI pick</span>: chose #' + esc(String(aiIdx)) + ' of ' + esc(String(e.variantCount || "?"));
    }
  } else {
    // Unknown action type — show the raw name so operators can spot it
    changes = '<em>' + esc(e.action || "unknown action") + '</em>';
  }
  var revertHtml = "";
  ...
  // Non-settings events (variant.* / parent.*) are system-generated and have
  // no actor field — show "system" instead of an empty column.
  var actorText = e.actor ? e.actor.split("@")[0] : "system";
  return '<div class="audit-row">'
    + '<div class="audit-time">' + fmtAuditTime(e.ts) + '</div>'
    + '<div class="audit-actor">' + esc(actorText) + '</div>'
...
async function fetchAuditLog(append) {
  try {
    // type=settings filters out variant.pick/regenerate/etc — those belong
    // in a future review activity panel, not the settings activity log.
    var r = await fetch("api/audit?type=settings&limit=" + auditLimit + "&offset=" + (append ? auditOffset : 0));
    var d = await r.json();
...
async function revertAudit(id, btn) {
  var ok = await twModal.confirm({
    title: "Revert settings change?",
    message: "The previous values will be restored. This itself creates a new audit entry.",
    confirmText: "Revert",
    danger: true,
  });
  if (!ok) return;
  ...
  try {
    ...
    var d = await r.json();
    if (d.error) { await twModal.alert({ title: "Revert failed", message: d.error }); btn.disabled = false; btn.textContent = "Revert"; return; }
    btn.parentElement.innerHTML = '<span class="audit-reverted">Reverted</span>';
    fetchStats();
  } catch(e) {
    await twModal.alert({ title: "Revert failed", message: "Check the server logs and try again." });
    btn.disabled = false;
