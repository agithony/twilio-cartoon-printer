...
router.get("/api/review-count", (req, res) => {
    res.json({ count: countFiles(REVIEW_DIR) });
});
...
<div class="review-banner" id="reviewPanel" style="display:none">
  ...
  <div class="rv-bulk-bar" id="rvBulkBar">
    ...
    <div class="rv-bulk-actions">
      ...
      <button class="rv-btn rv-btn-reject" onclick="bulkReviewAction('reject')" title="Re-generate all selected images">Reject Selected</button>
    </div>
  </div>
...
function renderReviewQueue(jobs) {
  ...
  for (var j of jobs) {
    ...
    h += '<img src="'+imgSrc+'" onclick="openReviewModal(\\'/images/'+fp+'_output.png\\')" title="Click to enlarge">';
    h += '<div class="rv-meta">'+escHtml(j.style||"unknown")+' &middot; '+timeAgo(j.reviewAt)+'</div>';
    h += '<div class="rv-actions">';
    h += '<button class="rv-btn rv-btn-approve" onclick="reviewAction(\\''+fn+'\\',\\'approve\\')" title="Send this image to the user">Approve</button>';
    h += '<div class="rv-reject-row">';
    h += '<button class="rv-btn rv-btn-reject" onclick="reviewAction(\\''+fn+'\\',\\'reject\\')" title="Re-generate with the same prompt">Reject</button>';
    h += '<button class="rv-btn rv-btn-notify" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{notify:true})" title="Re-generate and send the user an SMS asking them to try a different photo">Reject + Notify</button>';
    h += '<button class="rv-btn rv-btn-reanalyze" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{reanalyze:true})" title="Re-generate with fresh scene analysis (use when subject count or pets are wrong)">Reject + Re-analyze</button>';
    h += '</div></div></div>';
  }
  grid.innerHTML = h;
...
async function reviewAction(filename, action, opts) {
  try {
    ...
    if (d.ok) {
      ...
      var card = document.getElementById("rv-"+filename);
      if (card) card.remove();
      var msg = action === "approve" ? "Image approved" : opts.reanalyze ? "Image rejected, re-analyzing scene" : opts.notify ? "Image rejected, user notified" : "Image rejected, re-generating";
      showToast(msg);
      fetchReviewQueue();
      fetchStats();
    } else {
