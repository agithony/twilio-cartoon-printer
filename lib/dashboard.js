...
router.get("/api/report", async (req, res) => {
    ...
    const bugRedPath = path.join(assetsDir, "twilio-bug-red.png");
    ...
    function renderFooter() {
        doc.save();
        doc.moveTo(mx, ph - 36).lineTo(pw - mx, ph - 36).strokeColor(gray100).lineWidth(0.5).stroke();
        doc.restore();
        try { doc.image(bugRedPath, mx, ph - 30, { height: 14 }); } catch (_e) {}
        doc.font("TwilioText").fontSize(7.5).fillColor(gray300)
            .text("AI Photo Generator", mx + 20, ph - 28, { width: contentW / 2 - 20, align: "left", lineBreak: false });
    }

    // ── Helper: ensure enough room for a section, else start a new page ──
    function ensureSpace(needed) {
        const safeBottom = ph - 60; // leave room for footer
        if (doc.y + needed > safeBottom) {
            renderFooter(); // stamp footer on current page before leaving
            _totalPages++;
            doc.addPage({ margins: { top: 48, bottom: 0, left: 0, right: 0 } });
            doc.y = 48;
            doc.x = mx;
        }
    }
    ...
    // ── Footer: stamp last page + add page numbers ──
    renderFooter(); // footer on the final page
    _totalPages++;

    // Now stamp page numbers on every page (minimal text-only pass)
    const pages = doc.bufferedPageRange();
    for (let i = 0; i < pages.count; i++) {
        doc.switchToPage(pages.start + i);
        doc.font("TwilioMono").fontSize(7.5).fillColor(gray300)
            .text("Page " + (i + 1) + " of " + _totalPages, mx + contentW / 2, ph - 28, { width: contentW / 2, align: "right", lineBreak: false });
    }
