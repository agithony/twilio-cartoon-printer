...
function renderReviewQueue(jobs) {
  ...
  for (var j of jobs) {
    ...
    h += '<button class="rv-btn rv-btn-approve" onclick="reviewAction(\\''+fn+'\\',\\'approve\\')">Approve</button>';
    h += '<div class="rv-reject-row">';
    h += '<button class="rv-btn rv-btn-reject" onclick="reviewAction(\\''+fn+'\\',\\'reject\\')">Reject</button>';
    h += '<button class="rv-btn rv-btn-notify" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{notify:true})" title="Reject and send SMS to user">Reject + Notify</button>';
    h += '<button class="rv-btn rv-btn-reanalyze" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{reanalyze:true})" title="Reject and re-run scene analysis">Reject + Re-analyze</button>';
    h += '</div></div></div>';
  }
  grid.innerHTML = h;
}
