...
router.post("/api/review-job", async (req, res) => {
    ...
    try {
        if (action === "approve") {
            ...
            await rejectJob(filename, message);
        }
        res.json({ ok: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
function renderReviewQueue(jobs) {
  ...
  for (var j of jobs) {
    ...
    h += '<button class="rv-btn rv-btn-notify" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',true)" title="Reject and send SMS to user">Reject + Notify</button>';
    h += '</div></div></div>';
  }
  grid.innerHTML = h;
}

async function reviewAction(filename, action, notify) {
  try {
    ...
    if (d.ok) {
      ...
      if (card) card.remove();
      var msg = action === "approve" ? "Image approved" : notify ? "Image rejected, user notified" : "Image rejected, re-generating";
      showToast(msg);
      fetchReviewQueue();
      fetchStats();
    } else {
