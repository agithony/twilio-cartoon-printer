...
router.post("/api/dashboard-users/toggle-admin", async (req, res) => {
    try {
        ...
        if (!targetPhone) return res.status(404).json({ error: "User not found" });
        const nowAdmin = await toggleAdmin(targetPhone);
        _statsCache.ts = 0;
        console.log(`👤 ${maskPhone(targetPhone)} ${nowAdmin ? "added to" : "removed from"} admins`);
...
function resetUserUsage(id) {
  ...
  fetch("api/dashboard-users/reset-usage", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ id: id, event: ev }) })
    ...
    .then(function(d) { if (d.ok) { showToast("Usage reset"); fetchDashboardUsers(); fetchStats(); } else { showToast("Error: " + (d.error || "unknown")); } })
    .catch(function() { showToast("Failed to reset usage"); });
}

function toggleUserAdmin(id) {
  fetch("api/dashboard-users/toggle-admin", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ id: id }) })
    .then(function(r) { return r.json(); })
