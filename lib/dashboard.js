...
router.get("/api/settings/files", async (req, res) => {
    ...
    res.json({ templates, videos, printers, events, brandReferences, styleReferences, eventProfiles });
});
...
router.post("/api/settings/upload", (req, res) => {
    ...
    if (!["template", "video", "brand-reference", "style-reference", "booth-image"].includes(type)) {
        return res.status(400).json({ error: "type must be 'template', 'video', 'brand-reference', 'style-reference', or 'booth-image'" });
    }
    ...
    const allowed = type === "template"
        ...
        : (type === "brand-reference" || type === "style-reference")
        ? [".png", ".jpg", ".jpeg", ".gif"]
        : type === "booth-image"
    ...
    req.on("end", () => {
        ...
        writeStream.end(() => {
            ...
            const files = type === "template" ? settings.listTemplates() : type === "brand-reference" ? settings.listBrandReferences() : type === "style-reference" ? settings.listStyleReferences() : type === "video" ? settings.listVideos() : [];
