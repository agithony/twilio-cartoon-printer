...
router.get("/api/report", async (req, res) => {
    ...
    for (let i = 0; i < pages.count; i++) {
        ...
        doc.restore();
        // Left: Twilio bug + branding text
        try {
            doc.image(bugRedPath, mx, ph - 30, { height: 14 });
        } catch (_e) { /* bug missing — skip */ }
        doc.font("TwilioText").fontSize(7.5).fillColor(gray300)
            .text("AI Photo Generator", mx + 20, ph - 28, { width: contentW / 2 - 20, align: "left" });
        // Right: page number
