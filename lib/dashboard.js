...
const { getPrinterBusyState } = require("./queue");
...
function computeStats(eventFilter, excludeAdmin) {
    ...
    // Failure breakdown
    const failureBreakdown = { moderation: 0, face_detection: 0, generation: 0, printer: 0, max_retries: 0, unknown: 0 };
...
router.get("/api/report", async (req, res) => {
    ...
    if (totalFails > 0) {
        ...
        const failColors = { moderation: "#E04444", face_detection: orange, generation: purple, printer: blue, max_retries: "#8B95A5", unknown: light };
...
router.get("/api/settings/files", async (req, res) => {
    ...
    res.json({ templates, videos, printers, events });
});
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hdr-item.hdr-action {
    background: #F22F46; border-color: #F22F46; color: #fff; font-weight: 600;
    letter-spacing: .2px;
  }
  .hdr-item.hdr-action:hover { background: #D42840; border-color: #D42840; }
  .hdr-item.hdr-action:disabled { opacity: .6; cursor: default; }
  ...
  .card {
    ...
    border-top: 3px solid #1E222B;
    border-radius: 14px;
    padding: clamp(18px, 1.8vw, 28px) clamp(14px, 1.2vw, 20px);
    text-align: center;
    position: relative;
    overflow: hidden;
  ...
  .card:hover { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(0,0,0,.3); border-color: #2A3040; }
  .card:nth-child(1) { border-top-color: #4B8BF5; }
  .card:nth-child(2) { border-top-color: #2EBA54; }
  .card:nth-child(3) { border-top-color: #9B6FE8; }
  .card:nth-child(4) { border-top-color: #E8853A; }
  .card:nth-child(5) { border-top-color: #E86B9E; }
  ...
  .card .value { font-size: clamp(24px, 2.4vw, 42px); font-weight: 700; color: #F7F8F8; font-variant-numeric: tabular-nums; position: relative; }
  .card .label { font-size: 12px; color: #636B78; margin-top: 6px; text-transform: uppercase; letter-spacing: .6px; font-weight: 500; position: relative; }
  ...
  .panel h2 {
    ...
    padding-left: 12px; border-left: 3px solid #4B8BF5;
    line-height: 1; padding-top: 1px; padding-bottom: 1px;
  }
  /* Panel accent colors by section */
  .sg-analytics .panel::before { background: linear-gradient(90deg, #4B8BF5, #4B8BF500); }
  .sg-analytics .panel h2 { border-left-color: #4B8BF5; }
  .sg-users .panel::before { background: linear-gradient(90deg, #2EBA54, #2EBA5400); }
  .sg-users .panel h2 { border-left-color: #2EBA54; }
  .sg-failures .panel::before { background: linear-gradient(90deg, #E8853A, #E8853A00); }
  .sg-failures .panel h2 { border-left-color: #E8853A; }
  .sg-operations .panel::before { background: linear-gradient(90deg, #9B6FE8, #9B6FE800); }
  .sg-operations .panel h2 { border-left-color: #9B6FE8; }
  ...
  .paper-sub { text-align: center; font-size: 13px; color: #636B78; margin: 6px 0 16px; }
  .progress-bar { height: 8px; background: #1E222B; border-radius: 4px; overflow: hidden; margin-bottom: 18px; }
  ...
  .paper-controls label { font-size: 13px; color: #636B78; display: flex; align-items: center; gap: 6px; }
  .paper-controls input {
    width: 56px; background: #0B0D11; border: 1px solid #1E222B; border-radius: 8px;
    color: #B0B8C4; padding: 6px 8px; font-size: 13px; text-align: center; font-family: inherit;
    transition: border-color .15s;
  }
  .paper-controls input:focus { outline: none; border-color: #9B6FE8; box-shadow: 0 0 0 3px rgba(155,111,232,.1); }
  .btn {
    background: #1E222B; color: #B0B8C4; border: 1px solid #2A3040; border-radius: 8px;
    padding: 7px 16px; font-size: 13px; font-weight: 500; font-family: inherit;
    cursor: pointer; transition: background .15s, border-color .15s, transform .1s;
  }
  .btn:hover { background: #2A3040; transform: translateY(-1px); }
  .btn:active { transform: translateY(0); }
  .btn-primary { background: #2EBA54; border-color: #2EBA54; color: #fff; font-weight: 600; }
  .btn-primary:hover { background: #26A348; }
  .btn-danger { background: #E04444; border-color: #E04444; color: #fff; font-weight: 600; }
  ...
  .paper-alert.warning, .stuck-alert.warning { display: block; background: #E8853A12; color: #E8853A; border: 1px solid #E8853A33; }
  ...
  .queue-row {
    ...
    background: #0B0D11; border: 1px solid #1E222B; border-radius: 8px;
    transition: border-color .15s;
  }
  .queue-row:hover { border-color: #252A34; }
  ...
  .queue-count { font-weight: 700; font-size: clamp(15px, 1.3rem, 24px); font-variant-numeric: tabular-nums; color: #F7F8F8; }

  /* Style bars */
  .style-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 4px 0; }
  .style-name { width: clamp(60px, 6vw, 100px); font-size: 13px; color: #8B95A5; text-align: right; flex-shrink: 0; font-weight: 500; }
  .style-bar-bg { flex: 1; height: clamp(16px, 1.4vw, 24px); background: #0B0D11; border-radius: 6px; overflow: hidden; border: 1px solid #1E222B; }
  .style-bar { height: 100%; border-radius: 5px; transition: width .5s ease; min-width: 2px; }
  .style-count { width: 34px; font-size: 13px; font-weight: 700; text-align: right; flex-shrink: 0; font-variant-numeric: tabular-nums; color: #F7F8F8; }
  ...
  .hourly-bar {
    flex: 1; background: linear-gradient(180deg, #4B8BF5, #3A6FCC); border-radius: 4px 4px 0 0; min-height: 2px;
    transition: height .3s ease, opacity .15s; cursor: default; position: relative;
  }
  .hourly-bar:hover { opacity: .8; }
  .hourly-bar:hover .hourly-tip { display: block; }
  .hourly-tip {
    display: none; position: absolute; bottom: calc(100% + 6px); left: 50%; transform: translateX(-50%);
    background: #252A34; color: #F7F8F8; font-size: 12px; font-weight: 600; padding: 4px 10px;
  ...
  .printer-status {
    ...
    background: #0B0D11; border-radius: 8px; border: 1px solid #1E222B;
  }
  ...
  .user-row:hover { background: #1A1E27; }
  .user-phone { color: #8B95A5; font-family: "SF Mono", "Fira Code", monospace; font-size: 13px; }
  .user-count { font-weight: 700; font-variant-numeric: tabular-nums; font-size: 14px; color: #F7F8F8; }

  /* Failure breakdown bars */
  .fail-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 4px 0; }
  .fail-name { width: clamp(75px, 7vw, 110px); font-size: 13px; color: #8B95A5; text-align: right; flex-shrink: 0; font-weight: 500; }
  .fail-bar-bg { flex: 1; height: clamp(16px, 1.4vw, 24px); background: #0B0D11; border-radius: 6px; overflow: hidden; border: 1px solid #1E222B; }
  .fail-bar { height: 100%; border-radius: 5px; transition: width .5s ease; min-width: 2px; }
  .fail-count { width: 34px; font-size: 13px; font-weight: 700; text-align: right; flex-shrink: 0; font-variant-numeric: tabular-nums; color: #F7F8F8; }

  /* Geography bars */
  .geo-row { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 4px 0; }
  .geo-name { width: clamp(90px, 9vw, 150px); font-size: 13px; color: #8B95A5; text-align: right; flex-shrink: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-weight: 500; }
  .geo-bar-bg { flex: 1; height: clamp(16px, 1.4vw, 24px); background: #0B0D11; border-radius: 6px; overflow: hidden; border: 1px solid #1E222B; }
  .geo-bar { height: 100%; border-radius: 5px; transition: width .5s ease; min-width: 2px; background: #3BC4CC; }
  .geo-count { width: 34px; font-size: 13px; font-weight: 700; text-align: right; flex-shrink: 0; font-variant-numeric: tabular-nums; color: #F7F8F8; }
  ...
  .health-stat {
    ...
    padding: 12px 14px; background: #0B0D11; border-radius: 10px; border: 1px solid #1E222B;
  }
  ...
  .health-lbl { font-size: 11px; color: #636B78; text-transform: uppercase; letter-spacing: .5px; font-weight: 500; }
  .health-timing {
    display: flex; gap: 12px; margin-top: 12px; padding-top: 12px; border-top: 1px solid #1E222B;
  }
    ...
    <div class="panel">
      ...
      <div class="health-grid">
        <div class="health-stat">
          <span class="health-val" style="color:#2EBA54" id="healthDone">--</span>
        ...
        <div class="health-stat">
          <span class="health-val" style="color:#4B8BF5" id="healthRate">--</span>
          <span class="health-lbl">Success Rate</span>
        </div>
        <div class="health-stat">
          <span class="health-val" style="color:#E8853A" id="healthModRate">--</span>
          <span class="health-lbl">Rejection Rate</span>
        </div>
      </div>
      <div class="health-timing">
        <div class="health-stat">
          <span class="health-val health-val-sm" style="color:#9B6FE8" id="avgGenTime">--</span>
          <span class="health-lbl">Avg Generation</span>
        </div>
        <div class="health-stat">
          <span class="health-val health-val-sm" style="color:#2EBA54" id="avgPrintTime">--</span>
...
<div class="section-group sg-operations">
  ...
  <div class="grid">
    <div class="panel">
      ...
      <div style="margin-top:20px; padding-top:16px; border-top:1px solid #1E222B">
        <h2>Printers</h2>
        <div id="printerList"><span style="color:#636B78">--</span></div>
      </div>
    </div>
    <div class="panel">
      <h2>Paper Counter <span style="font-size:10px;font-weight:400;text-transform:none;letter-spacing:0;color:#636B78">(Estimated)</span></h2>
      ...
      <div style="text-align:center;font-size:12px;color:#4D5562;margin-bottom:14px">Based on prints sent &mdash; press Reset after reloading tray</div>
...
function render(d) {
  ...
  if (d.printers && d.printers.length > 0) {
    for (const p of d.printers) {
      phtml += '<div class="printer-status"><span class="printer-dot" style="background:' + (printerColors[p.status] || "#8B95A5") + '"></span><span>' + p.name + ' — ' + p.message + '</span></div>';
    }
  } else {
    phtml = '<div class="printer-status"><span class="printer-dot" style="background:#E8853A"></span><span>No printers active</span></div>';
  }
  ...
  const FAIL_META = [
    ...
    { key: "max_retries", label: "Crash Recovery", color: "#8B95A5" },
