...
router.get("/api/failed-jobs", async (req, res) => {
    const eventFilter = req.query.e || "all";
    const candidates = readJobs(FAILED_DIR).filter((j) => {
...
router.get("/api/done-jobs", async (req, res) => {
    const eventFilter = req.query.e || "all";
    const candidates = readJobs(DONE_DIR).filter((j) => {
        if (eventFilter !== "all" && j.eventName !== eventFilter) return false;
        return true;
    });
...
async function fetchJobs() {
  ...
  // Fire both requests in parallel — previously they were sequential, which
  // doubled the wait before jobs could render.
