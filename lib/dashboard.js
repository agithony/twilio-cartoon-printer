...
function getStuckJobs(eventFilter) {
    ...
    // Only report review as stuck if manual review is enabled
...
router.get("/api/stats", async (req, res) => {
    try {
        ...
        // Auto-approve stale review items if manual review is now OFF
        if (!settings.get("enableManualReview")) {
            const staleReview = getReviewQueue("all");
            for (const job of staleReview) {
                try {
                    await approveJob(job.filename);
                    console.log(`✅ Auto-approved (review disabled): ${job.filename}`);
                } catch (err) {
        ...
        const queue = {
            ...
            review: getReviewQueue(eventFilter).length,
...
router.post("/api/settings", async (req, res) => {
    const wasReviewEnabled = settings.get("enableManualReview");
    const result = settings.updateForUser(req.user.email, req.body);

    // Auto-approve all pending review items when manual review is turned OFF
    if (wasReviewEnabled && !settings.get("enableManualReview")) {
        const reviewJobs = getReviewQueue("all");
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Failed jobs list */
  .fj-list { max-height: 320px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--th-input-border) var(--th-card); }
  .fj-row {
...
function renderReviewQueue(jobs) {
  ...
  for (var j of jobs) {
    ...
    h += '<img src="'+imgSrc+'" onclick="openReviewModal(\\'/images/staging/'+fp+'_output.png\\')" title="Click to enlarge">';
    ...
    h += '<button class="rv-btn rv-btn-reanalyze" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{reanalyze:true})" title="Discard and re-generate with fresh scene analysis — the user is not notified (use when subject count or pets are wrong)">Reject + Re-analyze</button>';
    h += '</div></div></div>';
  }
  grid.innerHTML = h;
...
async function reviewAction(filename, action, opts) {
  try {
    ...
    if (d.ok) {
      ...
      var card = document.getElementById("rv-"+filename);
      if (card) card.remove();
      var msg = action === "approve" ? "Image approved" : opts.reanalyze ? "Image rejected, re-generating with fresh analysis" : opts.notify ? "Image rejected, user notified" : "Image rejected";
      showToast(msg);
      fetchReviewQueue();
      fetchStats();
    } else {
