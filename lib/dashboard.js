...
router.get("/api/stats", async (req, res) => {
    try {
        ...
        // Auto-approve stale review items whose event has manual review OFF
...
function renderReviewQueue(jobs) {
  ...
  for (var j of jobs) {
    ...
    h += '<button class="rv-btn rv-btn-reanalyze" onclick="reviewAction(\\''+fn+'\\',\\'reject\\',{reanalyze:true})" title="Re-generate with fresh analysis — optionally add instructions (e.g. fix the logo, include the dog, fix second person)">Re-analyze</button>';
...
async function reviewAction(filename, action, opts) {
  try {
    ...
    if (opts.reanalyze) {
      ...
      var fb = prompt("Optional: describe what to fix (e.g. fix the logo, include the dog, remove the hat).\\n\\nLeave blank to re-generate with fresh analysis only.");
      if (fb === null) return; // cancelled
      if (fb.trim()) payload.feedback = fb.trim();
    }
