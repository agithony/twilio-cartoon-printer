...
router.post("/api/events/:eventName/reset-data", async (req, res) => {
    ...
    if (!eventName || eventName.includes("..") || eventName.includes("/")) {
    ...
    _statsCache.ts = 0;
    _jobsCache.clear();
    // Rebuild the usage cache since we deleted jobs it counted.
    try { const { buildUsageCache } = require("./queue"); buildUsageCache(); } catch {}
