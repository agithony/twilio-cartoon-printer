...
// Read another event's style prompt overrides (for importing into current event)
router.get("/api/settings/event-styles/:eventName", (req, res) => {
    ...
    res.json({ stylePromptOverrides: eventSettings.stylePromptOverrides || {} });
});
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .audit-actor { color: var(--th-text-muted); min-width: 120px; overflow: hidden; text-overflow: ellipsis; }
  ...
  .user-phone { color: var(--th-text-secondary); font-family: 'Twilio Sans Mono', monospace; font-size: 13px; }
  .user-count { font-weight: 700; font-variant-numeric: tabular-nums; font-size: 14px; color: var(--th-text); font-family: 'Twilio Sans Display', sans-serif; letter-spacing: 0.02em; }
...
<div class="section-group sg-audit">
  ...
  <div class="grid">
    <div class="panel" style="grid-column: 1 / -1">
      ...
      <div id="auditLog" style="font-size:13px;color:var(--th-text-muted)">Loading...</div>
...
function render(d) {
  ...
  document.getElementById("topUsers").innerHTML = uhtml || '<div style="color:var(--th-text-muted);font-size:13px">No data yet</div>';
...
document.addEventListener("visibilitychange", function() {
  if (!document.hidden) { fetchStats(); fetchReviewQueue(); }
});
...
function renderAuditEntry(e) {
  ...
  if (e.action === "settings.update" && e.before && e.after) {
    ...
    changes = keys.map(function(k) {
      return '<span class="key">' + k + '</span>: <span class="old">' + fmtValue(e.before[k]) + '</span> &rarr; <span class="new">' + fmtValue(e.after[k]) + '</span>';
    }).join("<br>");
  } else if (e.action === "settings.revert") {
    changes = '<em>Reverted change ' + (e.originalId || "").slice(0, 8) + '</em>';
  }
  var revertHtml = "";
  if (e.action === "settings.update") {
    if (e.reverted) {
      ...
      revertHtml = '<button onclick="revertAudit(\'' + e.id + '\', this)">Revert</button>';
    }
  }
