...
const { getPrinterBusyState, incrementUsage } = require("./queue");
...
router.get("/api/settings/files", async (req, res) => {
    ...
    res.json({ templates, videos, printers, events, brandReferences });
});
...
function render(d) {
  ...
  d.queue = d.queue || { pending: 0, generating: 0, ready: 0, printing: 0 };
