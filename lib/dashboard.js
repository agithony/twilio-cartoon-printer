...
function renderDoneJobs(jobs) {
  ...
  for (var j of jobs) {
    ...
    if (j.canReprint) {
      h += printerDropdown("dj-ptr-"+escHtml(j.filename), j.printerName ? "Same printer" : "Any printer");
      h += '<button class="dj-reprint" onclick="reprintJob(\\''+escHtml(j.filename)+'\\',\\''+escHtml(j.printerName || "")+'\\')">Reprint</button>';
    } else {
...
async function reprintJob(filename, origPrinter) {
  try {
    var body = { filename };
    ...
    if (sel && sel.value) {
      ...
      body.targetPrinter = origPrinter;
    }
