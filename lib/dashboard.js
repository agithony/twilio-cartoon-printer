...
router.get("/api/done-jobs", async (req, res) => {
    ...
    // Parallel access() checks replace per-job sync existsSync + per-row
    // browser 404s. Two checks per job (output.png for reprint, mms.jpg for
    // thumb) fire concurrently; Azure Files rtt is ~30-80ms so the fan-out
    // matters.
    const jobs = await Promise.all(candidates.map(async (j) => {
        const filename = `${j.filePrefix}.json`;
        const evName = j.eventName || eventFilter;
        const { outputPath } = jobPaths(j);
        let canReprint = false;
        let hasOutputThumb = false;
        await Promise.all([
            fsp.access(outputPath).then(() => { canReprint = true; }).catch(() => {}),
            (async () => {
                if (!j.filePrefix || !evName) return;
                const mmsPath = path.join(settings.getDownloadDir(evName), `${j.filePrefix}_output_mms.jpg`);
                try { await fsp.access(mmsPath); hasOutputThumb = true; } catch {}
            })(),
        ]);
        return {
            filename,
...
function renderActiveRow(j) {
  ...
  if (j.filePrefix && j.eventName) {
    var thumbUrl = "/photogallery/img/" + encodeURIComponent(j.eventName) + "/" + encodeURIComponent(j.filePrefix) + "_output_mms.jpg";
