...
router.get("/api/stats", async (req, res) => {
    try {
        ...
        const relayEntries = relayList
            ...
            .map(rp => ({ name: rp.name, status: "ready", message: "Relay", source: "relay" }));
...
router.get("/api/dashboard-users", (req, res) => {
    ...
    for (const [phone, count] of userMap) {
        users.push({
            ...
            leadCompleted: leads.isCompleted(phone, eventName),
        });
    }
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Paper counter */
  .paper-big { font-size: clamp(36px, 3.5vw, 60px); font-weight: 800; text-align: center; line-height: 1; font-variant-numeric: tabular-nums; font-family: 'Twilio Sans Display', sans-serif; letter-spacing: 0.02em; }
  .paper-sub { text-align: center; font-size: 13px; color: var(--th-text-muted); margin: 6px 0 16px; }
  .progress-bar { height: 8px; background: var(--th-card-border); border-radius: 4px; overflow: hidden; margin-bottom: 18px; }
  ...
  .paper-controls label { font-size: 13px; color: var(--th-text-muted); display: flex; align-items: center; gap: 6px; }
  .paper-controls input {
    width: 56px; background: var(--th-bg); border: 1px solid var(--th-card-border); border-radius: 8px;
    color: var(--th-text-dim); padding: 6px 8px; font-size: 13px; text-align: center; font-family: inherit;
    transition: border-color .15s;
  }
  .paper-controls input:focus { outline: none; border-color: var(--blue-500); box-shadow: 0 0 0 3px rgba(24,102,238,.1); }
  .btn {
  ...
  .paper-alert.warning, .stuck-alert.warning { display: block; background: rgba(25,171,243,.07); color: var(--blue-300); border: 1px solid rgba(25,171,243,.2); }
  .paper-alert.empty { display: block; background: rgba(239,34,58,.07); color: var(--brand-red); border: 1px solid rgba(239,34,58,.2); }
  ...
  .dj-list { max-height: 420px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--th-input-border) var(--th-card); }
  .dj-row {
  ...
  .dj-phone { color: var(--th-text-secondary); font-family: 'Twilio Sans Mono', monospace; min-width: 100px; }
  .dj-style { color: var(--th-text-muted); min-width: 70px; }
  .dj-printer { color: var(--th-text-muted); font-size: 11px; }
  .dj-time { color: var(--th-text-muted); font-size: 11px; min-width: 50px; text-align: right; flex: 1; }
  .dj-reprint {
  ...
  .dj-reprint:hover { background: rgba(33,136,239,.16); border-color: var(--blue-400); }
  .dj-reprint:disabled { opacity: .4; cursor: default; }
  .dj-empty { color: var(--th-text-muted); font-size: 13px; padding: 16px 0; }
  ...
  .user-phone { color: var(--th-text-secondary); font-family: 'Twilio Sans Mono', monospace; font-size: 13px; flex: 1; min-width: 0; }
  ...
  /* Failed jobs list */
  .fj-list { max-height: 420px; overflow-y: auto; scrollbar-width: thin; scrollbar-color: var(--th-input-border) var(--th-card); }
  .fj-row {
    display: flex; flex-wrap: wrap; align-items: center; gap: 8px 10px; padding: 10px 12px; margin-bottom: 4px;
    background: var(--th-bg); border-radius: 8px; border: 1px solid var(--th-card-border); font-size: 13px;
  }
  .fj-phone { color: var(--th-text-secondary); font-family: 'Twilio Sans Mono', monospace; min-width: 100px; }
  .fj-style { color: var(--th-text-muted); min-width: 70px; }
  .fj-reason {
    font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 6px;
    text-transform: uppercase; letter-spacing: .3px;
  }
  .fj-reason.moderation { background: rgba(239,34,58,.09); color: var(--brand-red); }
  .fj-reason.face_detection { background: rgba(25,171,243,.09); color: var(--blue-300); }
  .fj-reason.multi_subject { background: rgba(206,147,37,.09); color: #ce9325; }
  .fj-reason.ai_review_rejected { background: rgba(164,94,229,.09); color: #a45ee5; }
  .fj-reason.review_rejected { background: rgba(164,94,229,.09); color: #a45ee5; }
  .fj-reason.content_rejected { background: rgba(239,34,58,.09); color: var(--brand-red); }
  .fj-reason.generation { background: rgba(24,102,238,.09); color: var(--blue-500); }
  .fj-reason.printer { background: rgba(33,136,239,.09); color: var(--blue-400); }
  .fj-reason.max_retries { background: rgba(107,117,133,.09); color: var(--th-text-muted); }
  .fj-reason.unknown { background: rgba(107,117,133,.09); color: var(--th-text-muted); }
  .fj-time { color: var(--th-text-muted); font-size: 11px; min-width: 50px; text-align: right; flex: 1; }
  .fj-detail {
  ...
  .fj-retry:hover { background: rgba(33,136,239,.16); border-color: var(--blue-400); }
  .fj-retry:disabled { opacity: .4; cursor: default; }
  .fj-empty { color: var(--th-text-muted); font-size: 13px; padding: 16px 0; }
  .fj-toast {
...
<div class="section-group sg-operations">
  ...
  <div class="grid">
    ...
    <div class="panel">
      <h2>Paper Counter <span style="font-size:10px;font-weight:400;text-transform:none;letter-spacing:0;color:var(--th-text-muted)">(Estimated)</span></h2>
      ...
      <div style="text-align:center;font-size:12px;color:var(--th-text-muted);margin-bottom:14px">Based on prints sent &mdash; press Reset after reloading tray</div>
...
<div class="section-group sg-users">
  ...
  <div class="grid">
    <div class="panel">
      <h2>Users</h2>
      <div id="dashboardUsers" class="fj-list"></div>
    </div>
    <div class="panel">
      <h2>Job Health</h2>
...
<div class="section-group sg-failures">
  ...
  <div class="panel" style="margin-top:14px">
    <h2>Failed Jobs</h2>
    <div id="failedJobsList" style="color:var(--th-text-muted);font-size:13px">Loading...</div>
  </div>
  <div class="panel" style="margin-top:14px">
    <h2>Completed Jobs</h2>
    <div id="doneJobsList" style="color:var(--th-text-muted);font-size:13px">Loading...</div>
  </div>
</div>
...
function render(d) {
  ...
  document.getElementById("paperRemaining").style.color = p.isEmpty ? "#EF223A" : p.isWarning ? "#19ABF3" : "var(--th-text)";
  ...
  bar.style.width = pct + "%";
  bar.style.background = p.isEmpty ? "#EF223A" : p.isWarning ? "#19ABF3" : "#2188EF";
...
document.addEventListener("visibilitychange", function() {
  if (!document.hidden) { fetchStats(); fetchReviewQueue(); fetchAuditLog(false); fetchDashboardUsers(); }
});
...
// escHtml is defined below with full escaping (including " and ')
...
function renderFailedJobs(jobs) {
  ...
  var oldList = el.querySelector(".fj-list");
  ...
  for (var j of jobs) {
    ...
    h += '<div class="fj-row" id="fj-'+escHtml(j.filename)+'">';
    h += '<span class="fj-phone">'+escHtml(j.phone)+'</span>';
    h += '<span class="fj-style">'+escHtml(j.style)+'</span>';
    h += '<span class="fj-reason '+escHtml(j.failReason)+'">'+escHtml(j.failReason).replace(/_/g," ")+'</span>';
    h += '<span class="fj-time">'+timeAgo(j.createdAt)+'</span>';
    if (j.canRetry && isPrinterFail) {
      h += printerDropdown("fj-ptr-"+escHtml(j.filename), "Any printer");
      h += '<button class="fj-retry" onclick="retryJob(&apos;'+escHtml(j.filename)+'&apos;,true)" title="Re-queue directly to print — image already exists">Retry Print</button>';
    } else if (j.canRetry) {
      h += '<button class="fj-retry" onclick="retryJob(&apos;'+escHtml(j.filename)+'&apos;,false)" title="Re-queue this job for fresh generation — the user is not notified">Retry</button>';
    } else {
      h += '<button class="fj-retry" disabled title="Cannot retry moderation failures">Retry</button>';
    }
    if (detail) h += '<div class="fj-detail">'+detail+'</div>';
    h += '</div>';
  }
  h += '</div>';
  el.innerHTML = h;
  var newList = el.querySelector(".fj-list");
...
async function retryJob(filename, isPrintRetry) {
  try {
    var body = { filename };
    if (isPrintRetry) {
      var sel = document.getElementById("fj-ptr-" + filename);
...
function renderDoneJobs(jobs) {
  var el = document.getElementById("doneJobsList");
  if (!jobs.length) { el.innerHTML = '<div class="dj-empty">No completed jobs</div>'; return; }
  // Preserve scroll position across re-renders
  var oldList = el.querySelector(".dj-list");
  var scrollTop = oldList ? oldList.scrollTop : 0;
  var h = '<div class="dj-list">';
  for (var j of jobs) {
    h += '<div class="dj-row" id="dj-'+escHtml(j.filename)+'">';
    h += '<span class="dj-phone">'+escHtml(j.phone)+'</span>';
    h += '<span class="dj-style">'+escHtml(j.style)+'</span>';
    if (j.printerName) h += '<span class="dj-printer">'+escHtml(j.printerName)+'</span>';
    h += '<span class="dj-time">'+timeAgo(j.completedAt)+'</span>';
    if (j.canReprint) {
      h += printerDropdown("dj-ptr-"+escHtml(j.filename), "Any printer");
      h += '<button class="dj-reprint" onclick="reprintJob(&apos;'+escHtml(j.filename)+'&apos;)">Reprint</button>';
    } else {
      h += '<button class="dj-reprint" disabled title="Output image no longer exists">Reprint</button>';
    }
    h += '</div>';
  }
  h += '</div>';
  el.innerHTML = h;
  var newList = el.querySelector(".dj-list");
  if (newList && scrollTop) newList.scrollTop = scrollTop;
}
...
async function reprintJob(filename) {
  try {
    var body = { filename };
    var sel = document.getElementById("dj-ptr-" + filename);
    if (sel && sel.value) {
    ...
    var d = await r.json();
    if (d.ok) {
      var row = document.getElementById("dj-"+filename);
      if (row) row.remove();
...
fetchDoneJobs();
setInterval(fetchDoneJobs, 10000);
