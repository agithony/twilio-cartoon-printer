...
// TTL cache for directory reads — avoids re-reading hundreds of files from
// Azure File Share on every stats poll (each readFileSync has network latency).
const _dirCache = new Map();
const DIR_CACHE_TTL = 3_000; // 3 seconds

function _cachedReaddir(dir) {
    const now = Date.now();
...
function readJobs(dir) {
    ...
    try {
        const files = _cachedReaddir(dir);
        const jobs = files.map((f) => {
            ...
            .filter(Boolean);
        _jobsCache.set(dir, { jobs, ts: now });
        return jobs;
    } catch {
        return [];
    }
...
function countFiles(dir) {
    return _cachedReaddir(dir).length;
}
...
const STATS_CACHE_TTL = 3_000; // 3 seconds
...
router.post("/api/settings", async (req, res) => {
    ...
    // Auto-approve pending review items for current event when manual review is turned OFF
    if (wasReviewEnabled && !settings.get("enableManualReview")) {
        const eventName = settings.get("eventName");
        const reviewJobs = getReviewQueue(eventName);
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rv-images { display: flex; gap: 6px; margin-bottom: 8px; }
  .rv-images img { flex: 1; min-width: 0; border-radius: 6px; cursor: pointer; object-fit: cover; aspect-ratio: 2/3; }
  .rv-images .rv-img-label {
    position: absolute; bottom: 4px; left: 50%; transform: translateX(-50%);
    font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px;
    background: rgba(0,0,0,.6); color: #fff; padding: 2px 6px; border-radius: 4px;
    pointer-events: none;
  }
...
function showFeedback(fn) {
  ...
  if (panel) { panel.classList.add("open"); panel.querySelector("textarea").focus(); }
}
