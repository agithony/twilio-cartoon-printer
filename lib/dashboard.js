...
router.post("/api/settings/upload", (req, res) => {
    ...
    if (!["template", "video", "brand-reference", "booth-image"].includes(type)) {
        return res.status(400).json({ error: "type must be 'template', 'video', 'brand-reference', or 'booth-image'" });
    }
    ...
    req.on("end", () => {
        ...
        writeStream.end(() => {
            ...
            const files = type === "template" ? settings.listTemplates() : type === "brand-reference" ? settings.listBrandReferences() : type === "video" ? settings.listVideos() : [];
...
function render(d) {
  ...
  if (firstLoad && d.eventName && d.events && d.events.includes(d.eventName)) {
    ...
    fetchStats();
    fetchReviewQueue();
    _duFetching = false; // ensure re-fetch isn't blocked by the initial "all" fetch
    fetchDashboardUsers();
    return;
  }
  firstLoad = false;
...
// ── User Management ──
var _duFetching = false;
async function fetchDashboardUsers() {
  if (_duFetching) return;
  _duFetching = true;
  try {
  ...
  } catch(e) { /* retry next poll */ } finally { _duFetching = false; }
}
...
function renderDashboardUsers(users) {
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    html += '<div class="user-row">'
      ...
      + '<button class="btn btn-xs" onclick="resetUserUsage('+u.id+')" title="Reset usage count to 0">Reset</button>'
...
function resetUserUsage(id) {
  if (selectedEvent === "all") { showToast("Select a specific event to reset usage"); return; }
  var ev = selectedEvent;
  if (!confirm("Reset this user's usage count to 0 for " + ev + "?")) return;
  fetch("api/dashboard-users/reset-usage", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ id: id, event: ev }) })
    ...
    .then(function(d) { if (d.ok) { showToast("Usage reset"); _duFetching = false; fetchDashboardUsers(); fetchStats(); } else { showToast("Error: " + (d.error || "unknown")); } })
    .catch(function() { showToast("Failed to reset usage"); });
}
...
function toggleUserAdmin(id) {
  ...
  fetch("api/dashboard-users/toggle-admin", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ id: id, event: ev }) })
    .then(function(r) { return r.json(); })
    .then(function(d) { if (d.ok) { showToast(d.isAdmin ? "Admin added" : "Admin removed"); _duFetching = false; fetchDashboardUsers(); fetchStats(); } else { showToast("Error: " + (d.error || "unknown")); } })
...
function renderFailedJobs(jobs) {
  ...
  for (var j of jobs) {
    ...
    if (j.canRetry) {
      h += '<button class="fj-retry" onclick="retryJob(\\''+j.filename+'\\')" title="Re-queue this job for fresh generation — the user is not notified">Retry</button>';
    } else {
