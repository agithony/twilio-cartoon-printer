...
function renderShareFunnel(d) {
  ...
  for (var ri = 0; ri < recent.length; ri++) {
    ...
    if (showPII) {
      ...
      who.textContent = r.firstName ? r.firstName : "(identified user — add ?xa=1 to reveal)";
    }
...
function renderMultiReviewCard(j) {
  ...
  variants.forEach(function(v, idx){
    ...
    h += '<div class="rv-variant'+(failed?" is-failed":"")+'" data-variant-idx="'+idx+'" data-variant-id="'+vid+'" id="rvv-'+pid+'-'+idx+'">';
    h += '<div class="rv-v-imgwrap" onclick="openMultiReviewModal(\\''+pid+'\\','+idx+')">';
    ...
    if (failed) {
      ...
      h += '<button class="rv-v-send" onclick="pickVariantDash(\\''+pid+'\\',\\''+vid+'\\')" title="Send variant '+(idx+1)+' to the user via MMS">Send</button>';
      h += '<button class="rv-v-regen" onclick="regenVariantDash(\\''+pid+'\\',\\''+vid+'\\')"'+regenDisabled+' title="'+regenTitle+'">↻ Regen</button>';
    }
    h += '</div>';
    h += '</div>';
  });
