...
router.get("/api/images", (req, res) => {
    try {
        ...
        const allFiles = new Set(fs.readdirSync(dlDir));
        const images = [...allFiles]
            .filter((f) => f.endsWith("_output.png"))
            .sort()
            .reverse()
            .map((f) => ({
                file: f,
                hasSelfie: allFiles.has(f.replace(/_output\.png$/, "_input.jpg")),
            }));
        res.json({ images, total: images.length, revealAnimation: settings.get("revealAnimation") || "off", photoBookAutoplay: settings.get("photoBookAutoplay") !== false, photoBookInterval: settings.get("photoBookInterval") || 10 });
    } catch {
        res.json({ images: [], total: 0, revealAnimation: settings.get("revealAnimation") || "off", photoBookAutoplay: settings.get("photoBookAutoplay") !== false, photoBookInterval: settings.get("photoBookInterval") || 10 });
    }
});
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Reveal animations ── */
  @keyframes reveal-pixel { 0% { filter: blur(20px) brightness(0.5); } 100% { filter: blur(0) brightness(1); } }
  @keyframes reveal-brush { 0% { clip-path: inset(0 100% 0 0); } 100% { clip-path: inset(0 0 0 0); } }
  @keyframes reveal-sketch-to-color { 0% { filter: grayscale(1) contrast(2); } 100% { filter: grayscale(0) contrast(1); } }
  #mainImg.reveal-pixel { animation: reveal-pixel 1.5s ease-out forwards; }
  #mainImg.reveal-brush { animation: reveal-brush 1.5s ease-in-out forwards; }
  #mainImg.reveal-sketch-to-color { animation: reveal-sketch-to-color 2s ease-out forwards; }

  /* Original-removed placeholder (back face of flip) */
  .original-removed {
    position: absolute; top: 0; left: 0; width: 100%; height: 100%;
    transform: rotateY(180deg);
    backface-visibility: hidden;
  ...
  .removed-content svg {
    width: 36px; height: 36px; margin-bottom: 14px; opacity: 0.6;
  }
  .removed-title {
  ...
  .removed-desc {
    font-family: 'Twilio Sans Text', sans-serif;
    font-weight: 400; font-size: 0.8rem; color: rgba(255,255,255,0.5);
  ...
  <div class="viewer" id="viewer">
    ...
    <div class="img-flipper" id="flipper" style="display:none">
      <div class="flip-inner">
        ...
        <div class="original-removed" id="origPlaceholder" style="display:none"><div class="removed-content"><svg viewBox="0 0 24 24" fill="none" stroke="#EF223A" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/><circle cx="12" cy="16" r="1"/></svg><div class="removed-title">Original Photo Removed</div><div class="removed-desc">The original selfie has been deleted to protect the subject&#39;s privacy.</div></div></div>
      </div>
...
function originalUrl(filename) { return "/images/" + filename.replace("_output.png", "_input.jpg"); }
...
async function fetchImages() {
  ...
  try {
    ...
    if (newInterval !== AUTO_INTERVAL || newAutoplay !== photoBookAutoplay) {
      ...
      else if (!photoBookAutoplay) { stopAuto(); paused = true; }
    }
  ...
  } catch(e) { /* retry next poll */ } finally { _fetching = false; }
}
...
function showImage(idx) {
  ...
  // Toggle back face: real image vs placeholder
  var placeholder = document.getElementById("origPlaceholder");
  var entry = images[idx];
  if (entry.hasSelfie !== false) {
    orig.style.display = "";
  ...
  img.classList.remove("reveal-pixel", "reveal-brush", "reveal-sketch-to-color");
  setTimeout(function() {
    img.onload = function() {
      img.classList.remove("fade-out");
    };
    img.src = imgUrl(entry.file);
    if (entry.hasSelfie !== false) orig.src = originalUrl(entry.file);
  }, 200);
...
function toggleOriginal() {
  ...
  if (showingOriginal) {
    ...
    if (revealAnimation !== "off") {
      ...
      img.classList.add("reveal-" + revealAnimation);
    }
  }
}
...
function renderThumbs() {
  ...
  for (var i = 0; i < images.length; i++) {
    html += '<div class="thumb' + (i === currentIdx ? ' active' : '') + '" data-idx="' + i + '">'
      + '<img src="' + imgUrl(images[i].file) + '" loading="lazy" alt="">'
      + '</div>';
  }
...
// ── Wake Lock (keep screen on in fullscreen) ──
...
document.addEventListener("webkitfullscreenchange", handleWakeLock);
