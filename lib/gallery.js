...
router.get("/api/images", (req, res) => {
    try {
        const files = fs.readdirSync(settings.getDownloadDir())
            ...
            .sort()
            .reverse();
        res.json({ images: files, total: files.length, revealAnimation: settings.get("revealAnimation") || "off" });
    } catch {
        res.json({ images: [], total: 0, revealAnimation: settings.get("revealAnimation") || "off" });
    }
});
