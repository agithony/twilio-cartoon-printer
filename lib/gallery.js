...
router.get("/api/images", (req, res) => {
    try {
        const files = fs.readdirSync(settings.getDownloadDir())
            .filter((f) => f.endsWith("_output_mms.jpg"))
            .sort()
            .reverse();
        res.json({ images: files, total: files.length, revealAnimation: settings.get("revealAnimation") || "off", photoBookAutoplay: settings.get("photoBookAutoplay") !== false, photoBookInterval: settings.get("photoBookInterval") || 10 });
    } catch {
...
function showImage(idx) {
  ...
  // Crossfade / reveal
  var isFirstReveal = revealAnimation !== "off" && !revealedImages.has(images[idx]);
