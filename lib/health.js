...
function mountHealth(app) {
    app.get("/healthz", (req, res) => ready ? res.send("ok") : res.status(503).send("starting"));

    // Detailed health (dashboard / debugging only)
    app.get("/healthz/details", async (req, res) => {
        try {
...
module.exports = { trackApiCall, getHealthStatus, mountHealth, setReady, isReady };
