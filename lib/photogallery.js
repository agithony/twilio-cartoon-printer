...
router.get("/api/events", async (req, res) => {
    try {
        ...
        res.json({ events, currentEvent: settings.get("eventName"), termsUrl: settings.get("termsUrl") || "", revealAnimation: settings.get("revealAnimation") || "off" });
    } catch {
        res.json({ events: [], currentEvent: settings.get("eventName"), termsUrl: settings.get("termsUrl") || "", revealAnimation: settings.get("revealAnimation") || "off" });
    }
});
...
.portrait.reveal-sketch-to-color { animation: reveal-sketch 2s ease-out forwards; }
