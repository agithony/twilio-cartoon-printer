...
@media (orientation: portrait) {
  ...
  html:not(.combo-embedded) .book-wrap { width: 94vw; height: 58vh; }
...
html.portrait-host .book-wrap { width: 94vw; height: 56vh; }
...
html[data-theme="light"] body {
  ...
  background-image: radial-gradient(ellipse at 50% 40%, rgba(239,34,58,0.06) 0%, transparent 70%);
}
...
function configureNativePageDrag() {
  var standalonePortrait = window.top === window && window.matchMedia("(orientation: portrait)").matches;
  if (!bookReady || (!standalonePortrait && !hostIsPortrait())) return;
  try {
    var data = $("#book").data();
    var cornerSize = Math.max(100, Math.min(180, document.getElementById("book").clientHeight * 0.28));
    for (var page in data.pages) {
      if (Object.prototype.hasOwnProperty.call(data.pages, page)) {
        data.pages[page].flip("options", { cornerSize: cornerSize });
      }
    }
  } catch(e) {}
}
