...
router.get("/api/images", (req, res) => {
    try {
        ...
        for (const { dir, event: ev } of dirs) {
            ...
            const files = fs.readdirSync(dir).filter((f) => f.endsWith("_output_mms.jpg"));
            for (const f of files) images.push({ file: f, event: ev });
        }
...
// BRB button — only when standalone (not in combo iframe)
if (window.self === window.top) {
  document.getElementById("brbBtn").addEventListener("click", function() { toggleBrb(); });
} else {
  var bb = document.getElementById("brbBtn"); if (bb) bb.style.display = "none";
}
</script>
</body>
</html>`;
