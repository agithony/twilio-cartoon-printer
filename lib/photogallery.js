...
// Cached prefix-to-job metadata map (stale-while-revalidate; never blocks event loop)
let _styleMapCache = { map: {}, ts: 0 };
let _styleMapRefresh = null;
...
async function _refreshStyleMap() {
    if (_styleMapRefresh) return _styleMapRefresh;
    const dirs = [DONE_DIR, FAILED_DIR, READY_DIR, PRINTING_DIR, REVIEW_DIR, GENERATING_DIR, PENDING_DIR];
    _styleMapRefresh = (async () => {
        const map = {};
        for (const qDir of dirs) {
            try {
                const files = (await fsp.readdir(qDir)).filter(f => f.endsWith(".json"));
                for (let i = 0; i < files.length; i += STYLE_READ_BATCH_SIZE) {
                    const jobs = await Promise.all(files.slice(i, i + STYLE_READ_BATCH_SIZE).map(async (f) => {
                        try {
                            return JSON.parse(await fsp.readFile(path.join(qDir, f), "utf-8"));
                        } catch {
                            return null;
                        }
                    }));
                    for (const j of jobs) {
                        if (j && j.filePrefix && j.eventName) {
                            map[`${j.eventName}:${j.filePrefix}`] = {
                                style: j.style || null,
                                styleName: j.styleName,
                                locale: j.locale,
                                // Jobs created before output profiles were portrait-only.
                                orientation: j.outputProfile && j.outputProfile.orientation || "portrait",
                            };
                        }
                    }
                }
    ...
    try {
        return await _styleMapRefresh;
    } finally {
        _styleMapRefresh = null;
    }
}

async function _getStyleMap() {
    const now = Date.now();
    if (_styleMapCache.ts === 0) {
        _refreshStyleMap().catch(() => {});
...
router.get("/api/images", async (req, res) => {
    try {
        ...
        const styleMap = await _getStyleMap();
        ...
        for (const { dir, event: ev } of dirs) {
            ...
            for (let i = 0; i < outputFiles.length; i += STYLE_READ_BATCH_SIZE) {
                const batch = await Promise.all(outputFiles.slice(i, i + STYLE_READ_BATCH_SIZE).map(async (f) => {
                    ...
                    const styleInfo = styleMap[`${ev}:${prefix}`] || {};
...
module.exports = { buildPageHtml, mountPhotoGallery };
