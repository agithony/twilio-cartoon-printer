...
// Cached prefix→style map (avoids re-reading hundreds of job files on every poll)
let _styleMapCache = { map: null, ts: 0 };
const STYLE_MAP_TTL = 5_000; // 5 seconds

function _getStyleMap() {
    const now = Date.now();
    if (_styleMapCache.map && (now - _styleMapCache.ts) < STYLE_MAP_TTL) {
        return _styleMapCache.map;
    }
    const map = {};
    for (const qDir of [DONE_DIR, READY_DIR, PRINTING_DIR, REVIEW_DIR, GENERATING_DIR, PENDING_DIR]) {
        try {
            if (!fs.existsSync(qDir)) continue;
            for (const f of fs.readdirSync(qDir).filter(f => f.endsWith(".json"))) {
                try {
                    const j = JSON.parse(fs.readFileSync(path.join(qDir, f), "utf-8"));
                    if (j.filePrefix && j.style) map[j.filePrefix] = j.style;
                } catch {}
            }
        } catch {}
    }
    _styleMapCache = { map, ts: now };
    return map;
}
...
router.get("/api/events", (req, res) => {
    try {
        ...
        const events = fs.readdirSync(dlRoot, { withFileTypes: true })
...
router.get("/api/images", (req, res) => {
    try {
        ...
        if (event && event !== "all") {
            ...
            const dlRoot = path.join(ROOT_DIR, "downloads");
            dirs = fs.readdirSync(dlRoot, { withFileTypes: true })
                .filter((d) => d.isDirectory())
        ...
        const activeStyleCount = Object.keys(activeStyles).length;
...
router.delete("/api/images", (req, res) => {
    ...
    if (!fs.existsSync(dir)) return res.json({ deleted: 0, errors: [] });
    ...
    for (const { prefix, type } of items) {
        ...
        for (const suffix of suffixes) {
            ...
            try {
                ...
                errors.push(`${prefix}${suffix}: ${e.message}`);
            }
        }
    }
...
router.delete("/api/images/all", (req, res) => {
...
/* Style tag */
.style-tag {
  position: absolute; bottom: clamp(12px,2vh,24px); right: clamp(14px,2vw,28px);
...
function initBook() {
  ...
  for (var i = 0; i < n; i++) {
    ...
    var styleTag = (activeStyleCount >= 2 && images[i].style) ? '<div class="style-tag">' + images[i].style + '</div>' : '';
    pg.innerHTML =
