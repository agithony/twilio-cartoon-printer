...
router.get("/api/events", async (req, res) => {
    try {
        ...
        res.json({ events, currentEvent: settings.get("eventName"), termsUrl: settings.get("termsUrl") || "", revealAnimation: settings.get("revealAnimation") || "off", photoBookAutoplay: settings.get("photoBookAutoplay") !== false, photoBookInterval: settings.get("photoBookInterval") || 10 });
    } catch {
        res.json({ events: [], currentEvent: settings.get("eventName"), termsUrl: settings.get("termsUrl") || "", revealAnimation: settings.get("revealAnimation") || "off", photoBookAutoplay: settings.get("photoBookAutoplay") !== false, photoBookInterval: settings.get("photoBookInterval") || 10 });
    }
});
...
.top-bar {
  ...
  padding: clamp(8px,1.2vh,14px) 20px clamp(4px,0.6vh,8px); gap: 0; position: relative; z-index: 5;
}
.top-bar-row {
...
.count-label {
  ...
  font-size: clamp(14px,1.2vw,20px); color: rgba(242,47,70,0.7);
...
.count-label strong {
  ...
  font-size: clamp(20px,1.8vw,34px); font-style: normal;
...
.milestone-banner {
  ...
  background: rgba(0,13,37,0.85); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
  border: 1px solid rgba(255,215,0,0.3); border-radius: 20px;
  padding: 32px 56px; text-align: center;
  font-family: 'Playfair Display', Georgia, serif; font-size: clamp(28px, 4vw, 56px); font-weight: 600;
  color: #FFD700; text-shadow: 0 0 30px rgba(255,215,0,0.3);
  box-shadow: 0 8px 60px rgba(0,0,0,0.6), 0 0 40px rgba(255,215,0,0.1);
...
function initBook() {
  ...
  for (var i = 0; i < n; i++) {
    ...
    var revealClass = (revealAnimation !== "off" && !revealedImages.has(images[i].file)) ? " reveal-" + revealAnimation : "";
    if (revealClass) revealedImages.add(images[i].file);
    pg.innerHTML =
      '<div class="photo-mount">' +
        '<div class="photo-frame">' +
          ...
          '<img class="portrait' + revealClass + '" src="' + imgUrl(images[i]) + '" loading="lazy" alt="">' +
...
async function fetchImages() {
  try {
    ...
    // Milestone check
    if (d.total > oldLen && oldLen > 0) {
      var thresholds = [50, 100];
      for (var m = 200; m <= 10000; m += 100) thresholds.push(m);
      for (var mi = 0; mi < thresholds.length; mi++) {
        if (oldLen < thresholds[mi] && d.total >= thresholds[mi] && thresholds[mi] > lastMilestone) {
          lastMilestone = thresholds[mi];
          sessionStorage.setItem("lastMilestone", String(lastMilestone));
          triggerMilestone(thresholds[mi]);
          break;
        }
      }
    }
...
function onFullscreenChange() { resizeBook(); handleWakeLock(); }
...
function triggerMilestone(count) {
  if (typeof confetti === "function") {
    confetti({ particleCount: 150, spread: 90, origin: { y: 0.6 }, colors: ["#FFD700", "#FF6B35", "#EF223A", "#2188EF", "#fff"] });
    setTimeout(function() {
      confetti({ particleCount: 80, spread: 120, origin: { y: 0.5 }, colors: ["#FFD700", "#FF6B35", "#EF223A", "#2188EF", "#fff"] });
    }, 300);
  }
