...
router.post("/", async (req, res) => {
    try {
        ...
        // Kiosk submissions always deliver via SMS, even if this phone was
        // previously seen on WhatsApp. Record "sms" explicitly so the job's
        // channel snapshot (set at enqueue time) is correct and all async
        // sends (delivery MMS, NPS, promo) don't silently route to WhatsApp.
        if (contactPhone) channel.record(contactPhone, "sms");
        const extras = {
            apiSubmission: true,
