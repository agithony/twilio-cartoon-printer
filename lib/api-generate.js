...
router.post("/", async (req, res) => {
    try {
        ...
        const imageUrl = `${baseUrl}/photogallery/img/${encodeURIComponent(eventName)}/${encodeURIComponent(inputFilename)}`;
        ...
        const languageMode = settings.getForEvent("languageMode", eventName);
        const requestedLocale = i18n.normalizeLocale(req.query.locale);
        const locale = languageMode === "ask"
            ? (requestedLocale || (contactPhone ? contacts.getPreferredLocale(contactPhone, eventName) : null) || i18n.DEFAULT_LOCALE)
        ...
        const extras = {
            ...
            channel: "sms",
            locale,
            styleName: optionI18n.localizeOption("style", style, activeStyles[style] || { name: style }, locale).name,
            eventName,
        };
...
router.get("/:prefix/debug", async (req, res) => {
    ...
    const outputFile = path.join(downloadDir, `${prefix}_output.png`);
    const stagingFile = path.join(downloadDir, ".staging", `${prefix}_output.png`);
    res.json({
        prefix,
        state,
        job,
...
// Streams the exact PNG output master.
...
router.get("/:prefix/result", async (req, res) => {
    ...
    const filePath = path.join(settings.getDownloadDir(eventName), `${prefix}_output.png`);
    if (!fs.existsSync(filePath)) {
        return res.status(404).json({ error: "Output image missing on disk" });
    }
    res.setHeader("Content-Type", "image/png");
    res.setHeader("Content-Disposition", `inline; filename="${prefix}_output.png"`);
