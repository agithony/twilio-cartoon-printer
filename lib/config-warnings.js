...
function channelWarnings(cfg) {
    const out = [];
    if (!cfg.twilioAccountSid) out.push({ fatal: true, message: "TWILIO_ACCOUNT_SID is required." });
    if (!cfg.twilioAuthToken) out.push({ fatal: true, message: "TWILIO_AUTH_TOKEN is required for API calls and webhook validation." });
    ...
    if (!hasSms && !hasWa) {
        out.push({ fatal: true, message: "No sender configured — set an SMS or WhatsApp number/Messaging Service SID." });
    }
