...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .brand-ref-tag { display: inline-flex; align-items: center; gap: 8px; padding: 5px 12px; border-radius: 8px; background: #1c2230; color: #b8c0cc; font-size: 13px; border: 1px solid #252d3a; transition: border-color .15s; }
  .brand-ref-tag:hover { border-color: #2e3744; }
  .brand-ref-tag img { width: 32px; height: 32px; object-fit: cover; border-radius: 6px; }
  .brand-ref-tag .remove { cursor: pointer; color: #525c6c; font-weight: bold; font-size: 13px; line-height: 1; transition: color .15s; }
  .brand-ref-tag .remove:hover { color: #F22F46; }
  ...
  .style-card-actions .reset-link { font-size: 11px; color: #f0983a; cursor: pointer; background: none; border: none; padding: 0; font-family: inherit; display: none; }
...
<div class="section">
  ...
  <div class="settings-panel" id="settingsPanel">
    ...
    <div class="sg-group">
    <div class="sg-group-header">Event</div>
    ...
    <div class="sg-group">
    <div class="sg-group-header">Art &amp; Branding</div>
    ...
    <div class="sg-group">
    <div class="sg-group-header">Delivery</div>
    ...
    <div class="sg-group">
    <div class="sg-group-header">Booth Display</div>
    ...
    <div class="sg"><h4>Social Sharing <span class="tip" data-tip="Include Twitter/X and LinkedIn share links in the delivery SMS so users can share their portrait with one tap.">?</span></h4>
      ...
      <div id="shareLinksSection" style="display:none">
        <div class="sf"><label>Twitter/X Handle <span class="tip" data-tip="Included in the tweet text, e.g. @twilio">?</span></label><input type="text" id="sTwitterHandle" placeholder="@twilio"></div>
        <div class="sf"><label>LinkedIn Share Text <span class="tip" data-tip="Template for LinkedIn shares. Use {eventName} as a placeholder.">?</span></label><input type="text" id="sLinkedInText" placeholder="Check out my AI portrait from {eventName}, powered by Twilio!"></div>
      </div>
    </div>
    ...
    <div class="sg-group">
    <div class="sg-group-header">Operations</div>
    ...
    <div class="sg-group">
    <div class="sg-group-header">API Keys</div>
...
function populateSettings() {
  ...
  renderBrandRefs(_files.brandReferences || []);
  ...
  for (var mi = 0; mi < msgIds.length; mi++) {
    ...
    if (el) el.value = _messages[msgIds[mi]] || "";
  }
...
// ── Brand Reference Files ──
function renderBrandRefs(files) {
  _brandRefFiles = files || [];
  var container = document.getElementById("brandRefList");
  if (_brandRefFiles.length === 0) { container.innerHTML = ""; return; }
  container.innerHTML = _brandRefFiles.map(function(f) {
    return '<span class="brand-ref-tag"><img src="/brand-references/' + encodeURIComponent(f) + '"> ' + f + ' <span class="remove" onclick="removeBrandRef(\\\'' + f.replace(/'/g, "\\\\'") + '\\\')">x</span></span>';
  }).join("");
}
...
// Hide fullscreen + BRB buttons when embedded in combo iframe
if (window.self !== window.top) {
  document.getElementById("fsBtn").style.display = "none";
  document.getElementById("brbBtn").style.display = "none";
}
