...
<div class="section">
  ...
  <div class="settings-panel" id="settingsPanel">
    ...
    <div id="userSettingsBanner" style="background:rgba(33,136,239,.08);border:1px solid rgba(33,136,239,.25);border-radius:10px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:10px">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--blue-400)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
      <span style="font-size:13px;color:var(--blue-200)">Personal Settings Mode — your changes won't affect the live SMS/print pipeline or other admins until you <strong>Save Globally</strong>.</span>
    </div>
  ...
  <div class="settings-actions" id="settingsActions">
    <button class="btn btn-primary" onclick="saveSettings()">Save My Settings</button>
    <button class="btn btn-success" onclick="saveGlobally()">Save Globally</button>
    <button class="btn btn-secondary" onclick="resetToGlobal()">Reset to Global</button>
...
async function _doSaveSettings() {
  ...
  try {
    ...
    _settings = await r.json();
    showBanner("Your settings have been saved. Use Save Globally to apply them to the live pipeline.", "success");
    return true;
...
async function saveGlobally() {
  if (!confirm("Push your current settings to the live pipeline? This will update the global settings used by SMS handlers and all admins' defaults.")) return;
  // First save the current form to personal settings
  var saved = await saveSettings();
  if (!saved) return;
  try {
    var r = await fetch("/dashboard/api/settings/save-global", { method: "POST" });
    _settings = await r.json();
    showBanner("Settings saved globally -- the live pipeline is now updated.", "success");
  } catch(e) { alert("Failed to save globally: " + e.message); }
}

async function resetToGlobal() {
  if (!confirm("Reset your personal settings to match the current global settings? Your custom changes will be lost.")) return;
  try {
    ...
    _messages = {};
    _leadCaptureFields = {};
    _backgroundChoices = (_settings.backgroundChoices || []).slice();
    populateSettings();
    showBanner("Reset to global settings.", "success");
  } catch(e) { alert("Failed to reset: " + e.message); }
}
