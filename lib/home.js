...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .style-row { display: flex; align-items: center; justify-content: space-between; padding: 9px 0; border-bottom: 1px solid #1A1E27; }
  .style-row:last-child { border-bottom: none; }
  .style-row .sname { font-size: 14px; color: #B0B8C4; }
  .style-row .slabel { font-size: 11px; color: #4D5562; margin-left: 6px; }
  ...
  .style-prompt { display: none; margin-top: 8px; padding: 10px 14px; border-radius: 8px; background: #0E1117; border: 1px solid #1A1E27; font-size: 12px; color: #636B78; line-height: 1.6; white-space: pre-wrap; word-break: break-word; }
  .style-prompt.open { display: block; }
  .prompt-toggle { padding: 0; border: none; background: none; color: #4B8BF5; font-size: 11.5px; cursor: pointer; font-family: inherit; margin-left: 8px; }
...
<div class="actions">
  ...
  <a href="/outreach/" class="action-card outreach">
    ...
    <h2>SMS Outreach</h2>
    <p>Send broadcast messages to attendees, run raffles, and manage event communications.</p>
  </a>
</div>
...
<div class="section">
  ...
  <div class="settings-panel" id="settingsPanel">
    ...
    <div class="sg"><h4>Delivery <span class="tip" data-tip="Controls how portraits reach attendees. When printing is enabled, portraits are both texted and printed. When disabled, portraits are sent via MMS only (no printer required).">?</span></h4>
      <div class="sf">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div><label style="margin-bottom:0">Enable Printing</label><div style="font-size:11.5px;color:#4D5562;margin-top:3px">When off, portraits are delivered digitally via MMS only</div></div>
          <label class="toggle-sw"><input type="checkbox" id="sEnablePrinting"><span class="slider"></span></label>
        </div>
      </div>
    </div>

    <div class="sg"><h4>Booth Hardware</h4>
      <div class="sf">
      ...
      <div class="sf">
        ...
        <select id="sTemplate"><option value="">None</option></select>
      ...
      <div class="sf">
        ...
        <select id="sVideo"><option value="">Loading...</option></select>
    ...
    <div class="sg"><h4>Art Styles <span class="tip" data-tip="Toggle built-in styles on/off. Attendees type a style name with their selfie. Disabled styles won't appear in SMS messages. Click 'prompt' to view/copy any style's AI prompt.">?</span></h4>
    ...
    <div class="sg"><h4>Promotional Messages <span class="tip" data-tip="These messages are appended to SMS replies. Leave blank to disable. First-time is sent with a user's very first photo. Returning is sent with subsequent photos and when their print is ready.">?</span></h4>
      <div class="sf"><label>First-Time User Message <span class="tip" data-tip="Appended to the confirmation SMS for first-time users only.">?</span></label><textarea id="sPromoIntro" rows="3" placeholder="e.g. P.S. Join us at SIGNAL 2025, June 25-26! Register free: https://signal.twilio.com"></textarea></div>
      <div class="sf"><label>Returning User Message <span class="tip" data-tip="Appended to confirmation and print-ready SMS for returning users.">?</span></label><textarea id="sPromoReturning" rows="3" placeholder="e.g. Have you registered for SIGNAL yet? Don't miss out: https://signal.twilio.com"></textarea></div>
    </div>
...
<div class="section">
  ...
  <div class="steps">
    ...
    <div class="step"><div class="step-num">6</div><div class="step-text">Use <a href="/outreach/" target="_blank">SMS Outreach</a> to send broadcast messages, run raffles, and engage attendees</div></div>
  </div>
</div>
...
function populateSettings() {
  ...
  document.getElementById("sEnablePrinting").checked = _settings.enablePrinting !== false;
...
function renderStyles() {
  ...
  _builtInStyles.forEach(function(s, i) {
    var checked = !disabled.includes(s.key) ? "checked" : "";
    html += '<div>'
      + '<div class="style-row"><div><span class="sname">' + s.name + '</span><span class="slabel">(built-in)</span>'
      + '<button class="prompt-toggle" onclick="togglePrompt(\\'bp'+i+'\\')">prompt</button></div>'
      + '<label class="toggle-sw"><input type="checkbox" data-style="' + s.key + '" ' + checked + '><span class="slider"></span></label></div>'
      + '<div class="style-prompt" id="bp' + i + '">' + escHtml(s.prompt) + '</div>'
      + '</div>';
  });

  Object.keys(_customStyles).forEach(function(k) {
    html += '<div class="style-row"><div><span class="sname">' + escHtml(_customStyles[k].name) + '</span><span class="slabel">(custom)</span>'
      + '<button class="prompt-toggle" onclick="togglePrompt(\\'cp_'+k+'\\')">prompt</button></div>'
      + '<button class="btn btn-danger" style="padding:4px 10px;font-size:11px" onclick="removeCustomStyle(\\''+k+'\\')">Remove</button></div>'
      + '<div class="style-prompt" id="cp_' + k + '">' + escHtml(_customStyles[k].prompt || "") + '</div>';
  });
...
async function saveSettings() {
  ...
  var body = {
    ...
    enablePrinting: document.getElementById("sEnablePrinting").checked,
