...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero .subtitle { font-size: 16px; color: #636B78; font-weight: 400; margin-top: 6px; }
  ...
  .action-card p { font-size: 14.5px; color: #636B78; line-height: 1.6; }
  ...
  .step-text { font-size: 13.5px; color: #8B95A5; line-height: 1.5; }
  ...
  .sg h4 { font-size: 14.5px; font-weight: 600; color: #E8EAED; margin-bottom: 16px; }
  .sf { margin-bottom: 16px; }
  .sf:last-child { margin-bottom: 0; }
  .sf label { display: block; font-size: 13.5px; color: #636B78; margin-bottom: 6px; font-weight: 500; }
  .sf input[type="text"], .sf input[type="number"], .sf input[type="url"], .sf select, .sf textarea {
    width: 100%; padding: 10px 14px; border-radius: 8px; border: 1px solid #252A34;
    background: #0E1117; color: #E8EAED; font-size: 15px; font-family: inherit;
  ...
  .combo-dropdown .combo-item .combo-current { font-size: 10px; color: #4B8BF5; margin-left: 6px; }
  ...
  .phone-tag { display: inline-flex; align-items: center; gap: 5px; padding: 6px 12px; border-radius: 6px; background: #1A1E27; color: #B0B8C4; font-size: 13.5px; }
  ...
  .save-msg { font-size: 13.5px; color: #2EBA54; opacity: 0; transition: opacity .3s; }
  ...
  <div class="settings-panel" id="settingsPanel">
    ...
    <div class="sg"><h4>Delivery &amp; Printing <span class="tip" data-tip="Configure delivery mode and physical printer settings. Disable printing to run digital-only (no printer required).">?</span></h4>
      ...
      <div class="sf">
        ...
        <div id="printerChecklist" style="margin-bottom:6px"><span style="color:#636B78">Loading...</span></div>
        <div style="display:flex;align-items:center;gap:10px">
          <button class="btn btn-sm" onclick="refreshPrinters()">Refresh</button>
          <span id="printerCount" style="font-size:0.8rem;color:#636B78"></span>
        </div>
      </div>
    ...
    <div class="sg"><h4>Promotional <span class="tip" data-tip="Optional follow-up SMS sent after each portrait is delivered.">?</span></h4>
      <div class="sf"><label>Promotional Message <span class="tip" data-tip="Sent as a standalone SMS after each portrait is delivered. Leave blank to disable.">?</span></label><textarea id="sPromoMessage" rows="3" placeholder="e.g. Join us at SIGNAL 2025, June 25-26! Register free: https://signal.twilio.com"></textarea></div>
    </div>
...
function renderPrinterChecklist(printers, active) {
  ...
  for (var i = 0; i < printers.length; i++) {
    ...
    html += '<label style="display:flex;align-items:center;gap:8px;padding:5px 0;cursor:pointer"><input type="checkbox" class="printer-cb" data-printer="' + name + '"' + (checked ? ' checked' : '') + '> <span style="font-family:monospace;font-size:0.85rem">' + name + '</span></label>';
  }
...
async function refreshPrinters() {
  var container = document.getElementById("printerChecklist");
  container.innerHTML = '<span style="color:#636B78">Refreshing...</span>';
  try {
