...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Action cards */
  .actions { display: grid; grid-template-columns: repeat(3, 1fr); gap: clamp(12px, 1.4vw, 20px); margin-bottom: clamp(28px, 3vw, 48px); }
  @media (max-width: 800px) { .actions { grid-template-columns: 1fr 1fr; } }
  ...
  .action-card.video { border-top: 3px solid #a371f7; }
  .action-card.video h2 { color: #a371f7; }
  .action-card.gallery { border-top: 3px solid #f0883e; }
  .action-card.gallery h2 { color: #f0883e; }
...
<div class="actions">
  ...
  <a href="/home/video" target="_blank" class="action-card video">
    <div class="card-icon">&#x1F4FA;</div>
    <h2>Launch Booth Display</h2>
    <p>Play the intro video on your booth monitor to show attendees how it works. Opens in a new tab so you can keep working here.</p>
  </a>
  <a href="/gallery/" target="_blank" class="action-card gallery">
    <div class="card-icon">&#x1F5BC;</div>
    <h2>View Gallery</h2>
    <p>Browse a live showcase of all AI-generated portraits from this event. Great for displaying on a second screen.</p>
  </a>
</div>
...
<body>
<video id="vid" controls autoplay loop muted playsinline src="/assets/${VIDEO_FILE}"></video>
<script>
// Auto-request fullscreen on first user interaction
document.addEventListener("click", function goFull(e) {
  // Don't steal fullscreen when clicking native video controls
...
// Ensure video plays (some browsers need interaction first)
...
function mountHome(app) {
    ...
    app.use("/home", router);
    console.log("\\u{1F3E0} Home page mounted at /home");
}
