...
const { buildPrintCommand: buildCupsCommand } = require("../relay-app/cups-command");
...
function buildPrintCommand(filepath, printerName, outputProfile) {
    const profile = outputProfile || settings.getOutputProfile();
    return buildCupsCommand({
        filepath,
        printerName,
...
function printImage(filepath, printerName, outputProfile) {
    return new Promise((resolve, reject) => {
        let command;
        try {
            command = buildPrintCommand(filepath, printerName, outputProfile);
        } catch (err) {
            reject(err);
            return;
        }
...
const BAD_STATUS = ["looking for printer", "disabled", "unplugged or turned off"];

function findAllHealthyPrinters() {
    return new Promise((resolve, reject) => {
        exec("lpstat -p", (err, stdout) => {
            if (err) {
                reject(new Error(`Cannot list printers: ${err.message}`));
            ...
            const printers = stdout.split("\n")
                ...
                .filter((p) => p.name && !BAD_STATUS.some((b) => p.lower.includes(b)))
                .map((p) => p.name);
            resolve(printers);
        });
    });
...
async function getActivePrinters() {
    const allHealthy = await findAllHealthyPrinters();
    const disabled = settings.get("disabledPrinters") || [];
    const selected = settings.get("activePrinters") || [];

    let result;
    if (selected.length > 0) {
        // Filter to only selected printers that are currently healthy
        result = allHealthy.filter((p) => selected.includes(p));
    } else {
        // Fallback: prefix match on printerName (original behavior)
        const baseName = settings.get("printerName");
        if (!baseName) result = allHealthy.length > 0 ? [allHealthy[0]] : [];
        else {
            const matches = allHealthy.filter((p) => p.startsWith(baseName));
            result = matches.length > 0 ? matches : [];
        }
    }

    // Filter out manually disabled printers
    if (disabled.length > 0) {
        result = result.filter((p) => !disabled.includes(p));
    }
    return result;
}
