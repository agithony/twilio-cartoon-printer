...
router.post("/api/send-message", async (req, res) => {
    ...
    for (const id of ids) {
        ...
        try {
            // Broadcast is business-initiated and may land outside the WhatsApp
            // 24h session window — route through a template when one is configured.
            await sendSms(user.phone, user.appPhone, message, null, {
                templateKey: "sendMessage",
...
router.post("/api/nudge", async (req, res) => {
    ...
    try {
        // Drop-off nudges are business-initiated and typically land outside
        // WhatsApp's 24h session window — route WhatsApp recipients through an
        // approved Content Template (falls back to SMS text for SMS users).
        await sendSms(match.phone, match.appPhone, message, null, {
            templateKey: "nudgeDropoff",
            contentVariables: { 1: match.eventName },
        });
