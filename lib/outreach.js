...
function renderUsers() {
  ...
  for (const u of users) {
    ...
    if (u.lastPrefix && u.lastEvent) {
      ...
      thumbHtml = '<a class="uthumb-link" href="' + thumbUrl + '" target="_blank" rel="noopener" title="Open latest portrait"><img class="uthumb" src="' + thumbUrl + '" loading="lazy" alt="" onerror="this.parentElement.outerHTML=\\'<span class=&quot;uthumb-empty&quot;></span>\\'"></a>';
    }
    ...
    for (var s of data) {
      ...
      (function(prefix, event, original) {
        ...
        emailInput.addEventListener("blur", async function() {
          ...
          try {
            ...
            this.value = prev;
            console.error(err);
            alert("Couldn't save email: " + err.message);
          } finally {
            this.disabled = false;
          }
        });
      ...
      (function(prefix, event, row) {
        dismissBtn.addEventListener("click", async function() {
          if (!confirm("Remove this kiosk submission from the list?\\n\\nThis does NOT delete the generated portrait — just the contact info record.")) return;
          try {
...
async function dismissDropOff(id, phone) {
  if (!confirm("Dismiss " + phone + " from drop-offs?\\n\\nThey won't get a nudge from you. If they text again later, they'll reappear.")) return;
  try {
