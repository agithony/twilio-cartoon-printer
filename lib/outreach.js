...
router.post("/api/send-message", async (req, res) => {
    ...
    for (const id of ids) {
        ...
        try {
            ...
            const hasTemplate = adapter && adapter.name === "whatsapp" && settings.getContentSid("broadcast");
            const smsMessage = /reply\s+stop/i.test(message) ? message : `${message}\n\nReply STOP to unsubscribe.`;
            const result = hasTemplate
                ? await messaging.send(user.phone, "broadcast", { 1: message }, { adapter })
                : await messaging.send(user.phone, "_raw", {}, { _body: smsMessage, requiresSession: true, ...(adapter ? { adapter } : {}) });
...
router.post("/api/nudge", async (req, res) => {
    ...
    try {
        ...
        const hasTemplate = adapter && adapter.name === "whatsapp" && settings.getContentSid("nudgeDropoff");
        const result = hasTemplate
            ? await messaging.send(match.phone, "nudgeDropoff", { 1: match.eventName }, { adapter })
