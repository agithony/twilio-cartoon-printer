...
window.onerror = function(msg, src, line) {
  ...
  if (el) el.innerHTML = '<p style="color:red;padding:20px">Error: ' + msg + ' (line ' + line + ')</p>';
};
var currentSettings = {};
var fileOptions = {};
var loadedEventName = '';
...
async function loadSettings() {
  try {
    var [sRes, fRes] = await Promise.all([
      ...
      fetch('/review/api/settings/files')
    ]);
    ...
    fileOptions = await fRes.json();
    loadedEventName = currentSettings.eventName || '';
    renderSettings();
  } catch (err) {
    document.getElementById('settingsBody').innerHTML = '<p style="color:var(--brand-red)">Failed to load settings: ' + (err.message || err) + '</p>';
  }
}
...
async function saveSettings() {
  ...
  try {
    var res = await fetch('/review/api/settings', {
      method: 'POST',
...
try { loadSettings(); } catch(e) {
  document.getElementById('settingsBody').innerHTML = '<p style="color:red;padding:20px">Init error: ' + e.message + '</p>';
}
</script>
</body></html>`;
}
