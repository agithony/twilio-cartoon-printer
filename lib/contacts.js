...
function getDropOffs(eventName, activeJobs, adminPhones) {
    ...
    for (const [key, contact] of Object.entries(contactsData)) {
        if (key.startsWith("__session__:")) continue;
        // Filter by event
...
// Session keys (__session__:<phone>) intentionally survive deleteByEvent — a phone's
// channel preference is global, not tied to a single event.
