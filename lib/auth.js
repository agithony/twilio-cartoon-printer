...
.magic-hat-svg {
  ...
  filter: drop-shadow(0 4px 8px rgba(0,0,0,.4)) drop-shadow(0 0 12px rgba(239,34,58,.3));
  transform-origin: 50% 90%;
  transition: transform .4s cubic-bezier(.2,.9,.3,1.2);
}
.magic-hat:hover .magic-hat-svg { animation: magic-hat-wobble .6s ease-in-out; transform: scale(1.1); }
.magic-hat-sparkle {
...
module.exports = { mountAuth, requireAuth, isPublicRoute, userBarSnippet, magicHatSnippet, makeReviewToken, verifyReviewToken, parseCookie, REVIEW_TOKEN_TTL };
