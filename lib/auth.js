...
function isPublicRoute(req) {
    ...
    // the read paths are public — GET / (HTML), GET /img/* (images),
    // and GET /api/* (event + photo listings). The DELETE / POST /api/*
...
router.get("/login", (req, res) => {
    ...
    const escError = (error || "").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
    const errorHtml = error ? `<div class="error">${escError}</div>` : "";
...
router.get("/callback", async (req, res) => {
    ...
    try {
        ...
        const safeUrl = (next_url.startsWith("/") && !next_url.startsWith("//")) ? next_url : "/dashboard";
...
<div class="${className}" id="${id}" onclick="(function(){var c=document.documentElement.getAttribute('data-theme')||'dark';var n=c==='dark'?'light':'dark';document.documentElement.setAttribute('data-theme',n);localStorage.setItem('twilio-theme',n)})()" title="Toggle light/dark mode">
  <svg class="icon-moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
...
module.exports = { mountAuth, requireAuth, isPublicRoute, userBarSnippet, themeToggleButton, magicHatSnippet, twModalSnippet, makeReviewToken, verifyReviewToken, parseCookie, REVIEW_TOKEN_TTL };
