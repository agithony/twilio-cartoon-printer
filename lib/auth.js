...
function isPublicRoute(req) {
    ...
    if (["/start", "/start/", "/start/open", "/start/open/"].includes(p) && (method === "GET" || method === "HEAD")) return true;
...
function safeNextPath(value, fallback = "/home") {
...
function requireAuth(req, res, next) {
    ...
    const next_url = encodeURIComponent(safeNextPath(req.originalUrl, "/home"));
...
function renderLogin(req, res, { error = "", status = 200, next = req.query.next } = {}) {
    ...
    const nextUrl = safeNextPath(next, "/home");
    const methods = [];

    if (googleEnabled) {
        methods.push(`<a class="btn-google" href="/auth/google?next=${encodeURIComponent(nextUrl)}">
...
router.get("/google", (req, res) => {
    ...
    const oauthState = makeOAuthState(safeNextPath(req.query.next, "/home"));
    setOAuthNonceCookie(res, oauthState.nonce, req);
    res.setHeader("Cache-Control", "no-store");
    res.setHeader("Pragma", "no-cache");
    res.setHeader("Referrer-Policy", "no-referrer");
    res.redirect(googleAuthUrl(req, oauthState.token));
});
...
router.post("/pin", (req, res) => {
    ...
    res.redirect(safeNextPath(req.body?.next, "/home"));
});
