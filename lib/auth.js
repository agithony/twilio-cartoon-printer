...
.user-bar {
  ...
  background: #1a2030; border: 1px solid #252d3a; border-radius: 10px;
  padding: 6px 12px 6px 6px; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
}
.user-bar-avatar {
  width: 28px; height: 28px; border-radius: 50%; object-fit: cover; border: 1px solid #333;
}
...
(function(){
  fetch('/auth/me').then(r=>r.ok?r.json():null).then(u=>{
    ...
    bar.append(img,name,btn);document.body.appendChild(bar);
  });
})();
`;
