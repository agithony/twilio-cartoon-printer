...
// Keys that are excluded from per-user settings (global + operational keys)
...
function update(changes) {
    ...
    if (isEventSwitch) {
        console.log(`🔍 [DEBUG] Event switch detected: "${oldEventName}" → "${changes.eventName}"`);
        console.log(`🔍 [DEBUG] Dropping non-eventName keys from changes:`, Object.keys(changes).filter(k => k !== "eventName"));
        changes = { eventName: changes.eventName };
    }

    console.log(`🔍 [DEBUG] update() processing keys:`, Object.keys(changes));
    console.log(`🔍 [DEBUG] enableManualReview in changes:`, "enableManualReview" in changes, changes.enableManualReview);
    console.log(`🔍 [DEBUG] enableManualReview in overrides before:`, overrides.enableManualReview);
    ...
    console.log(`🔍 [DEBUG] enableManualReview in overrides after:`, overrides.enableManualReview);
    save();
    ...
    if (newEventName !== oldEventName) {
        ...
        console.log(`🔍 [DEBUG] Saving event profile for "${oldEventName}", enableManualReview in overrides:`, overrides.enableManualReview);
        ...
        console.log(`🔍 [DEBUG] Loaded event profile for "${newEventName}", enableManualReview in profile:`, perEventOverrides.enableManualReview);
        ...
        console.log(`🔍 [DEBUG] After rebuild, enableManualReview in overrides:`, overrides.enableManualReview);
        save();
...
function userSettingsPath(email) {
    ...
    if (safe.includes("..")) throw new Error("Invalid email for settings path");
    return path.join(USER_SETTINGS_DIR, safe + ".json");
}

function getUserOverrides(email) {
    try {
        const p = userSettingsPath(email);
        if (fs.existsSync(p)) {
...
function saveUserOverrides(email, data) {
    if (!fs.existsSync(USER_SETTINGS_DIR)) fs.mkdirSync(USER_SETTINGS_DIR, { recursive: true });
    fs.writeFileSync(userSettingsPath(email), JSON.stringify(data, null, 2));
}

function getAllForUser(email) {
    return { ...DEFAULTS, ...overrides, ...getUserOverrides(email) };
}

function getForUser(email, key) {
    if (USER_EXCLUDED_KEYS.has(key)) return get(key);
    const userOvr = getUserOverrides(email);
    if (key in userOvr) return userOvr[key];
    return get(key);
}

function getMsgForUser(email, key, vars) {
    const userOvr = getUserOverrides(email);
    const userMessages = userOvr.messages || {};
    const messages = { ...DEFAULTS.messages, ...(overrides.messages || {}), ...userMessages };
    const template = messages[key] !== undefined ? messages[key] : (DEFAULTS.messages[key] || "");
    return vars ? interpolate(template, vars) : template;
}

function updateForUser(email, changes) {
    const userOvr = getUserOverrides(email);
    const globalChanges = {};
    ...
    for (const [key, value] of Object.entries(changes)) {
        ...
        // Global/excluded keys go to global settings
        if (USER_EXCLUDED_KEYS.has(key)) {
        ...
        const validated = validate(key, value);
        if (validated === undefined) continue;
        userOvr[key] = validated;
    }

    saveUserOverrides(email, userOvr);

    // Apply any global key changes through the normal update path
    if (Object.keys(globalChanges).length > 0) {
        console.log(`🔍 [DEBUG] updateForUser globalChanges keys:`, Object.keys(globalChanges));
        console.log(`🔍 [DEBUG] enableManualReview in globalChanges:`, globalChanges.enableManualReview);
        update(globalChanges);
    }

    const result = getAllForUser(email);
    console.log(`🔍 [DEBUG] updateForUser returning enableManualReview:`, result.enableManualReview);
    return result;
}

function saveUserToGlobal(email) {
    const userOvr = getUserOverrides(email);
    const globalChanges = {};
    for (const [key, value] of Object.entries(userOvr)) {
        if (!USER_EXCLUDED_KEYS.has(key)) {
            globalChanges[key] = value;
        }
    }
    if (Object.keys(globalChanges).length > 0) {
        // Direct merge into overrides (skip event-switching logic since eventName is excluded)
        for (const [key, value] of Object.entries(globalChanges)) {
            overrides[key] = value;
        }
        save();
        // Also save to current event profile
