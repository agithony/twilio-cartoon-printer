...
function getTemplateCompatibility(filename, eventName) {
    const compatibility = {
        ...DEFAULTS.templateCompatibilityOverrides,
        ...(getForEvent("templateCompatibilityOverrides", eventName) || {}),
    };
    return compatibility[path.basename(filename || "")] || "native";
}
...
function validate(key, value) {
    switch (key) {
        ...
        case "templateFilesByOrientation":
            ...
            return {
                portrait: path.basename(String(value.portrait || "").trim()),
                landscape: path.basename(String(value.landscape || "").trim()),
            };
        case "templateCompatibilityOverrides": {
            ...
            for (const [filename, mode] of Object.entries(value)) {
                if (["native", "portrait", "landscape", "both"].includes(mode)) {
                    compatibility[path.basename(filename)] = mode;
                }
            }
        ...
        return Promise.all(files.map(async (filename) => {
            try {
                ...
                const orientation = width && height
                    ...
                    : "unknown";
                return { filename, width, height, orientation };
            } catch {
                return { filename, width: null, height: null, orientation: "unknown" };
            }
        }));
    } catch {
        return [];
