...
const GLOBAL_KEYS = new Set([
    ...
    "enableWhatsApp", "whatsappTemplates",
...
const DEFAULTS = {
    ...
    // WhatsApp channel. When enabled, the app replies on whichever channel a
    // user reached us on (SMS or WhatsApp) and can send business-initiated
    // WhatsApp messages via approved Content Templates. Requires a WhatsApp
    // sender in the Messaging Service pool (or a WhatsApp-enabled From number).
    enableWhatsApp: process.env.ENABLE_WHATSAPP === "true",
    // Approved WhatsApp Content Template SIDs (HXxxxxxxxx...), keyed by the
    // message they back. WhatsApp requires an approved template for messages
    // sent OUTSIDE the 24h customer-service window (drop-off nudges, delayed
    // NPS surveys). Leave blank to fall back to free-form text — which only
    // succeeds while still inside the session window. See .env.example for the
    // template bodies to submit for approval in the Twilio Content Editor.
    whatsappTemplates: {
        nudgeDropoff: process.env.WHATSAPP_TEMPLATE_NUDGE || "",
        npsPrompt: process.env.WHATSAPP_TEMPLATE_NPS || "",
        sendMessage: process.env.WHATSAPP_TEMPLATE_SEND_MESSAGE || "",
    },
    // OpenAI
...
// Resolve the approved WhatsApp Content Template SID for a message key, or ""
// when none is configured. Used by sendSms() to route business-initiated
// WhatsApp messages through an approved template.
function getWhatsAppTemplate(key) {
    const map = get("whatsappTemplates") || {};
    return (map[key] || "").trim();
}
