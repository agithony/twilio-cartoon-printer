...
function get(key) {
    ...
    if (key === "contentTemplates") {
        const templates = { ...(overrides.contentTemplates || {}) };
        for (const [templateKey, sid] of Object.entries(DEFAULTS.contentTemplates || {})) {
            if (sid) templates[templateKey] = sid;
        }
        return templates;
    }
