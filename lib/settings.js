...
// Backward-compat: compute default promo messages from old env vars
function _computeDefaultPromoIntro() {
    const name = process.env.PROMO_EVENT_NAME || "";
    const date = process.env.PROMO_EVENT_DATE || "";
    const url = process.env.PROMO_EVENT_URL || "";
    if (!name || !url) return "";
    return `P.S. Join us at ${name}${date ? `, ${date}` : ""}! It's Twilio's annual flagship developer conference and it's free to attend. Register here: ${url}`;
}

function _computeDefaultPromoReturning() {
    const name = process.env.PROMO_EVENT_NAME || "";
    const date = process.env.PROMO_EVENT_DATE || "";
    const url = process.env.PROMO_EVENT_URL || "";
    if (!name || !url) return "";
    return `Have you registered for ${name} yet? It's Twilio's annual flagship developer conference${date ? ` -- ${date}` : ""}. Don't miss out, it's free to attend! Register here: ${url}`;
}
...
const DEFAULTS = {
    ...
    promoIntro: _computeDefaultPromoIntro(),
    promoReturning: _computeDefaultPromoReturning(),
...
function getPromoIntro() {
    const text = get("promoIntro");
    return text ? `\n\n${text}` : "";
}

function getPromoReturning() {
    const text = get("promoReturning");
    return text ? `\n\n${text}` : "";
}
...
function getActiveStyles() {
    ...
    return gas(get("disabledStyles"), get("customStyles"));
}

function getActiveStyleList() {
    const { getActiveStyleList: gasl } = _getStylesModule();
    return gasl(get("disabledStyles"), get("customStyles"));
}
