...
const { atomicWriteFile, renameWithRetry } = require("./atomic-write");
const relayHeartbeats = require("./relay-heartbeats");
const { acquireRelayJobLock } = require("./relay-job-lock");

const QUEUE_TMP_DIR = path.join(path.dirname(READY_DIR), ".tmp");
...
function relaySupportsLandscape(version) {
...
function relayRequiresEvent(req) {
    return req.headers["x-relay-event-filter"] === "required";
}

function isLandscapeJob(job) {
    return job.outputProfile && (job.outputProfile.orientation === "landscape" || job.outputProfile.printSize === "6x4");
}

function claimMatches(job, claimId) {
    if (job.claimId) return claimId === job.claimId;
    if (job.terminalClaimId) return claimId === job.terminalClaimId;
    if (job.claimRevokedAt) return false;
    return !claimId;
}
...
async function atomicWriteJob(filePath, data) {
    await atomicWriteFile(filePath, data, { tempDir: QUEUE_TMP_DIR });
}
...
// GET /events — list event names that a relay can explicitly select.
router.get("/events", (req, res) => {
    try {
        const currentEvent = settings.get("eventName") || "";
        const events = new Set(settings.listEvents().filter((name) => typeof name === "string" && name));
        if (currentEvent) events.add(currentEvent);
        res.json({ events: [...events].sort(), currentEvent });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// GET /jobs — list jobs in ready state (optionally filtered by printer and event)
router.get("/jobs", async (req, res) => {
    try {
        ...
        if (!eventFilter && relayRequiresEvent(req)) {
            return res.status(400).json({ error: "An event selection is required" });
        }
        const supportsLandscape = relaySupportsLandscape(req.headers["x-relay-version"]);
        ...
        for (const f of files) {
            try {
                ...
                if (job.completedAt || job.deliveryPending || job.failureEffectsPending) continue;
                if (eventFilter && job.eventName !== eventFilter) continue;
                // Targeted jobs are visible only to the named station.
                if (job.targetPrinter && job.targetPrinter !== printerFilter) continue;
                if (isLandscapeJob(job) && !supportsLandscape) continue;
                ...
                jobs.push({
                    ...
                    style: job.style,
                    readyAt: job.readyAt,
                    // Masked phone so the relay UI can render a row that matches the dashboard (thumbnail, masked
                    ...
                    outputProfile: job.outputProfile || settings.getOutputProfile(job.eventName),
                });
...
router.post("/jobs/:filename/ack", async (req, res) => {
    ...
    if (requestBody.eventName !== undefined && typeof requestBody.eventName !== "string") {
        return res.status(400).json({ error: "Invalid event filter" });
    }
    const eventFilter = requestBody.eventName || null;
    if (!eventFilter && relayRequiresEvent(req)) {
        return res.status(400).json({ error: "An event selection is required" });
    }
    const supportsClaimProtocol = relaySupportsLandscape(req.headers["x-relay-version"]);
    const readyPath = path.join(READY_DIR, filename);
    const printingPath = path.join(PRINTING_DIR, filename);

    if (eventFilter) {
        try {
            const candidate = JSON.parse(await fsp.readFile(readyPath, "utf-8"));
            if (candidate.eventName !== eventFilter) {
                return res.status(404).json({ error: "Job not found for selected event" });
            }
        } catch (err) {
            if (err.code === "ENOENT") {
    ...
    try {
        ...
        if (job.completedAt || job.deliveryPending || job.failureEffectsPending) {
            const { settlePendingRelayTransition } = require("./queue");
            await settlePendingRelayTransition(filename);
            return res.status(409).json({ error: "Job is completing terminal effects" });
        }
        if (eventFilter && job.eventName !== eventFilter) {
            await renameWithRetry(printingPath, readyPath);
            return res.status(404).json({ error: "Job not found for selected event" });
        }
        if (isLandscapeJob(job) && !relaySupportsLandscape(req.headers["x-relay-version"])) {
            await renameWithRetry(printingPath, readyPath);
            return res.status(426).json({ error: "Print Station 1.3.0 or newer is required for landscape jobs" });
        }
        const now = Date.now();
        job.printingAt = now;
        job.stateChangedAt = now;
        job.printerName = requestBody.printerName || "relay";
        if (supportsClaimProtocol) job.claimId = crypto.randomUUID();
        else delete job.claimId;
        ...
        if (!job.outputProfile) {
            ...
            job.outputProfile = settings.getOutputProfileForDimensions(metadata.width, metadata.height, job.eventName);
        }

        await atomicWriteJob(printingPath, JSON.stringify(job));
        try { require("./photogallery").rememberPhotoGalleryJob(job); } catch {}
        ...
        res.json({
            ...
            job: {
                ...
                // raw number. The relay reuses the downloaded print PNG for its preview.
                userPhone: job.userPhone ? maskPhoneForRelay(job.userPhone) : null,
                outputProfile: job.outputProfile || settings.getOutputProfile(job.eventName),
            },
        });
        ...
        await renameWithRetry(printingPath, readyPath).catch((rollbackErr) => {
            console.error(`Failed to return relay claim ${filename} to ready: ${rollbackErr.message}`);
        });
        res.status(500).json({ error: err.message });
    }
});
...
router.post("/jobs/:filename/heartbeat", async (req, res) => {
    ...
    try {
        ...
        const job = JSON.parse(await fsp.readFile(printingPath, "utf-8"));
        if (!claimMatches(job, req.body.claimId)) {
            return res.status(409).json({ error: "Print claim no longer belongs to this station" });
        }
        relayHeartbeats.record(filename, job.claimId || "legacy");
        res.json({ ok: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post("/jobs/:filename/complete", async (req, res) => {
    const { filename } = req.params;
    if (!safeName(filename)) return res.status(400).json({ error: "Invalid filename" });
    const { success, error: errorMsg, claimId } = req.body || {};
    const printingPath = path.join(PRINTING_DIR, filename);
    ...
    try {
        ...
        if (!fs.existsSync(printingPath)) {
            ...
            if (fs.existsSync(donePath)) {
                const job = JSON.parse(await fsp.readFile(donePath, "utf-8"));
                if (job.terminalClaimId && claimId !== job.terminalClaimId) {
                    return res.status(409).json({ error: "Print claim no longer belongs to this station" });
                }
            ...
            if (fs.existsSync(failedPath)) {
                const job = JSON.parse(await fsp.readFile(failedPath, "utf-8"));
                if (job.terminalClaimId && claimId !== job.terminalClaimId) {
                    return res.status(409).json({ error: "Print claim no longer belongs to this station" });
                }
                await finishPendingRelayEffects(failedPath, filename);
                return res.json({ ok: true, state: "already_failed" });
            }
...
// POST /jobs/:filename/reprint — re-queue a completed or failed job so the relay prints
...
// must be terminal, not already queued, and its output image must still exist.
// reprint=true keeps it out of quota and (with smsSentAt) suppresses a dup SMS.
router.post("/jobs/:filename/reprint", (req, res) => {
    const { filename } = req.params;
    if (!safeName(filename)) return res.status(400).json({ error: "Invalid filename" });
    const requestBody = req.body || {};
    if (requestBody.eventName !== undefined && requestBody.eventName !== null && typeof requestBody.eventName !== "string") {
        return res.status(400).json({ error: "Invalid event filter" });
    }
    if (!requestBody.eventName && relayRequiresEvent(req)) {
        return res.status(400).json({ error: "An event selection is required" });
    }
    ...
    const result = requeueDoneJobForReprint(filename, {
        targetPrinter: requestBody.printerName || null,
        eventName: requestBody.eventName || null,
    });
...
router.get("/status", (req, res) => {
    res.json({
        ...
        outputProfile: settings.getOutputProfile(),
        minimumLandscapeRelayVersion: "1.3.0",
    });
});
...
module.exports = { mountPrintRelay, getRelayPrinters, shouldHideFromPrinter, relaySupportsLandscape, claimMatches, relayHeartbeats };
