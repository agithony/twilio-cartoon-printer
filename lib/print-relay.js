    ...
    try {
        ...
        for (const f of files) {
            try {
                ...
                // Skip jobs that already failed on this printer (let a different relay claim it).
                // On the last retry before MAX_RETRIES, stop filtering so any printer can try.
                const failed = job.failedPrinters || [];
                if (printerFilter && failed.includes(printerFilter) && (job.retries || 0) < MAX_RETRIES - 1) continue;
                jobs.push({
                    filename: f,
...
module.exports = { mountPrintRelay, getRelayPrinters };
