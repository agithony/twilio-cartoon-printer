...
function findShareUrl(filePrefix) {
    try {
        const files = fs.readdirSync(DONE_DIR);
        for (const f of files) {
            if (f.startsWith(filePrefix) && f.endsWith(".json")) {
                const job = JSON.parse(fs.readFileSync(path.join(DONE_DIR, f), "utf-8"));
                return job.shareUrl || null;
            }
        }
    } catch { /* ignore */ }
    return null;
...
router.get("/:filePrefix", (req, res) => {
    ...
    // Prefer the short link in social share URLs for cleaner posts
    const socialUrl = findShareUrl(filePrefix) || sharePageUrl;
