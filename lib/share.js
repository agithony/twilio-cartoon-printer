...
function resolveImage(filePrefix, eventName) {
    ...
    const imagePath = path.join(dlDir, imageFile);
...
router.get("/:filePrefix/img", (req, res) => {
    const eventName = req.query.e || settings.get("eventName");
    const { imagePath, exists } = resolveImage(req.params.filePrefix, eventName);
    if (!exists) return res.status(404).end();
    res.sendFile(imagePath);
});

router.get("/:filePrefix", (req, res) => {
    const filePrefix = req.params.filePrefix;
    const eventName = req.query.e || settings.get("eventName");
    ...
    // Settings
    const pageTitle = settings.getForEvent("sharePageTitle", eventName) || "My AI Portrait";
    ...
    // Build intent URLs — both use sharePageUrl so crawlers see OG meta tags
    const twitterIntentUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(twitterText)}&url=${encodeURIComponent(sharePageUrl)}`;
