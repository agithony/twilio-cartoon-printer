...
function sanitizeEventParam(s) {
    return String(s || "").replace(/[^a-zA-Z0-9_\-\s]/g, "").trim();
}
...
function resolveImage(filePrefix, eventName, suffix = "_output.png") {
    const dlDir = settings.getDownloadDir(eventName);
    const imageFile = `${filePrefix}${suffix}`;
...
router.get("/:filePrefix/img", (req, res) => {
    ...
    const eventName = job && job.eventName || sanitizeEventParam(req.query.e) || settings.get("eventName");
    const { imageFile, imagePath, exists } = resolveImage(filePrefix, eventName);
    if (!exists) return res.status(404).end();
    res.type("png");
    res.setHeader("Cache-Control", "public, max-age=0, must-revalidate");
    res.setHeader("Content-Disposition", `inline; filename="${imageFile}"`);
    res.sendFile(imagePath);
});

// The compressed derivative is transport-only and is exposed solely for Twilio MMS fetches.
router.get("/:filePrefix/mms", (req, res) => {
    const filePrefix = sanitizeParam(req.params.filePrefix);
    const job = findJob(filePrefix);
    const eventName = job && job.eventName || sanitizeEventParam(req.query.e) || settings.get("eventName");
    const { imageFile, imagePath, exists } = resolveImage(filePrefix, eventName, "_output_mms.jpg");
    if (!exists) return res.status(404).end();
    res.type("jpeg");
    res.setHeader("Cache-Control", "public, max-age=0, must-revalidate");
    res.setHeader("Content-Disposition", `inline; filename="${imageFile}"`);
...
router.get("/:filePrefix/download", (req, res) => {
    const filePrefix = sanitizeParam(req.params.filePrefix);
    const job = findJob(filePrefix);
    const eventName = job && job.eventName || sanitizeEventParam(req.query.e) || settings.get("eventName");
    const { imageFile, imagePath, exists } = resolveImage(filePrefix, eventName, "_output.png");
    if (!exists) return res.status(404).end();
    res.type("png");
    res.setHeader("Cache-Control", "public, max-age=0, must-revalidate");
    res.download(imagePath, imageFile);
});
...
router.get("/:filePrefix", (req, res) => {
    ...
    const { exists } = resolveImage(filePrefix, eventName);
    ...
    const downloadUrl = `${baseUrl}/s/${filePrefix}/download?e=${eParam}`;
    const downloadFile = `${filePrefix}_output.png`;
    ...
    if (instagramEnabled) {
        buttonsHtml += `
        <a href="${escHtml(downloadUrl)}" download="${escHtml(downloadFile)}" class="share-btn share-btn-instagram" data-share-platform="instagram">
    ...
    buttonsHtml += `
    <a href="${escHtml(downloadUrl)}" download="${escHtml(downloadFile)}" class="share-btn share-btn-download" data-share-platform="download">
