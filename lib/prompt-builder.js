...
function build(input) {
    ...
    const isStickerStyle = !!(styleObj && styleObj.sticker);
    if (isStickerStyle) {
        // Sticker prompts carry their own transparent-background instruction.
        // Never append a competing background line (reference, exact, default).
    } else if (bgMode === "ai" && bgRefBuffers.length > 0 && bgAnalysis) {
        ...
        fullPrompt += "\nBackground: The area around the subject must be fully transparent with no environment, no floor, no shadow cast onto a surface, and no color fill. Render only the subject as a cut-out. The transparent area will be replaced with the actual background image in post-processing.";
