...
function build(input) {
    ...
    if (bgMode === "ai" && bgRefBuffers.length > 0 && bgAnalysis) {
        ...
        fullPrompt += "\nBackground: Output a fully transparent background (alpha = 0) around the subject. Render only the subject with no environment, no floor, no shadow cast onto a surface, and no color fill behind them. The transparent area will be replaced with the exact uploaded background image in post-processing, so the model must not draw any background at all.";
